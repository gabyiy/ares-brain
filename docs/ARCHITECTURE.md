...
The bounded hardware probe `scripts/manual_diagnose_active_lifecycle_audio.py` and `scripts/run_ares_standby_voice.py` consume one authoritative production active-audio factory. That factory owns CLI-to-adapter mapping, the real ALSA microphone adapter, canonical ACTIVE request profile, constrained lifecycle-audio recognizer, and bounded Whisper helper. The diagnostic no longer manually reconstructs a similar-looking graph. It receives the production pipeline infrastructure, observes the same finalized-audio decision in compare-only mode, constructs no `BrainRuntime`, and executes no lifecycle transition, CoreService request, skill, or owner-memory operation.

`SingleTurnVoicePipeline` exposes one typed `finalized_audio_hook` after capture has stopped, the writer is closed, canonical WAV validation and minimum-RMS validation have passed, and before Whisper starts. The hook receives the existing `AudioChunk`; its `wav_path` and `final_whisper_input_path` identify the same closed 16 kHz mono signed 16-bit little-endian WAV. `SingleTurnFinalizedAudioDecision` has one exclusive outcome: handle the turn without Whisper, or continue to Whisper. The lifecycle recognizer does not copy or recapture audio. Production handles a high-confidence lifecycle result or a medium-confidence confirmation request at this boundary; ordinary, low, unmatched, missing-confidence, timeout, and backend-failure decisions continue into the existing Whisper adapter with the same `AudioChunk`. The diagnostic records the constrained result but deliberately continues to Whisper for side-by-side reporting on that same file. Comparison Whisper remains mandatory for ordinary phrases, but cannot invalidate high-confidence lifecycle evidence that production would have handled without Whisper.

`core.ActiveLifecycleAudioRecognizer` is a pure evidence component. Production `VoskLifecycleGrammarBackend` owns one reusable spawned worker process that loads the configured local Vosk model once and creates a fresh constrained recognizer for each finalized candidate. The request deadline bounds child readiness, model loading, recognizer construction, native Vosk calls, PCM decoding, and result waiting. Parent-side `multiprocessing.Process.start()` and pipe `send()` remain synchronous setup calls and are not independently preempted by this helper. When a bounded child wait expires, cleanup terminates, kills if necessary, and requires the worker to be reaped; unproven cleanup fails closed and prevents a competing worker. A later turn can start a fresh worker only after cleanup is confirmed. Its 105-entry action/rejection grammar contains 21 exact action phrases and 84 bounded non-action acoustic/negative/descriptive competitors, plus `[unk]`; competitors are outside the lifecycle phrase policy and therefore always reject as ordinary. The rejection side consists of 36 explicit phrases and a 48-phrase bounded negation expansion. The result contains `classification`, `canonical_phrase`, `recognized_text`, complete `recognized_tokens`, minimum aligned word confidence, confidence availability/tier, backend, proposed classification, confirmation requirement, and rejection reason. It has no microphone dependency, capture method, runtime/session reference, CoreService, memory service, or transition callback. Only `BrainSessionManager` may apply the action after the runtime has accepted the evidence or completed confirmation.

This boundary follows real hardware isolation, not a synthetic assumption. The repaired production-composed Raspberry Pi probe opened `plughw:2,0`, read a nonzero complete 640-byte canonical frame, and finalized all four requested candidates. Ordinary Whisper correctly transcribed calculator and owner-memory speech, while short lifecycle audio produced `*clears throat*` and `Shut down, artist.`. The capture checkpoint is therefore closed; constrained lifecycle recognition on those same owner WAVs remains a separate Raspberry Pi acceptance requirement. The diagnostic exits nonzero unless goodbye and shutdown are high-confidence exact canonical actions with no production Whisper fallback and both ordinary phrases reject lifecycle action and request fallback; only then does it print `all four phrases passed the constrained lifecycle policy`.
...
Typed/simulated ACTIVE lifecycle matching remains deterministic and whole-phrase based for compatibility tests. Real audio authorization uses only the narrower constrained grammar and confidence policy below. Negated, descriptive, scheduled, embedded, or otherwise longer phrases do not execute lifecycle actions in either path.

The diagnostic-only production composition banner identifies the resolved runtime, lifecycle authority, standby listener, active input adapter, pipeline, ALSA recorder, constrained Vosk recognizer, Whisper adapter, shared gate, and routing revision. Each `ACTIVE COMMAND DIAGNOSTIC` reports capture readiness/timing, canonical WAV evidence, constrained text/tokens/confidence/tier/backend/classification/canonical/rejection, confirmation status, selected action, whether Whisper fallback ran, the optional raw/cleaned Whisper transcript, routing bypass, pre/post state and session, cleanup, and terminal reason. These values remain terminal-only under diagnostic flags and are not written to events, memory, or operational logs.
...
`core.LifecycleControl` retains its deterministic state-specific text entry points for simulated input, attention-only handling, diagnostics, and conservative command shaping. Real ACTIVE audio authorization now occurs earlier at the constrained finalized-WAV boundary. A Whisper transcript alone cannot bypass the lifecycle-audio confidence and confirmation policy to authorize shutdown. Text normalization still returns raw, cleaned, command-body, canonical-name, removed-alias/type/position, negation, action, matched-phrase, and rejection fields, but audio-derived lifecycle execution requires the associated accepted constrained result. Arbitrary substrings are never rewritten and there is no fuzzy, edit-distance, semantic, or learned-alias path.

The audio grammar is deliberately narrower than standby wake aliases and historical Whisper acoustic forms. Standby permits exactly `goodbye ares`, `goodbye aris`, `good bye ares`, `good bye aris`, `bye ares`, `bye aris`, `go standby ares`, `standby ares`, and `sleep ares`. Shutdown permits exactly `shutdown ares`, `shutdown aris`, `shut down ares`, `shut down aris`, `ares shutdown`, `aris shutdown`, `ares shut down`, `aris shut down`, `turn off ares`, `turn off aris`, `power off ares`, and `power off aris`. Bounded negative competitors include name-only speech and the observed `artist`, `paris`, `harris`, `aries`, `rs`, `clears throat`, wrong-name lifecycle phrases, `shutdown computer`, `goodbye everyone`, `go to sleep`, and `turn it off`, plus every finite combination of `do not`/`don't`/`never`, the shutdown/goodbye/sleep/standby roots, and `ares`/`aris`. These can never select an action. Vosk also receives `[unk]` as an explicit rejection slot.

Each constrained request gives the child work a 5-second deadline; timeout cleanup then has a 0.25-second terminate grace and 0.25-second kill/reap grace. This is a bounded child-readiness/native/result contract, not a claim that synchronous parent `Process.start()` or pipe `send()` can be preempted mid-call. Standby confidence tiers are high at `0.70` and medium at `0.50`; shutdown is stricter at `0.78` and `0.60`. Constructors reject configuration below safety floors of standby `0.60`/`0.40`, shutdown `0.70`/`0.50`, and positive confirmation `0.75`. Confidence is the minimum word confidence only when backend tokens align exactly with the normalized recognized phrase. Missing or unaligned confidence fails closed to ordinary fallback. Medium evidence returns `uncertain` plus the proposed action and cannot transition. Capital/Core stores one pending action bound to the current session and a 10-second monotonic deadline, asks one action-specific question through the normal gated output adapter, and evaluates the next finalized WAV against only `yes`, `yes ares`, `confirm`, the matching `confirm standby`/`confirm shutdown`, `no`, `cancel`, `never mind`, or `continue`. Positive confirmation requires `0.80`; cancellation requires `0.50`. Cancellation, wrong-action confirmation, expiry, a consumed unmatched turn, session replacement, or lifecycle-state change clears the pending request without executing it.
...
After capture, the finalized-audio hook runs before Whisper. High-confidence exact lifecycle evidence is transported as a typed command and bypasses CoreService and Whisper. Medium evidence is transported as a typed confirmation request and bypasses both until the next owner turn resolves or clears it. Otherwise the same audio enters Whisper and the untouched transcript remains available at its dedicated pre-normalization hook; this fallback is ordinary input and cannot independently authorize an audio lifecycle transition. Completion of any bounded voice turn is transport-local, not completion of the persistent foreground runtime. Active-command EOF, empty or falsey transcription, no speech, recognizer/backend failure, timeout, lifecycle mismatch, and transcription failure remain nonterminal. Only a genuine routed owner command records activity and resets the manager's 30-second deadline; attention-only input, ARES playback, empty/no-usable capture, diagnostic output, and a pending lifecycle guess do not. At expiry the manager returns through `RETURNING_TO_STANDBY` without terminating the foreground process.
...
Active command inference is now bounded at the speech-engine process group, not by a post-hoc elapsed-time check. `SingleTurnVoiceRequestV1.transcription_timeout_seconds` is carried in the finalized `AudioChunk` metadata to `LinuxWhisperSpeechToTextAdapter`; the foreground Raspberry Pi launcher sets a 15-second inference timeout independently of its broader 300-second process budget. `WhisperSubprocessRunner` uses `Popen(..., shell=False, start_new_session=True)` on Linux and records PID/PGID. stdout and stderr are drained through separate bounded temporary files instead of inherited pipes. Timeout sends `SIGTERM` to the entire group, waits at most one second, escalates to group `SIGKILL`, and has a three-second hard cleanup deadline covering termination, reap, and handle closure. The runner exposes the same cancellation boundary to Ctrl+C and pipeline shutdown. It reports completion only after real cleanup state is known.
...
Deterministic tests prove wake-state isolation, `attention_only` session/deadline stability, exactly one activation acknowledgement, exact constrained grammar and confidence boundaries, false-positive rejection, high-confidence Whisper bypass, same-WAV ordinary fallback, session-bound confirmation/cancellation/expiry, CoreService bypass, ordinary calculator and owner-memory routing, session clearing/reactivation, inactivity, single-turn/EOF nontermination, playback gating, one prompt per capture phase, Whisper timeout cleanup, event-lock degradation, and Ctrl+C cleanup. They cannot prove the owner's Raspberry Pi accent produces the required constrained text/confidence. The owner-run acceptance sequence must therefore activate, calculate two plus two, return with constrained `goodbye Ares`, reactivate into a new session, store and recall the video-game memory, and stop once with constrained `shutdown Ares`. `scripts/run_ares_standby_voice.py` intentionally remains foreground until an authorized shutdown control or Ctrl+C. Missing local dependencies fail before microphone capture. No script installs a service or starts at boot.

Real Raspberry Pi output exposed several independent defects. Historical Whisper normalization accepted bounded acoustic name forms such as `Aris`, `Aries`, or `RS`, but that transcript policy is no longer an audio lifecycle authority; the constrained ACTIVE grammar now accepts only `ares` and `aris` in its explicit complete phrases. Earlier one-shot standby capture was replaced by a persistent stream and one-time calibration. A later 6/10 run then reported seven opens and calibrations because the reliability verifier activated and returned to standby after each accepted prompt, intentionally triggering microphone ownership handoffs; rejected candidates were already reusing the stream. The corrected reliability phase measures recognition without activation and leaves one stream/calibration in place. Real Vosk results of `ares` at `0.574` and `0.616` also proved the prior wake threshold too strict, resulting in the bounded wake-only `0.55`/`0.40` policy. A later successful activation crashed before acknowledgement because an old timestamp-only `event_history.json.lock` raised `MigrationError` inside a fake single-turn output path. Event telemetry is now fail-open, production uses one shared store, acknowledgement is output-only, and owner-aware lock inspection/recovery plus process exclusivity prevent recurrence without weakening critical-memory locks. Another hardware run reached `Transcribing command`; active Whisper now has its own process-group boundary and cleanup deadline. Command mode retains its independently validated 15-second utterance maximum and 0.9-second terminal-silence policy.
