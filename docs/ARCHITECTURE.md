...
PCM failures retain a typed stage such as microphone open, PCM read, invalid frame, VAD, WAV write, or Whisper; the exception class and complete message; failing adapter and method; requested format/device; stream state; process context; ALSA stderr; and cleanup outcome. An explicit diagnostic run may print the full traceback. Normal production output remains bounded and does not expose tracebacks. The earlier implementation reduced low-level exceptions to `pcm_stream_error:<class>`, so the exact RuntimeError and ALSA stderr from the first Raspberry Pi attempt are not recoverable and are not inferred. Passing deterministic tests proves composition, sequencing, error preservation, and cleanup; it does not prove Raspberry Pi audio. Hardware acceptance begins only when a full PCM frame reaches the recorder, owner speech yields a candidate longer than zero, and the retained WAV is audible. Lifecycle recognition remains unevaluated while capture fails.
...
`core.LifecycleControl` provides two state-specific entry points over the same deterministic token rules. `normalize_lifecycle_command()` remains the standby/wake-compatible complete-phrase boundary. `normalize_active_lifecycle_command()` is authoritative after successful active Whisper transcription and before generic `TranscriptNormalization`, `CoreService`, parser, planner, or skill routing. It returns raw, cleaned, command-body, canonical-name, removed-alias/type/position, negation, action, matched-phrase, and rejection fields. It lowercases, removes ordinary punctuation, collapses whitespace, maps `good bye` to `goodbye`, `stand by` to `standby`, and `shut down` to `shutdown`. One validated assistant alias may be removed only when it occupies a complete prefix or suffix token slot. Canonical/pronunciation aliases and the bounded active acoustic `rs` form are accepted at that edge; arbitrary substrings are never rewritten. Matching remains exact over the resulting complete command body, with no fuzzy, edit-distance, semantic, or substring path.

Active attention and lifecycle commands are derived from bounded roots rather than an open vocabulary. Exact name-only or greeting-plus-name input (`Ares`, `Aris`, `RS`, `Hey Ares`, `Hello Ares`) becomes `attention_only`: it bypasses CoreService, keeps the same session ID and inactivity deadline, and is silent when the optional `already_active_acknowledgement` is empty. The standby roots are `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; the shutdown roots are `shutdown`, `turn off`, `power down`, and `stop runtime`. A root may be bare or carry one supported assistant address at either outer edge. Standby transitions `ACTIVE -> RETURNING_TO_STANDBY -> STANDBY`, clears the session through `BrainSessionManager`, and leaves the foreground process alive. Shutdown transitions through `SHUTTING_DOWN -> STOPPED` and exits once. Negation (`do not`, `don't`, `dont`, `never`, `should not`, `shouldn't`) is detected before lifecycle execution. Consequently negated controls, explanatory or scheduled shutdown sentences, `paris`, `harris`, zodiac questions, embedded `rs`, multiple/ambiguous addresses, and lifecycle words inside longer input remain ordinary. Cross-category collisions and malformed configuration fail closed.
...
After Whisper succeeds, `SingleTurnVoicePipeline` exposes the untouched transcript at a dedicated pre-normalization hook. The active transport and `BrainRuntime` apply the authoritative active-state normalizer before requesting any manager transition or ordinary route. `attention_only`, standby, and shutdown bypass CoreService; only action `none` with usable text continues through the existing route using the normalized command body. Completion of that bounded voice turn is transport-local, not completion of the persistent foreground runtime. Active-command EOF, empty or falsey transcription, no speech, timeout, lifecycle mismatch, and transcription failure remain nonterminal. Only a genuine routed owner command records activity and resets the manager's 30-second deadline; attention-only input, ARES playback, empty/no-usable capture, and diagnostic output do not. At expiry the manager returns through `RETURNING_TO_STANDBY` without terminating the foreground process.
...
Production terminal status makes this handoff visible with `ARES is waiting for your command...`, capture-start, speech-detected, command-captured, transcribing, and processing messages. The waiting message is emitted only after the settle wait succeeds and `active_command` owns the capture gate; a shorter runtime poll that expires inside settling emits no prompt, eliminating duplicate messages for one phase. With `--diagnostic-routing`, the active-command section reports raw Whisper transcript, cleaned transcript, removed assistant alias and prefix/suffix/standalone position, lifecycle-normalized command body, negation, classification/action, CoreService bypass, state/session before and after, pipeline status, runtime-terminal flag/reason, capture timing, canonical WAV metadata, Whisper timing, routing, cleanup, and terminal-silence status. Wake-candidate and active-command diagnostics are separate; neither transcript enters events, memory, or persistent logs.
...
Deterministic tests prove wake-state isolation, `attention_only` session/deadline stability, exactly one activation acknowledgement, bare and addressed lifecycle roots, compound-word normalization, negation/long-sentence rejection, CoreService bypass, ordinary calculator and owner-memory routing, session clearing/reactivation, inactivity, single-turn/EOF nontermination, playback gating, one prompt per capture phase, Whisper timeout cleanup, event-lock degradation, and Ctrl+C cleanup. They cannot prove physical Raspberry Pi transcription or acoustic settling. The owner-run acceptance sequence must therefore activate once, repeat `Ares` without another acknowledgement, calculate two plus two under the same session, return with `goodbye Ares`, reactivate with a new session, and stop with `Ares shut down`; separate runs must prove `goodbye Aris` returns to standby and `do not shut down Ares` stays active. `scripts/run_ares_standby_voice.py` intentionally remains foreground until an exact shutdown control or Ctrl+C. Missing local dependencies fail before microphone capture. No script installs a service or starts at boot.
