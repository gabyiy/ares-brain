...
`core.ActiveLifecycleAudioRecognizer` is a pure evidence component. Production `VoskLifecycleGrammarBackend` owns one isolated non-daemon worker for one finalized candidate. The worker loads the local Vosk model, accepts one versioned request, returns one correlated result, acknowledges one explicit stop request, and exits before the result can reach `BrainRuntime`. The `ares.active_lifecycle_worker v1` protocol defines `ready`, `recognize_request`, `recognize_result`, `stop_request`, `stopped`, and `error` messages with one unique request ID. Parent and child exchange only size-capped, length-prefixed JSON over a private nonblocking socket pair. Each send/read/select and the overall request are monotonic-deadline bounded; an invalid length, payload over 32 KiB, malformed JSON, wrong protocol/type/request ID, EOF, crash, or timeout fails closed. The production path contains no raw `multiprocessing.Connection.send()`/`recv()` and no pickle-framed lifecycle message.

After receiving a recognition result, the parent deliberately withholds it while cleanup requests stop, waits for the correlated acknowledgement, closes transport endpoints, performs a bounded join, terminates and joins again, kills if supported and still necessary, verifies death, and reaps/closes the process handle. Cleanup failure propagates as a typed backend failure and remains eligible for the existing bounded same-WAV Whisper fallback/retry; it never releases an apparently successful action while a worker remains live. Foreground SIGINT remains parent-owned; the child ignores it so Ctrl+C cannot produce a child `KeyboardInterrupt` traceback or masquerade as a spoken lifecycle action. The loaded grammar has exactly 42 standby expansions, 36 shutdown expansions, 49 bounded non-action competitors, and `[unk]` (128 alternatives). Its result preserves raw recognized text/tokens and separately reports the detected lifecycle-slot alias, prefix/suffix position, alias-canonicalized transcript, canonical phrase, confidence/tier, classification, selected action, fallback decision, rejection reason, and request/cleanup timestamps. It has no microphone dependency, capture method, runtime/session reference, CoreService, memory service, or transition callback. Only `BrainSessionManager` may apply the action.

This boundary follows real hardware isolation, not a synthetic assumption. The repaired production-composed Raspberry Pi probe opened `plughw:2,0`, read nonzero complete canonical PCM, finalized every requested candidate, and later proved ordinary Whisper completion. The subsequent production failure occurred after `Processing command`; Ctrl+C then showed an old reusable lifecycle child inside its unconditional `connection.recv()`. That traceback proves the old child survived and was waiting, but does not conclusively identify the foreground parent's blocking location. Phase 120 therefore removes both suspect properties rather than inferring more from the trace: no reusable worker and no raw `Connection` receive. Active transport release still requests cleanup of any residual recognizer resource before standby microphone ownership resumes. The bounded lifecycle hardware mode accepts either high-confidence constrained authorization or a valid same-turn Whisper lifecycle fallback and cannot pass with a live worker. Post-fix Raspberry Pi spoken standby/shutdown acceptance remains required.
...
python scripts/manual_verify_standby_wake_hardware.py --verification-mode lifecycle --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --hardware-stage-timeout-seconds 45 --diagnostic-routing --diagnostic-wake
python scripts/run_ares_standby_voice.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0
```

For Phase 120, the lifecycle verifier is the immediate hardware gate. It prompts exactly `Ares -> goodbye Ares -> Ares -> shutdown Ares`; each stage has an outer deadline, each worker message is correlated, every child must be reaped, and microphone ownership must be released. Ctrl+C remains a separate `owner_cancellation` result and makes the verification fail. Deterministic non-Pi tests cannot establish these spoken transitions or the timing of the installed Raspberry Pi Vosk process.
