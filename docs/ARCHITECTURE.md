...
After endpoint correction, one narrowly bounded wake-only recovery handles a residual recognizer artifact: exactly two identical normalized configured alias tokens (`ares ares`, `aris aris`, or `aries aries`) may collapse to canonical `ares`. Both surface tokens must be the same alias, no unknown/prefix/other token may exist, the validated candidate must be nonzero and within the configured 4-second wake bound, and both token confidences must satisfy the existing policy. The decision uses the minimum confidence and diagnostics expose the original tokens, canonical tokens, minimum and mean values, and collapse status. Mixed aliases such as `ares aris`, `[unk] aris`, three or more tokens, prefix duplication, missing-confidence duplicates, over-duration audio, and unrelated repeated words reject. This exception does not alter active-command text.
...
`scripts/manual_diagnose_wake_word.py` captures exactly one candidate and exits. The hardware verifier first reports matching ARES processes and production lock ownership, refusing to compete with a live microphone owner. It then checks Vosk model, microphone adapter, ALSA open, and calibration health separately. The reliability phase prompts ten valid `Ares` attempts through one listener while the runtime remains in `STANDBY`; infrastructure failures are reported separately and do not masquerade as recognition misses or enter the recognition denominator. Before each attempt it clears only stale pre-prompt candidate history, prints `Ready for attempt N. Say 'Ares' now.`, keeps the existing stream live for a 0.6-second readiness interval, and starts the 5-second owner wait from that ready point. It leaves 0.5 seconds between completed attempts without reopening or recalibrating ALSA. Each result is rendered from the exact immutable attempt and includes IDs, stream generation, capture validity/duration, speech wait/start timing, terminal confirmation/resets, recognizer invocation, original/canonical tokens and confidence in diagnostic mode, classification, lifecycle outcome, and cleanup. This phase must normally finish with one stream open and one calibration; explicit device recovery is generation-labelled. A medium-confidence exact candidate prints `Low-confidence wake detected. Say Ares once more.`, preserves recognizer confirmation state, and consumes the immediate second candidate before scoring that attempt. The separate full lifecycle phase performs actual activation, acknowledgement, calculator, standby, reactivation, and shutdown, where closing and reopening around speaker/active-command ownership is intentional. `scripts/run_ares_standby_voice.py` intentionally remains in the foreground until an exact shutdown control or Ctrl+C. A missing Vosk package or model fails before microphone capture and reports the precise component status, expected model path, recommended Raspberry Pi model family, and installation command. Neither script installs a service or starts at boot. Raspberry Pi 9/10 reliability and the complete lifecycle remain owner-run hardware proof after pulling this checkpoint.
