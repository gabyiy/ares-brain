...
The Linux listener opens one raw canonical ALSA stream when `BrainRuntime` enters `STANDBY`; it does not reopen the device after each timeout or rejected candidate. Calibration requires 0.6 seconds of confirmed non-speech from that stream once per standby epoch and is bounded by 1.8 seconds. Detected speech clears the partial calibration window instead of contaminating the ambient estimate. The result remains valid until the configured 300-second interval expires, an owner requests recalibration, the clock rolls back, the device changes, or stream recovery starts a new epoch. A bounded rolling source retains at most 100 frames and replays only the configured 0.4-second pre-roll at the next VAD window, so speech beginning at a foreground poll boundary is not discarded. Candidate completion resets rolling history and drains only bounded PCM accumulated while Vosk was running, preventing stale tails from entering the next candidate. No queue or worker thread is created.
...
Before Vosk, a closed canonical 16 kHz mono signed 16-bit WAV is trimmed only beyond 0.24 seconds of leading and 0.20 seconds of trailing padding. The recognizer receives this validated file, not an open writer or unbounded raw capture. Diagnostics report noise floor and thresholds, bounded RMS samples, speech/quiet/pre/post-roll frames, duplicate-frame checks, stale-buffer discards, raw and trimmed duration, trim amounts, and endpoint reason. The existing ALSA normalization boundary prevents 44.1 kHz hardware data from being relabeled as 16 kHz.
