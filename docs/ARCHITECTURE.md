...
`OwnerProfileStore` uses `ares.owner_profile` schema v3. Its `data` contains `owner_id`, the existing keyed `facts`, structured `memories`, and optional bounded `pending_delete_all` state. General entries contain `memory_id`, `memory_type`, subject/predicate/object, canonical and owner-spoken text, topics, persistence, source, confidence, timestamps, and active/superseded state. The registered migration path is sequential: v1 -> v2 enriches keyed facts; v2 -> v3 preserves every fact and adds empty general-memory and pending-confirmation fields. A valid v3 profile is read directly. Unknown future versions and malformed/corrupt stores fail closed and are never interpreted as empty memory.
...
Broad deletion is two-step. A request such as `delete all long-term memory` stores a short-lived pending confirmation but does not delete anything. Only the exact second phrase `Yes, delete all my long-term owner memory` can complete it. A single ambiguous voice transcript cannot erase the profile.
...
`scripts/inspect_owner_memory.py` is read-only and can list keyed facts, all general memories, one topic, one memory type, or deterministic sanitized JSON. `scripts/manual_verify_general_long_term_memory.py` uses an isolated profile and fresh Python processes to prove explicit save, exact/topic/type recall, list, duplicate control, update, forget, restart persistence, non-persistence of ordinary speech, task collision safety, and absence of voice-owned memory.
