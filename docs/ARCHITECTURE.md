...
`scripts/manual_diagnose_wake_word.py` captures exactly one candidate and exits. The hardware verifier checks silence/unrelated speech, then prompts ten separate `Ares` attempts through one listener while the runtime remains in `STANDBY`. This classification-only reliability phase must normally finish with one stream open and one calibration; it fails if the stream identity or either count changes without an explicit device-recovery reason. A medium-confidence exact candidate prints `Low-confidence wake detected. Say Ares once more.`, preserves recognizer confirmation state, and consumes the immediate second candidate before scoring that attempt. The separate full lifecycle phase performs actual activation, acknowledgement, calculator, standby, reactivation, and shutdown, where closing and reopening around speaker/active-command ownership is intentional. Diagnostics include stream and ALSA handle IDs, open/close/calibration reasons, handoff endpoints, candidate confidence, transcript, and state/session outcomes. `scripts/run_ares_standby_voice.py` intentionally remains in the foreground until an exact shutdown control or Ctrl+C. A missing Vosk package or model fails before microphone capture and reports the expected path, recommended Raspberry Pi model family, and installation command. Neither script installs a service or starts at boot. Raspberry Pi 9/10 reliability and the complete lifecycle remain owner-run hardware proof after pulling this checkpoint.
