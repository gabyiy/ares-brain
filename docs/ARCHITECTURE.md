...
Successful calibration derives the existing ordered start/continuation/silence thresholds, leaves the stream open in `HEALTHY`, and remains valid until the configured 300-second interval expires, an owner requests recalibration, the clock rolls back, the device changes, or stream recovery starts a new epoch. Failure closes ALSA, clears candidate and recognizer state, releases capture ownership, and leaves the listener `FAILED`; health data distinguishes a failed open from an open that produced PCM and was deliberately closed during cleanup. Fresh calibration remains authoritative and this checkpoint does not use a cached noise profile. A bounded rolling source retains at most 100 frames and replays only the configured 0.4-second pre-roll at the next VAD window, so speech beginning at a foreground poll boundary is not discarded. Candidate completion resets rolling history and drains only bounded PCM accumulated while Vosk was running, preventing stale tails from entering the next candidate. No queue or worker thread is created.

Wake VAD waits up to three seconds for speech, uses 20 ms frames with two-frame speech-start evidence and three-frame resume hysteresis, accepts as little as 0.08 seconds of evidence, limits active wake speech to 1.6 seconds, confirms terminal silence after 0.55 seconds, and retains at most 0.12 seconds of post-roll. Wake-specific calibrated continue/silence floors are 160/120 RMS; the full-command VAD profile remains unchanged. Threshold validation enforces start greater than continue and continue greater than or equal to silence, with independent minimum and maximum clamps. The real endpoint defect was a gap between the continue and silence thresholds: a frame in that range was not resumed speech, but it reset terminal-silence progress to zero. Wake mode now counts every frame below the continuation threshold toward terminal silence while still requiring three consecutive continuation-level frames to resume. This preserves hysteresis without allowing low-level room noise to hold a short candidate open until its maximum. Frame-index assembly verifies that pre-roll, speech, and post-roll PCM are appended once.

Before Vosk, a closed canonical 16 kHz mono signed 16-bit WAV is trimmed only beyond 0.24 seconds of leading and 0.20 seconds of trailing padding. The recognizer receives this validated file, not an open writer or unbounded raw capture. Calibration diagnostics report frame count/duration, RMS minimum/median/p20/p80/maximum, speech and non-speech counts, longest non-speech run, bootstrap threshold, quiet-sample coverage, selected floor, clipping/zero counts, bounded rolling summaries, threshold output, and quality reason. Capture diagnostics then report speech/quiet/pre/post-roll frames, duplicate-frame checks, stale-buffer discards, raw and trimmed duration, trim amounts, and endpoint reason. Read-only ALSA diagnostics expose capture, microphone boost/input gain, and detectable AGC controls and may warn about extreme gain; they never mutate mixer state. The existing ALSA normalization boundary prevents 44.1 kHz hardware data from being relabeled as 16 kHz.

Whisper is not used in `STANDBY`. Real Raspberry Pi recordings showed that tiny Whisper could force a single spoken `Ares` into unrelated outputs such as `Alrighty`, `Okay`, or `Bye`; accepting those outputs as aliases would create dangerous false activations. `VoskWakeRecognizer` instead loads one local small English model at listener startup and constructs a constrained grammar from shared exact aliases `ares`, `aris`, and `aries`, the empty/`hey`/`hello`/`okay`/`wake up` prefixes, exact configured standby/shutdown controls, and `[unk]`. `Aries` is accepted only in the exact owner-name slot of one complete configured phrase. The model remains loaded across foreground candidates and is not reloaded after launcher health preflight. No model is downloaded at runtime. The capability manifest declares a 320 MB RAM estimate, normal recognition CPU weight, medium startup cost, serialized execution, and persistent model lifetime.
...
After endpoint correction, one narrowly bounded wake-only recovery handles a residual recognizer artifact: exactly two complete configured alias tokens may collapse to canonical `ares` only when both map to the same ARES identity, no unknown/prefix/other token exists, trimmed audio is nonzero and no longer than 1.4 seconds, and both token confidences satisfy the existing policy. The decision uses the minimum confidence and diagnostics expose both minimum and mean values. `[unk] aris`, three or more tokens, prefix duplication, missing-confidence duplicates, over-duration audio, and unrelated repeated words reject. This exception does not alter active-command text.
...
The persistent listener uses the explicit stream recovery states `CLOSED`, `OPENING`, `CALIBRATING`, `HEALTHY`, and `FAILED`. A stream is not healthy until calibration succeeds. Calibration or device recovery failure closes the exact ALSA handle, clears rolling pre-roll, candidate history, and recognizer attempt state, releases microphone ownership, and returns a structured failure. A successful reopen increments the generation; results from older generations cannot activate or appear in the new attempt diagnostics. Each Vosk candidate constructs a fresh constrained `KaldiRecognizer`, while the model remains loaded. No-speech, invalid-audio, calibration-failure, recovery, and exception paths explicitly reset ephemeral recognizer diagnostics.
...
`core.BrainRuntimeVoiceAdapters` reuses `SingleTurnVoicePipeline` as the active microphone/STT transport and as the TTS/speaker output boundary; it does not duplicate those subprocess paths. Acknowledgement and response speech use `run_local_output()`, an output-only synthesis/playback path that creates no fake input turn, emits no `voice_single_turn_started`, and never calls CoreService or skills. A shared `VoiceRuntimeGate` makes microphone ownership explicit. `standby_wake` owns capture only in `STANDBY`; the listener closes its stream before activation acknowledgement, active-command capture then owns the device, and a new standby epoch starts only after response playback plus the 0.35-second settling delay. The command adapter releases its capture ownership immediately after the recording stage closes and finalizes the WAV, before Whisper inference. The gate rejects any second simultaneous capture owner. Therefore ARES cannot transcribe its own output as a wake or command. Normal operation never plays captured owner audio. Production terminal status makes this handoff visible with `ARES is waiting for your command...`, capture-start, speech-detected, command-captured, transcribing, and processing messages. A bounded no-speech or transcription-failure turn reports that ARES remains active; the runtime stays active until the manager's inactivity deadline or an exact lifecycle command. With explicit terminal diagnostics enabled, the adapter reports the active command's raw/cleaned/alias-canonicalized transcript, lifecycle classification, CoreService bypass decision, before/after state and session, capture stop reason, raw and finalized durations, canonical WAV metadata, Whisper start/completion/timeout, parser and routing timing, cleanup result, and terminal-silence status. Wake-candidate and active-command diagnostics are separate; neither transcript enters events, memory, or persistent logs.

Active command inference is bounded at the speech-engine subprocess, not by a post-hoc elapsed-time check. `SingleTurnVoiceRequestV1.transcription_timeout_seconds` is carried in the finalized `AudioChunk` metadata to `LinuxWhisperSpeechToTextAdapter`; the foreground Raspberry Pi launcher sets it to 30 seconds independently of its broader 300-second process budget. `WhisperSubprocessRunner` uses `Popen(..., shell=False)` and `communicate(timeout=...)`, drains both pipes, terminates on timeout, escalates to kill only after a bounded grace period, and reaps the child. Ctrl+C uses the same child cleanup before propagating cancellation. Timeout, nonzero exit, malformed/empty output, missing or header-only audio, and noncanonical PCM return structured failures and cannot become standby or shutdown commands. Runtime command audio uses delete-always cleanup unless explicit diagnostic retention is enabled.
...
python scripts/manual_verify_standby_wake_hardware.py --diagnostic-wake --wake-reliability-attempts 10
python scripts/run_ares_standby_voice.py
```

`scripts/manual_diagnose_wake_word.py` captures exactly one candidate and exits. The hardware verifier first reports matching ARES processes and production lock ownership, refusing to compete with a live microphone owner. It then checks Vosk model, microphone adapter, ALSA open, and calibration health separately. The reliability phase prompts ten valid `Ares` attempts through one listener while the runtime remains in `STANDBY`; infrastructure failures are reported separately and do not masquerade as recognition misses or enter the recognition denominator. Each line is rendered from the exact immutable attempt and includes IDs, stream generation, capture validity/duration, recognizer invocation, transcript/confidence in diagnostic mode, classification, lifecycle outcome, and cleanup. This phase must normally finish with one stream open and one calibration; explicit device recovery is generation-labelled. A medium-confidence exact candidate prints `Low-confidence wake detected. Say Ares once more.`, preserves recognizer confirmation state, and consumes the immediate second candidate before scoring that attempt. The separate full lifecycle phase performs actual activation, acknowledgement, calculator, standby, reactivation, and shutdown, where closing and reopening around speaker/active-command ownership is intentional. `scripts/run_ares_standby_voice.py` intentionally remains in the foreground until an exact shutdown control or Ctrl+C. A missing Vosk package or model fails before microphone capture and reports the precise component status, expected model path, recommended Raspberry Pi model family, and installation command. Neither script installs a service or starts at boot. Raspberry Pi 9/10 reliability and the complete lifecycle remain owner-run hardware proof after pulling this checkpoint.

Real Raspberry Pi output exposed several independent defects. Whisper may transcribe the owner name as `Aris` or `Aries`; exact alias-slot matching canonicalizes `goodbye aris`, `goodbye aries`, `shutdown aris`, and `shut down aries` before strict lifecycle classification, while zodiac questions and arbitrary surrounding words do not canonicalize. Earlier one-shot standby capture was replaced by a persistent stream and one-time calibration. A later 6/10 run then reported seven opens and calibrations because the reliability verifier activated and returned to standby after each accepted prompt, intentionally triggering microphone ownership handoffs; rejected candidates were already reusing the stream. The corrected reliability phase measures recognition without activation and leaves one stream/calibration in place. Real Vosk results of `ares` at `0.574` and `0.616` also proved the prior threshold too strict, resulting in the bounded `0.55`/`0.40` policy above. A later successful activation crashed before acknowledgement because an old timestamp-only `event_history.json.lock` raised `MigrationError` inside a fake single-turn output path. Event telemetry is now fail-open, production uses one shared store, acknowledgement is output-only, and owner-aware lock inspection/recovery plus process exclusivity prevent recurrence without weakening critical-memory locks. The next hardware run reached `Transcribing command` and appeared to hang because the generic 300-second launcher timeout had also been injected into synchronous active Whisper, while the pipeline checked its stage timeout only after `transcribe()` returned. The dedicated 30-second subprocess boundary and terminal diagnostics now make that state bounded and observable. The hardware verifier labels wake and active-command transcript summaries separately. Command mode retains its independently validated 15-second utterance maximum and 0.9-second terminal-silence policy.
