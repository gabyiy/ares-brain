...
`core.LifecycleControl.normalize_lifecycle_command()` is the one authoritative deterministic classifier for production and tests. It receives the preserved raw Whisper transcript before generic `TranscriptNormalization`, `CoreService`, parser, planner, or skill routing and returns raw, cleaned, lifecycle-normalized, canonical-name, action, matched-phrase, and rejection fields. It lowercases, trims ordinary terminal punctuation, collapses whitespace, maps `good bye` to `goodbye`, maps `shut down` to the internal `shutdown` form, and canonicalizes only the complete final aliases `ares`, `aris`, and `aries`. Matching remains exact over the complete normalized phrase; there is no substring, fuzzy, edit-distance, or semantic path.

The configured standby templates are exact `goodbye ares`, `go to standby ares`, `go to sleep ares`, `standby ares`, and `sleep ares`; each accepts `ares`, `aris`, or `aries`, and `good bye` is a normalized surface form. They transition `ACTIVE -> RETURNING_TO_STANDBY -> STANDBY`, clear the active session through `BrainSessionManager`, bypass CoreService, and keep the foreground process alive. Full shutdown accepts only exact `shutdown {ares|aris|aries}` and the normalized `shut down` surface forms. `stop ares completely` is no longer a production shutdown command. Missing names, negated requests, `paris`, `harris`, zodiac questions, arbitrary surrounding sentences, and lifecycle words embedded inside longer input remain ordinary text. Cross-category collisions, duplicate normalized phrases, empty/oversized phrases, booleans used as numbers, non-finite values, and out-of-range timeout/failure settings fail closed. Each phrase group accepts 1-16 entries of at most 64 normalized characters; inactivity accepts 1-3600 seconds, failure limits 1-20, input polling 0.05-5 seconds, and synchronous command timeouts 0.1-600 seconds. Runtime inactivity/failure values must equal the composed manager configuration.
...
python scripts/manual_verify_standby_wake_hardware.py --verification-mode lifecycle --attempts-per-test 3 --diagnostic-routing --diagnostic-wake --wake-vad-sensitive
...
The bounded lifecycle-only mode prompts `Ares`, `goodbye Ares`, `Ares`, and `shutdown Ares`, with no more than three attempts for each requested phrase. It prints the actual active Whisper text and lifecycle classification, proves that goodbye clears the first session while leaving the process in `STANDBY`, requires a new session on reactivation, and accepts exactly one explicit shutdown. Deterministic tests prove normalization, whole-phrase rejection, CoreService bypass, state/session transitions, single-turn completion, active EOF behavior, and terminal-reason contracts without claiming Raspberry Pi speech recognition. The lifecycle-only command above remains the owner-run hardware proof that the installed Whisper model emits usable phrases and that the real foreground process follows the complete sequence. `scripts/run_ares_standby_voice.py` intentionally remains in the foreground until an exact shutdown control or Ctrl+C. A missing Vosk package or model fails before microphone capture and reports the precise component status, expected model path, recommended Raspberry Pi model family, and installation command. None of these scripts installs a service or starts at boot.
