...
The persistent raw boundary uses one canonical contract shared by capture and diagnostics: 16,000 Hz, one channel, signed 16-bit little-endian `S16_LE`, with each 20 ms frame containing 320 samples and 640 bytes. `SubprocessPcmFrameSource` copies every bytes-like low-level result before retaining it, accumulates short pipe reads losslessly until a complete 640-byte frame exists, and delivers only immutable complete frames. The rolling source makes another immutable boundary copy before live history, pre-roll replay, or VAD delivery. EOF, read errors, or incomplete upstream frames fail without zero filling. Candidate reset drains all bounded stale bytes already available rather than carrying a partial stale VAD frame forward. If that bounded read ends on the first byte of an `S16_LE` sample, the source records the phase and discards the matching continuation byte before assembling the next immutable 640-byte frame.
...
python scripts/manual_diagnose_persistent_pcm.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --record-seconds 4 --playback
...
The latest owner run proved persistent-stream ownership, one-time calibration, frame delivery, and Vosk recognition, but accepted only 6/10 wake prompts. The remaining failures were wake-only timing: the 1.6-second post-start maximum frequently won before terminal quiet, owner waiting was not represented as a distinct phase, and rapid prompt turnover made timing unnatural. The current capture separates a 5-second pre-speech wait from a 4-second post-start failsafe and requires 0.9 seconds of confirmed terminal quiet. This does not change command-mode capture, Vosk grammar, confidence tiers, or lifecycle ownership. Physical 9/10 reliability remains unverified until the owner reruns the bounded Raspberry Pi helper.
