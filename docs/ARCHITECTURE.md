...
The Linux listener opens one raw canonical ALSA stream when `BrainRuntime` enters `STANDBY`; it does not reopen the device after each timeout or rejected candidate. Calibration consumes 0.6 seconds from that stream once per standby epoch. The result remains valid until the configured 300-second interval expires, an owner requests recalibration, the clock rolls back, the device changes, or stream recovery starts a new epoch. A bounded rolling source retains at most 100 frames and replays only the configured 0.4-second pre-roll at the next VAD window, so speech beginning at a foreground poll boundary is not discarded. No queue or worker thread is created.

Wake VAD waits up to three seconds for speech, uses 20 ms frames with two-frame speech-start evidence and three-frame resume hysteresis, limits active wake speech to two seconds, confirms terminal silence after 0.65 seconds, and retains 0.12 seconds of end padding. Wake-specific calibrated continue/silence floors are 160/120 RMS; the full-command VAD profile remains unchanged. Threshold validation enforces start greater than continue and continue greater than or equal to silence, with independent minimum and maximum clamps. Low-level noise in `POSSIBLE_SILENCE` updates a bounded noise floor so it cannot repeatedly restore speech without three consecutive continue-level frames. The existing ALSA normalization boundary prevents 44.1 kHz hardware data from being relabeled as 16 kHz. Before recognition, the listener validates the canonical WAV header and enforces separate hard limits for assembled/normalized candidate duration and raw per-candidate speech-wait/utterance duration; one-time calibration is reported separately.

Whisper is not used in `STANDBY`. Real Raspberry Pi recordings showed that tiny Whisper could force a single spoken `Ares` into unrelated outputs such as `Alrighty`, `Okay`, or `Bye`; accepting those outputs as aliases would create dangerous false activations. `VoskWakeRecognizer` instead loads one local small English model at listener startup and constructs a constrained grammar from shared exact aliases `ares`, `aris`, and `aries`, the empty/`hey`/`hello`/`wake up` prefixes, exact configured standby/shutdown controls, and `[unk]`. `Aries` is accepted only in the exact owner-name slot of one complete configured phrase. The model remains loaded across foreground candidates and is not reloaded after launcher health preflight. No model is downloaded at runtime. The capability manifest declares a 320 MB RAM estimate, normal recognition CPU weight, medium startup cost, serialized execution, and persistent model lifetime.

Wake recognition fails closed. Vosk word output is enabled and the minimum confidence across all returned words controls a two-tier policy derived from owner-observed short-word results. An exact wake at or above `0.55` activates immediately. An exact activation from `0.40` through less than `0.55` remains rejected until the same complete phrase is recognized twice within eight seconds; controls never use this medium-confidence path. Confidence below `0.40`, `[unk]`, wrong phrases, or extra words reject. Missing confidence is accepted only when Vosk returns exactly one permitted activation phrase and the listener marked the candidate valid after VAD speech evidence, canonical WAV validation, and duration checks; missing-confidence controls and malformed candidates reject. This exception is not available to unrestricted Whisper transcription. The complete punctuation/case/whitespace-normalized result must equal one configured grammar phrase. There is no substring check, edit distance, learned alias, arbitrary fuzzy correction, or standby Whisper fallback. `okay`, `bye`, `alrighty`, `areas`, `air`, partial words, zodiac questions, and ordinary sentences containing Ares are non-wake speech.
