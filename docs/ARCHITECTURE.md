...
Wake recognition fails closed. Vosk word output is enabled and the minimum confidence across all returned words controls a two-tier policy. An exact wake at or above 0.8 activates immediately. An exact activation from 0.72 through less than 0.8 remains rejected until the same complete phrase is recognized twice within eight seconds; controls never use this medium-confidence path. Confidence below 0.72, missing/invalid confidence, `[unk]`, wrong phrases, or extra words reject. The complete punctuation/case/whitespace-normalized result must equal one configured grammar phrase. There is no substring check, edit distance, learned alias, arbitrary fuzzy correction, or standby Whisper fallback. `okay`, `bye`, `alrighty`, `areas`, `air`, partial words, zodiac questions, and ordinary sentences containing Ares are non-wake speech.
...
`scripts/manual_diagnose_wake_word.py` captures exactly one candidate and exits. The hardware verifier checks silence/unrelated speech, then prompts ten separate `Ares` attempts and requires at least nine accepted activations before running the complete calculator/standby/reactivation/shutdown lifecycle. It prints stream-open, calibration, candidate, pre-roll, confidence, transcript, and state/session diagnostics and fails nonzero without hiding misses. `scripts/run_ares_standby_voice.py` intentionally remains in the foreground until an exact shutdown control or Ctrl+C. A missing Vosk package or model fails before microphone capture and reports the expected path, recommended Raspberry Pi model family, and installation command. Neither script installs a service or starts at boot. Raspberry Pi 9/10 reliability and the complete lifecycle remain owner-run hardware proof after pulling this checkpoint.

Real Raspberry Pi output exposed two independent defects. First, Whisper may transcribe the owner name as `Aris` or `Aries`; exact alias-slot matching now canonicalizes `goodbye aris`, `goodbye aries`, `shutdown aris`, and `shut down aries` before strict lifecycle classification. Zodiac questions and arbitrary surrounding words do not canonicalize. Second, standby previously called one-shot capture for every foreground poll, which reopened ALSA, recalibrated ambient noise, reset pre-roll, and left timing-sensitive gaps around a one-word phrase. The persistent stream and one-time calibration remove that architecture. The hardware verifier labels wake and active-command transcript summaries separately. Command mode retains its independently validated 15-second utterance maximum and 0.9-second terminal-silence policy.
