...
In `STANDBY`, ordinary commands are ignored and no session ID exists. Exact normalized `ares`, `hey ares`, `hello ares`, or `wake up ares`, with `ares`, `aris`, or `aries` in the final name slot, activates one session and emits the configured `Yes Gabi.` acknowledgement only after the lifecycle transition succeeds. While active, ordinary commands use the existing `TranscriptNormalization -> SkillManager -> IntentParser -> Planner -> ExecutionPipeline -> Skill` route bound to the same CoreService, so calculator and central owner-memory behavior are unchanged. Repeating an activation phrase records activity without replacing the session ID.

`core.LifecycleControl.normalize_lifecycle_command()` is the one authoritative deterministic classifier for production, deterministic tests, and the Raspberry Pi verifier. It receives the preserved raw Whisper transcript before generic `TranscriptNormalization`, `CoreService`, parser, planner, or skill routing and returns raw, cleaned, normalized, canonicalized, canonical-name, matched-alias/type, negation, action, matched-phrase, and rejection fields. It lowercases, trims ordinary punctuation, collapses whitespace, maps `good bye` to `goodbye`, and maps `shut down` to the internal `shutdown` form. The shared general ARES identity remains `ares`, `aris`, and `aries`; lifecycle matching alone additionally recognizes exact acoustic name-slot forms `rs`, `r s`, and `are s`, all canonicalized to `ares`. These acoustic forms never enter wake grammar or global transcript cleanup. Matching remains exact over the complete normalized phrase; there is no substring, fuzzy, edit-distance, or semantic path.

The configured standby templates cover exact `goodbye ares`, `ares goodbye`, `go to standby ares`, `go to sleep ares`, `standby ares`, `ares standby`, and `sleep ares`; each expands only its complete name slot, and `good bye` is a normalized surface form. They transition `ACTIVE -> RETURNING_TO_STANDBY -> STANDBY`, clear the active session through `BrainSessionManager`, bypass CoreService, and keep the foreground process alive. Full shutdown accepts exact name-last `shutdown/shut down <name>` and name-first `<name> shutdown/shut down` controls, including `Ares shut down`. Lifecycle name slots accept `ares`, `aris`, `aries`, and the acoustic forms `rs`, `r s`, and `are s`; diagnostics identify canonical, pronunciation, or acoustic aliases. Negation (`do not`, `don't`, `dont`, `never`, `should not`, `shouldn't`) is detected before phrase matching. Therefore missing names, negated requests, `paris`, `harris`, zodiac questions, ordinary `rs` text, arbitrary surrounding sentences, and lifecycle words embedded inside longer input remain ordinary text. `stop ares completely` is not a production shutdown command. Cross-category collisions, duplicate normalized phrases, empty/oversized phrases, booleans used as numbers, non-finite values, and out-of-range timeout/failure settings fail closed. Each phrase group accepts 1-16 entries of at most 64 normalized characters; inactivity accepts 1-3600 seconds, failure limits 1-20, input polling 0.05-5 seconds, and synchronous command timeouts 0.1-600 seconds. Runtime inactivity/failure values must equal the composed manager configuration.
...
After Whisper succeeds, `SingleTurnVoicePipeline` exposes the untouched transcript at a dedicated pre-normalization hook. The active transport applies `normalize_lifecycle_command()` there, and `BrainRuntime` uses that same authoritative function before requesting any manager transition. Standby and shutdown controls are handled by `BrainSessionManager` without entering ordinary routing, while action `none` continues through generic transcript cleanup and CoreService. Completion of that bounded voice turn is transport-local, not completion of the persistent foreground runtime. Active-command EOF, empty or falsey transcription, no speech, timeout, lifecycle mismatch, and transcription failure remain nonterminal; they preserve `ACTIVE` until the manager's existing inactivity policy or a later explicit command. An active input adapter is never allowed to reinterpret those statuses as shutdown.
...
Production terminal status makes this handoff visible with `ARES is waiting for your command...`, capture-start, speech-detected, command-captured, transcribing, and processing messages. With `--diagnostic-routing` or the broader voice diagnostic mode, the active-command lifecycle section reports raw Whisper transcript; cleaned, normalized, and canonicalized transcript; matched alias and alias type; canonical name; negation status; lifecycle action and matched complete phrase; CoreService bypass; state before/after; session before/after; pipeline status; runtime-terminal flag/reason; capture stop reason; raw and finalized durations; canonical WAV metadata; Whisper timing; parser/routing timing; cleanup; and terminal-silence status. For `Shut down RS`, the normalized form is `shutdown rs`, the lifecycle-only canonicalized form is `shutdown ares`, alias type is `acoustic_alias`, and CoreService is bypassed. Wake-candidate and active-command diagnostics are separate; neither transcript enters events, memory, or persistent logs.

Active command inference is bounded at the speech-engine subprocess, not by a post-hoc elapsed-time check. `SingleTurnVoiceRequestV1.transcription_timeout_seconds` is carried in the finalized `AudioChunk` metadata to `LinuxWhisperSpeechToTextAdapter`; the foreground Raspberry Pi launcher sets it to 30 seconds independently of its broader 300-second process budget. `WhisperSubprocessRunner` uses `Popen(..., shell=False)` and `communicate(timeout=...)`, drains both pipes, terminates on timeout, escalates to kill only after a bounded grace period, and reaps the child. Ctrl+C uses the same child cleanup before propagating cancellation. Timeout, nonzero exit, malformed/empty output, missing or header-only audio, and noncanonical PCM return structured failures and cannot become standby or shutdown commands. Runtime command audio uses delete-always cleanup unless explicit diagnostic retention is enabled. Only the exact normalized shutdown command can produce the successful foreground terminal reason `explicit_shutdown_command`; cancellation and unrecoverable failure are separately named and never masquerade as owner shutdown.
...
The bounded verifier has separate `standby` and `shutdown` modes. Standby mode prompts `Ares`, `goodbye Ares`, `Ares`, and `Ares shut down`; shutdown mode prompts `Ares` and `Ares, shut down`, with no more than three attempts for each requested phrase. It prints the actual active Whisper text and the central lifecycle result, proves that goodbye clears the first session while leaving the process in `STANDBY`, requires a new session on reactivation, and accepts exactly one explicit shutdown. If Whisper emits `RS`, `R S`, or `Are S` in the exact name slot, the same central function canonicalizes it without changing ordinary routing. Deterministic tests prove normalization, whole-phrase rejection, negation precedence, CoreService bypass, state/session transitions, single-turn completion, active EOF behavior, and terminal-reason contracts without claiming Raspberry Pi speech recognition. The hardware commands remain owner-run proof that the installed Whisper model emits a supported lifecycle form and that the real foreground process follows the complete sequence. `scripts/run_ares_standby_voice.py` intentionally remains in the foreground until an exact shutdown control or Ctrl+C. A missing Vosk package or model fails before microphone capture and reports the precise component status, expected model path, recommended Raspberry Pi model family, and installation command. None of these scripts installs a service or starts at boot.

Real Raspberry Pi output exposed several independent defects. Whisper may transcribe the owner name as `Aris`, `Aries`, or the separate letters `RS`; exact lifecycle name-slot matching canonicalizes those forms before strict standby/shutdown classification, while wake grammar, zodiac questions, ordinary `rs` text, and arbitrary surrounding words do not canonicalize. The latest observed `Shut down RS` failure is addressed by that lifecycle-only acoustic form plus exact name-first controls such as `Ares shut down`. Earlier one-shot standby capture was replaced by a persistent stream and one-time calibration. A later 6/10 run then reported seven opens and calibrations because the reliability verifier activated and returned to standby after each accepted prompt, intentionally triggering microphone ownership handoffs; rejected candidates were already reusing the stream. The corrected reliability phase measures recognition without activation and leaves one stream/calibration in place. Real Vosk results of `ares` at `0.574` and `0.616` also proved the prior threshold too strict, resulting in the bounded `0.55`/`0.40` policy above. A later successful activation crashed before acknowledgement because an old timestamp-only `event_history.json.lock` raised `MigrationError` inside a fake single-turn output path. Event telemetry is now fail-open, production uses one shared store, acknowledgement is output-only, and owner-aware lock inspection/recovery plus process exclusivity prevent recurrence without weakening critical-memory locks. The next hardware run reached `Transcribing command` and appeared to hang because the generic 300-second launcher timeout had also been injected into synchronous active Whisper, while the pipeline checked its stage timeout only after `transcribe()` returned. The dedicated 30-second subprocess boundary and terminal diagnostics now make that state bounded and observable. The hardware verifier labels wake and active-command transcript summaries separately. Command mode retains its independently validated 15-second utterance maximum and 0.9-second terminal-silence policy.
