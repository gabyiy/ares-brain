...
- `ActiveLifecycleAudioRecognizer` consumes the already-finalized canonical WAV before Whisper and uses the local `VoskLifecycleGrammarBackend`. Production Vosk runs in one reusable spawned worker under the existing deadline and cleanup policy. The component returns structured evidence and has no microphone, transition, CoreService, skill, event, or memory authority. Phase 116 replaces its initial 21-action/84-competitor table with a lifecycle-slot-generated 42 standby + 36 shutdown + 49 competitor + `[unk]` grammar.
...
ARES is currently at the completed Architecture Hardening foundation plus a central deterministic Brain session state machine and persistent foreground runtime, explicit ALSA microphone/speaker adapters, one continuously pumped and calibrated standby PCM stream, constrained Vosk wake recognition, constrained Vosk ACTIVE lifecycle recognition, process-group-bounded offline Whisper ordinary-command STT, Piper TTS, adaptive end-of-speech capture, calculator routing, and CoreService-owned explicit owner memory. Owner hardware already proves production-composed ACTIVE preflight, nonzero complete PCM, candidate finalization, and ordinary Whisper transcription. Phase 118 separates controlled microphone shutdown evidence from STT configuration health; the lightweight STT preflight and complete two-phrase lifecycle decisions remain owner-run Raspberry Pi acceptance. Microphone gain, VAD thresholds, wake matching, and lifecycle confidence are not the next tuning target.
...
41. Run the two-phrase `scripts/manual_diagnose_active_lifecycle_audio.py` probe. Require each phrase to finish within its capture/Whisper/total bounds, one Ctrl+C to trigger visible group cleanup, medium/rejected constrained evidence to run same-turn Whisper fallback, and no previous child or audio owner to survive into the next phrase.
42. Run the bounded production calculator/standby/reactivation/shutdown sequence. Require goodbye to return to standby without process exit and shutdown to terminate once. Do not retune microphone, wake, VAD, or Vosk thresholds during this checkpoint.
