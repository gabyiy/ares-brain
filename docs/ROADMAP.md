...
The current pytest collection is 2203 tests, and all 2203 pass.
...
38. Before any further wake-reliability run, pull this checkpoint and run `python scripts/manual_diagnose_persistent_pcm.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --record-seconds 3 --playback`. Require four structurally valid WAVs, complete transport accounting, a persistent spoken signal above its quiet stage, reasonably comparable direct/persistent contrast, and clearly audible speech in both spoken WAVs. The code-level pull-only/backpressure defect is repaired; this owner run is the required physical proof that the repaired persistent path now carries realistic live amplitude.
39. Only after item 38 passes, run `python scripts/manual_verify_standby_wake_hardware.py --diagnostic-wake --wake-vad-sensitive --wake-reliability-attempts 10`. Require at least 9/10 valid quiet-room wakes, real speech frames and positive candidates, Vosk invocation only for those candidates, ordinary-silence rejection, normally one stream open/calibration, and the complete calculator/standby/reactivation/shutdown sequence before recording hardware proof.
40. Only after that stable hardware proof consider a separately reviewed systemd/boot-startup checkpoint. Wider background listening and autonomous City activation remain out of scope.
