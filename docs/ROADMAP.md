...
4. Run whisper.cpp installer and runtime verifier on the Raspberry Pi.
...
- No real voice/audio implementation beyond the safe VoiceService placeholder, microphone adapter abstraction, Linux ALSA one-shot microphone adapter, speech-to-text adapter abstraction, offline Whisper adapter, Raspberry Pi whisper.cpp setup/verification scripts, VoiceCommandRouter, VoicePipeline, ModuleLifecycleManager, null input/output components, and one-shot VoiceLoop text bridge.
