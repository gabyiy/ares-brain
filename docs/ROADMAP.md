...
The current pytest collection is 2172 tests, and all 2172 pass.
...
38. Before any further wake-reliability run, pull this checkpoint and run `python scripts/manual_diagnose_persistent_pcm.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --record-seconds 4 --playback`. Require structural integrity, a persistent spoken signal above its quiet stage, and clearly audible speech in both direct and persistent WAVs. Use the comparison to classify whether the physical fault is below, inside, or above the persistent path; do not infer amplitude from local tests.
