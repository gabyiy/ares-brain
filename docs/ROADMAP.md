...
- Added a separate validated 30-second active-command transcription default. Request timeout metadata now reaches the Whisper subprocess boundary, where stdout/stderr are drained with bounded `communicate()`. Timeout terminates, then kills only if needed, always reaps the child, returns a structured failure, cleans current-turn audio, and leaves the session available for a bounded retry. Ctrl+C also reaps active Whisper before cleanup.
...
40. Run the bounded `standby` and `shutdown` modes of `scripts/manual_verify_standby_wake_hardware.py` with three attempts, routing diagnostics, wake diagnostics, and the validated sensitive VAD profile. Require goodbye to resume standby without process exit, reactivation to create a new session ID, `Ares shut down` to terminate once, and any `RS` Whisper output to show lifecycle-only canonicalization. Do not retune microphone, wake, or Vosk thresholds during this checkpoint.
41. Only after that stable hardware proof consider a separately reviewed systemd/boot-startup checkpoint. Wider background listening and autonomous City activation remain out of scope.
