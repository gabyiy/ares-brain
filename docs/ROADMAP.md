...
6. Re-run reliable speech-input verification on the Raspberry Pi.
7. Implement a real TTS adapter.
8. Run a real single-turn voice loop.
9. Only later add wake-word/background listening.

What Must Not Be Started Yet

- No real voice/audio implementation beyond the safe VoiceService placeholder, microphone adapter abstraction, Linux ALSA one-shot microphone adapter, speech-to-text adapter abstraction, offline Whisper adapter, Raspberry Pi whisper.cpp setup/verification scripts, hardened manual speech-input diagnostics, ALSA monitoring helper, VoiceCommandRouter, VoicePipeline, ModuleLifecycleManager, null input/output components, and one-shot VoiceLoop text bridge.
...
- No Vosk, Piper, speaker/TTS, wake word, daemon/service installation, internet access, conversation loop, or background listening.
