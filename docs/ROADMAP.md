...
- `ActiveLifecycleAudioRecognizer` consumes the already-finalized canonical WAV before Whisper and uses the local `VoskLifecycleGrammarBackend` with 21 owner-validated standby/shutdown phrases, 84 bounded non-action acoustic/negative/descriptive competitors, and `[unk]`. The 84 rejection slots comprise 36 explicit phrases and a 48-phrase finite negation expansion; they are never aliases. Production Vosk runs in one reusable spawned worker. The request deadline covers child readiness, first-use model loading, recognizer/native work, PCM decoding, and result waiting; parent `Process.start()` and pipe `send()` are synchronous setup calls outside independent preemption. An expired child wait triggers terminate/kill/reap cleanup and fails closed if cleanup cannot be proven before any later worker restart. The component returns structured evidence and has no microphone, transition, CoreService, skill, event, or memory authority.
...
- Ordinary, low-confidence, unmatched, `[unk]`, missing-confidence, and backend-error results continue to the existing Whisper adapter on the same finalized WAV. The fallback transcript cannot bypass the audio authorization policy and turn a weak guess into standby or shutdown. Calculator, owner memory, all other skills, Whisper timeout handling, playback gating, and microphone ownership remain unchanged.
...
- All 2,662 collected tests pass with no skips or xfails. A focused 1,082-test lifecycle-audio/Vosk/Whisper/pipeline/runtime/session/calculator/memory/event/production-composition sweep, compileall, Phase 2 event/memory verification, configuration JSON validation, and `git diff --check` are green. This remains deterministic evidence, not Raspberry Pi lifecycle acceptance.
