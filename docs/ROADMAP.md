...
41. Run `scripts/manual_diagnose_active_transcription.py`, then the bounded production calculator/standby/reactivation/shutdown sequence. Require an audible canonical command WAV, timely transcript or confirmed group kill/reap, goodbye returning to standby without process exit, and `Ares shut down` terminating once. Do not retune microphone, wake, or Vosk thresholds during this checkpoint.
42. Only after that stable hardware proof consider a separately reviewed systemd/boot-startup checkpoint. Wider background listening and autonomous City activation remain out of scope.
