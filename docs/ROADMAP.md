...
All 2252 collected tests pass for Phase 109. Focused lifecycle, voice, and runtime tests; the complete pytest suite; compileall; Phase 2 event/memory verification; configuration JSON validation; and `git diff --check` are green. This deterministic evidence does not replace the bounded Raspberry Pi lifecycle run.
...
40. Run `python scripts/manual_verify_standby_wake_hardware.py --verification-mode lifecycle --attempts-per-test 3 --diagnostic-routing --diagnostic-wake --wake-vad-sensitive`. Require goodbye to resume standby without process exit, reactivation to create a new session ID, explicit shutdown to terminate once, and the actual Whisper transcript and classification to be visible. Do not retune microphone, wake, or Vosk thresholds during this checkpoint.
