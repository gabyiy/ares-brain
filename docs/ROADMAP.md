...
- Real production reached `Processing command` after successful capture and Whisper but did not complete spoken standby. Ctrl+C exposed `ares-active-lifecycle-vosk` blocked in its reusable worker loop's unconditional `connection.recv()`. The child had already returned recognition evidence, then waited forever for another request; standby resource release reset only policy state and did not reap it.
...
ARES is currently at the completed Architecture Hardening foundation plus a central deterministic Brain session state machine and persistent foreground runtime, explicit ALSA microphone/speaker adapters, one continuously pumped and calibrated standby PCM stream, constrained Vosk wake recognition, request-scoped constrained Vosk ACTIVE lifecycle recognition, process-group-bounded offline Whisper ordinary-command STT, Piper TTS, adaptive end-of-speech capture, calculator routing, and CoreService-owned explicit owner memory. Owner hardware already proves production-composed ACTIVE preflight, nonzero complete PCM, candidate finalization, and ordinary Whisper transcription. Phase 119 removes the idle reusable lifecycle child and requires a stop/reap handshake before runtime transition. The complete spoken standby/reactivation/shutdown sequence remains owner-run Raspberry Pi acceptance. Microphone gain, VAD thresholds, wake matching, and lifecycle confidence are not the next tuning target.
