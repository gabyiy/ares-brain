...
- Standby controls include exact configured phrases with one bounded owner-name policy. Shutdown controls are restricted to the complete `shutdown <name>` and `shut down <name>` forms; no additional shutdown alias is supported. `Aris` and `Aries` forms canonicalize through the shared policy rather than duplicating phrase lists.
...
Phase 98: Persistent Standby Audio and Wake Reliability Hardening

- Real Raspberry Pi attempts showed intermittent one-word capture because the standby listener reopened ALSA, recalibrated ambient noise, and recreated pre-roll on every foreground poll. A short `Ares` therefore depended on capture-window timing.
...
Phase 100: Event-History Lock Resilience and Runtime Exclusivity

- Real Raspberry Pi activation reached `ACTIVE` and then crashed before `Yes Gabi.` because a legacy timestamp-only `data/event_history.json.lock` caused `MigrationError` while acknowledgement output entered the normal single-turn event path.
- Event history is now explicitly non-critical telemetry. Expected append lock/persistence failures print one bounded warning per failure signature, increment an in-memory dropped-event counter, and do not interrupt activation, TTS, active commands, calculator or memory skills, standby, or shutdown. Unexpected programming errors still surface, and owner-memory locking remains strict.
- Production creates one authoritative `EventHistoryStore` and injects it through BrainRuntime, BrainSessionManager, CoreService/resource management, SkillManager, and SingleTurnVoicePipeline. Acknowledgement uses output-only TTS/playback and creates no fake input-turn event.
- Versioned file-lock metadata now records PID, hostname, owner token, owner kind, and creation time. Safe recovery is limited to expired local locks whose owners are proven dead; live, malformed, remote, and unprovable legacy locks are never stolen. `scripts/inspect_store_lock.py` exposes inspection and bounded recovery.
- The foreground standby launcher holds a separate process lock so a second ARES runtime exits with `ARES is already running`; normal shutdown and Ctrl+C release ownership. Hardware verification uses an isolated temporary runtime lock.
- Medium-confidence reliability attempts now visibly prompt for and consume one immediate confirmation candidate without resetting recognizer state. Confidence thresholds are unchanged.
- Deterministic verification passes 1967 tests. Event-history degradation is automated proof only; Raspberry Pi wake reliability and the complete live lifecycle remain owner-run requirements.

Phase 101: Bounded Active Whisper Transcription

- A real Raspberry Pi run completed Vosk wake, activation, acknowledgement, and active capture, then stopped visibly at `Transcribing command` for `goodbye Ares`. The active launcher had propagated its generic 300-second budget into synchronous Whisper; the pipeline's stage timeout was checked only after `transcribe()` returned and therefore could not interrupt inference.
- Added a separate active-command transcription timeout so request metadata reaches the Whisper subprocess boundary instead of relying on the post-return stage check. That first mitigation used `communicate(timeout=...)`, but later tracing proved its cleanup was incomplete: it launched no process group, signalled only the leader, and could return with `reaped=False` while descendants retained output pipes. Phase 111 replaces this historical implementation.
- Active capture now releases both ALSA and the shared microphone gate before inference. The finalized WAV must be closed, contain PCM payload, and be canonical 16 kHz mono signed 16-bit audio before Whisper starts. Missing, header-only, noncanonical, nonzero-exit, malformed, empty, and timeout cases fail before routing; none can falsely trigger standby or shutdown.
- Owner-terminal diagnostics now expose finalization, WAV bytes/format, Whisper start/completion/timeout and duration, parser status, routing timing/result, lifecycle before/after, and cleanup. Paths and transcripts remain opt-in terminal data and never enter events, owner memory, or persistent logs.
...
Phase 103: Generation-Bound Wake Attempts and Recovery Health

- Real hardware diagnostics exposed an impossible mixed record: Vosk text `ares` at confidence `1.000` was displayed beside a later zero-duration `invalid_audio` capture. The listener and verifier were reading independent mutable result/diagnostic snapshots, and failed recovery did not atomically replace both.
- One immutable `WakeAttemptResult` now binds unique attempt/candidate IDs, stream instance/generation, capture validity, recognition/classification, lifecycle outcome, infrastructure status, and cleanup. Invalid or empty audio never invokes Vosk or carries old text; old-generation recognition fails closed.
- Stream recovery is now explicit `CLOSED -> OPENING -> CALIBRATING -> HEALTHY` or `FAILED`. Calibration failure closes ALSA, clears all audio/recognizer state, releases ownership, and remains unhealthy until a later bounded recovery completes.
...
Phase 105: Wake-Only VAD Sensitivity and Stream Observability

- The latest Raspberry Pi run kept one persistent stream open and one calibration active, but no frame was promoted to speech. Ambient was approximately 389-430 RMS while the generic wake start gate reached about 658.5 RMS. Existing bounded RMS samples showed that post-calibration PCM was flowing; the remaining fault was the wake start policy, not ownership or Vosk.
- Added validated `conservative`, `normal`, and `sensitive` wake-only profiles. `normal` is the default; `--wake-vad-sensitive` is a bounded diagnostic override. All profiles keep ordered gates above ambient and require two consecutive start-level frames. Active-command VAD remains unchanged.
...
Phase 107: Persistent PCM Integrity and Direct Hardware Comparison

- The next Raspberry Pi checkpoint moves below VAD and Vosk. `scripts/manual_diagnose_persistent_pcm.py` compares one direct `arecord` WAV with the exact persistent standby raw stream on the same resolved device, using one quiet second followed by loud speech. It saves both canonical WAVs plus a JSON report, prints signed-sample/RMS/peak/repetition statistics, and plays the files only under explicit `--playback`.
- The shared raw contract is 16 kHz, mono, signed little-endian `S16_LE`: every 20 ms frame is exactly 320 samples and 640 bytes. Each low-level and rolling boundary takes an immutable copy. Partial pipe reads accumulate losslessly until a complete frame exists and EOF/errors fail without zero fill. Bounded stale pre-prompt draining removes every available stale byte; a half-sample endpoint causes its continuation byte to be discarded before fresh aligned frame assembly.
- Persistent diagnostics expose `total_low_level_reads`, `valid_full_pcm_frames`, `partial_reads`, `empty_reads`, `read_errors`, `discarded_bytes`, `zero_filled_bytes`, `repeated_frame_hashes`, `mutable_buffer_reuse_detected`, and `valid_microphone_bytes_delivered_to_vad`. Requested raw format is explicitly distinguished from negotiated or header-observed format. A no-speech candidate reports `beginning_clipped=false` and `beginning_clipped_status=not_applicable`; detected speech alone can report complete pre-roll as `no` or incomplete pre-roll as `yes`.
- All 2172 collected tests pass. Deterministic tests prove framing, alignment, ownership copies, counter propagation, and diagnostic classification, not physical microphone amplitude. The physical root cause remains open until direct-versus-persistent Raspberry Pi results show whether speech is audible and changes sample amplitude in both paths. The ten-attempt wake reliability run is deferred until the PCM probe passes and both saved WAVs are audible.

Phase 108: Continuous Persistent PCM Transport

- Owner hardware evidence established a healthy native `hw:2,0` 44.1 kHz direct recording while the persistent path intermittently delivered exact-size RMS 3-7 frames. The production trace classified the code-level difference: direct `arecord` continuously wrote its WAV, but the old persistent process drained stdout only when calibration or VAD synchronously requested a frame. Prompt, recognition, and consumer gaps therefore left stale quiet PCM at the head of the pipe and could block `arecord`; its separately piped stderr was also undrained.
- The same single ALSA process and owner now use `ContinuousPcmFrameSource`. One adapter-local producer continuously reconstructs immutable 640-byte frames into a bounded 50-frame latest-audio queue, drops and counts oldest backlog, drains stderr separately, and exposes command/PID/liveness/EOF/read-size/partial/queue/tiny-signal/duplicate-signal diagnostics. Odd or incomplete terminal PCM, dead processes, alive-process stdout closure, zero fill, and pathological identical non-silent runs fail closed. Calibration and wake polling remain consumers of the same stream; reset epochs discard only pre-boundary queued or in-flight frames.
- The integrity probe now compares separate three-second quiet and spoken native `hw:2,0` 44.1 kHz WAVs with separate three-second quiet and spoken WAVs from the exact production `plughw:2,0` 16 kHz stream. It retains all four WAVs and one JSON report, verifies complete byte/process accounting and direct-versus-persistent contrast, plays only after ALSA closes, and cannot pass without explicit owner confirmation that both spoken recordings are clear and audible.
- All 2203 tests collected for that checkpoint passed. Deterministic tests proved the transport, ownership, framing, calibration boundary, metrics, failure gates, and unchanged terminal-silence behavior; at that point they did not prove Raspberry Pi signal amplitude. Phase 109 records the later owner proof through the production wake-to-Whisper route.

Phase 109: Deterministic Active Lifecycle Control

- Owner Raspberry Pi evidence now proves the repaired persistent path reaches calibration, constrained Vosk wake, acknowledgement playback, active capture, speech endpointing, finalized command audio, and Whisper transcription. Persistent PCM is no longer the open checkpoint, and this phase does not change microphone ownership, VAD thresholds, wake confidence, Vosk, or the standby transport.
- One authoritative `normalize_lifecycle_command()` boundary now receives the raw Whisper transcript before calculator normalization, `IntentParser`, `CoreService`, planning, or skill routing. It preserves raw and cleaned forms, performs only bounded whitespace/punctuation cleanup, canonicalizes `good bye` to `goodbye`, canonicalizes `shut down` and `shutdown` to one internal form, and maps only the complete `ares`, `aris`, or `aries` name token to canonical `ares`.
- Exact standby controls are `goodbye <name>`, `standby <name>`, `go to standby <name>`, `sleep <name>`, and `go to sleep <name>`; the separated surface form `good bye <name>` normalizes to the same exact command. Exact shutdown controls are only `shutdown <name>` and `shut down <name>`. Lifecycle words inside longer sentences, missing owner names, unrelated suffixes, negative commands, substrings, fuzzy spellings, and broad aliases remain ordinary input.
- Standby and shutdown classifications bypass `CoreService` and all ordinary skills. Standby clears the active session through `BrainSessionManager`, transitions `ACTIVE -> RETURNING_TO_STANDBY -> STANDBY`, and keeps the foreground process alive. Shutdown transitions `ACTIVE -> SHUTTING_DOWN -> STOPPED` and terminates exactly once.
- Empty transcripts, no-speech results, Whisper failures, timeouts, phrase mismatches, a completed single voice turn, and source-local input EOF are nonterminal to the persistent runtime. They cannot become shutdown. Every actual process exit records and prints an explicit terminal reason such as `explicit_shutdown_command`, `owner_cancellation`, or `unrecoverable_failure`.
- Active-command diagnostics now show raw, cleaned, lifecycle-normalized, and canonical-name fields; classification and selected action; routing bypass; state/session before and after; pipeline status; and runtime-terminal status/reason. A bounded Raspberry Pi lifecycle mode runs activation, goodbye, reactivation with a new session, and explicit shutdown with at most three attempts per requested phrase while printing the actual Whisper transcript and lifecycle classification.
- Deterministic tests prove exact parsing, false-positive rejection, routing bypass, session replacement, single-turn/source-EOF nontermination, explicit shutdown, and the complete standby-to-shutdown state sequence. They do not prove Raspberry Pi Whisper output or physical lifecycle operation; that remains the next bounded owner verification.

Phase 110: Lifecycle-Only Acoustic Name Canonicalization

- Real Raspberry Pi diagnostics isolated the remaining shutdown miss to active Whisper output `Shut down RS`: command words and audio were correct, but exact lifecycle parsing did not recognize `rs` as the assistant name. Wake recognition, microphone ownership, VAD, Whisper transport, calculator, and owner memory were left unchanged.
- The single authoritative `normalize_lifecycle_command()` now supports exact lifecycle name slots for `ares`, `aris`, `aries`, and the active-Whisper acoustic forms `rs`, `r s`, and `are s`. Acoustic forms exist only inside complete standby or shutdown phrases; they are not added to Vosk wake grammar, global identity aliases, generic transcript normalization, or ordinary skill routing.
- Shutdown accepts name-last `shutdown/shut down <name>` and name-first `<name> shutdown/shut down`, including `Ares shut down`. Standby accepts `goodbye <name>`, `<name> goodbye`, `standby <name>`, `<name> standby`, and `go to standby <name>`, while preserving the existing sleep forms. All matches remain complete, token-based, deterministic, and CoreService-bypassing.
- Exact negation is evaluated before lifecycle matching. `do not`, `don't`, `dont`, `never`, `should not`, and `shouldn't` forms cannot terminate or change the session. Longer sentences, zodiac questions, `Paris`, `Harris`, missing names, bare `RS`, and ordinary text containing `rs` remain ordinary input.
- Runtime diagnostics now report raw, cleaned, normalized, and canonicalized transcripts; matched assistant alias and alias type; canonical name; negation; matched phrase/action; CoreService bypass; lifecycle states; and terminal reason. The hardware verifier adds bounded direct-shutdown and standby/reactivation modes that call the same central classifier and show the real Whisper transcript.
- All 2325 collected tests pass. Focused lifecycle, voice, and runtime suites; the complete pytest suite; compileall; Phase 2 event/memory verification; configuration validation; and `git diff --check` are green. This deterministic result still requires the owner-run Raspberry Pi standby and shutdown sequences.

Phase 111: Hard-Bounded Active Whisper Process Groups

- The latest Raspberry Pi run again completed wake, acknowledgement, active capture, speech endpointing, and WAV finalization, then stopped after `Transcribing command`. That terminal position could not distinguish a blocked Whisper wait, child cleanup, or event-history append, so this phase does not claim the log alone proved one cause.
- Code tracing found two concrete defects. Whisper ran without a new Linux session, cleanup signalled only the leader, and its last fallback could return with `reaped=False`; descendants inheriting stdout/stderr could therefore survive the nominal timeout. `EventHistoryStore` also entered an unbounded in-process `RLock` before its fail-open file-lock policy.
- The active Whisper defaults are now 15 seconds for inference, 1 second for group `SIGTERM` grace, and 3 seconds for the complete cleanup deadline. stdout/stderr use bounded temporary files, Linux starts a new session, timeout escalates from process-group `SIGTERM` to `SIGKILL`, and result metadata records PID/PGID, signals, reap, handle closure, cleanup reason, and elapsed time. Ctrl+C and pipeline shutdown share the same cancellation helper.
- `transcription_timeout` remains a distinct retryable failure: command routing and lifecycle classification are skipped, microphone ownership is released, current-turn audio is deleted unless explicitly retained, and runtime state remains `ACTIVE`. The owner receives a bounded retry message rather than an unknown-command, standby, or shutdown action.
- Noncritical event appends now wait at most 0.05 seconds for their in-process lock, warn once and increment drop accounting on contention, and continue runtime operation. Safe stale file-lock recovery is unchanged and never removes a valid lock owned by a live process.
- `scripts/manual_diagnose_active_transcription.py` captures one command, validates its real canonical WAV header, invokes the exact production Whisper helper, and prints command/PID/PGID/timing/transcript/exit/cleanup without routing or changing persistent state. Deterministic tests prove timeout escalation, output-buffer safety, retry state, cleanup, and lock bounds. The Raspberry Pi owner must still prove the captured WAV is audible and the installed Whisper process completes or is visibly killed and reaped.
- All 2358 collected tests pass with no skips or xfails. Focused process, active-command, lifecycle, runtime, and lock tests; compileall; Phase 2 event/memory verification; nonempty configuration JSON parsing; and `git diff --check` are green. This deterministic result does not replace the owner-run Raspberry Pi probe.

Phase 112: State-Specific Active Command and Lifecycle Routing

- Real Raspberry Pi evidence proved wake, capture, and bounded Whisper healthy, then exposed a routing-state collision: active `Ares` still entered the activation classifier, refreshed activity, and played the already-active acknowledgement. Wake recognition is now an activation authority only in `STANDBY`; active name-only and bounded greeting forms return typed `attention_only`, preserve the session/deadline, bypass CoreService, and are silent by default.
- Added one active-state normalization boundary. It removes at most one supported whole-token assistant address from a command prefix or suffix, canonicalizes `good bye`, `stand by`, and `shut down`, and exact-matches only bounded roots. Standby roots are `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; shutdown roots are `shutdown`, `turn off`, and `power down`. Bare and addressed roots work without fuzzy or substring matching.
- Negation is evaluated before lifecycle execution. Negated, explanatory, scheduled, and otherwise longer sentences remain ordinary and cannot change lifecycle. Ordinary usable owner commands alone reset the 30-second inactivity deadline; attention-only, playback, empty/no-usable speech, and diagnostics do not.
- Playback/capture exclusion remains shared across acknowledgement, response, standby, and shutdown speech. The active wait prompt now appears only after post-playback settling and successful capture-gate acquisition, once per genuine waiting phase.
- Deterministic verification covers the state boundary, variants, false-positive rejection, routing bypass, calculator/memory regressions, session identity, inactivity, playback/prompt ordering, Whisper timeout, event locking, and cleanup. Physical Raspberry Pi acceptance remains owner-run: repeat-name silence, calculator, goodbye/standby, reactivation, explicit shutdown, `goodbye Aris`, and negated-shutdown retention.

Phase 113: Frame-Safe ACTIVE Capture and Activation Authority

- The follow-up Raspberry Pi run still replayed `Yes Gabi.` after active commands, but it did not print or retain the raw active transcript or state at routing. Current source has no ACTIVE path that emits that acknowledgement: the literal output is owned by the `STANDBY` activation handler. The historical raw transcript therefore cannot be recovered honestly; a stale deployed process or an unexpected `STANDBY` state remains distinguishable only in the next instrumented hardware run.
- Production tracing found an independent, concrete audio defect. `ARES is waiting for your command...` was printed before `SingleTurnVoicePipeline.run_once()`, while the one-shot active recorder then consumed and excluded 0.75 seconds of calibration PCM. Speech begun at the advertised ready point could lose a leading `goodbye`, `shutdown`, `calculate`, or `remember` and leave only its trailing words for Whisper.
- Added a synchronous frame-safe readiness callback after ALSA open and calibration transition to `WAITING`. The production ACTIVE wrapper keeps thresholds, device, canonical 16 kHz mono `S16_LE`, start evidence, and maximum duration unchanged; it increases only active pre-roll from 0.25 to at least 0.5 seconds and keeps 0.9 seconds of continuous terminal quiet. Deterministic PCM tests prove calibration exclusion, sub-threshold consonant/onset retention, first-speech-frame retention, frame-boundary onset preservation, internal-pause continuation, and terminal-silence reset/completion.
- `BrainSessionManager.activate_session()` now rejects every non-`STANDBY` request under the manager lock without changing state, session, history, or acknowledgement count. `BrainRuntime` separately rejects any forced activation category outside `STANDBY`; ordinary active name-only input remains typed, silent `attention_only`. Exact shutdown roots also include bare or outer-edge-addressed `stop runtime` without weakening negation or whole-phrase safety.
- Added an exact launcher composition test through `create_runtime()`, the production input/output adapters, raw Whisper hook, central normalizer, runtime, session authority, and shared gate. A diagnostic-only composition revision makes an old checkout/alternate process visible. Active diagnostics now include capture readiness, pre-roll, first/last speech, clipping/trim, raw transcript, alias removal, lifecycle decision, pre-route state/session, and activation-handler use.
- Added `scripts/manual_diagnose_active_lifecycle_audio.py`. It asks for four fixed phrases, uses the production recorder and bounded Whisper helper, prints what lifecycle action would be selected, and never executes runtime lifecycle, CoreService, skills, events, or owner memory. Deterministic verification cannot establish Raspberry Pi audibility or final acceptance; the owner must run this probe before the production sequence.
- All 2487 tests pass with no skips or xfails. Focused capture, lifecycle, state-authority, exact-composition, calculator, owner-memory, event, and Whisper checks plus compileall, Phase 2 event/memory verification, configuration validation, and `git diff --check` are green.

Phase 114: Production-Composed ACTIVE PCM Diagnostic

- The first Raspberry Pi execution of the new four-phrase probe failed before useful audio on every phrase: no pre-roll, zero candidate duration, no transcript, and `pcm_stream_error:RuntimeError`. This establishes a capture-stage failure only. It provides no evidence about aliases, Whisper, goodbye, standby, or shutdown classification.
- Code audit found that the diagnostic manually instantiated `LinuxAlsaMicrophoneAdapter`, `WhisperSubprocessRunner`, and `LinuxWhisperSpeechToTextAdapter` instead of receiving the foreground launcher's production active-audio composition. Existing diagnostic tests injected a fake microphone whose `record_until_silence()` directly wrote a complete canonical WAV, so open-before-read, the first PCM frame, VAD, ALSA stderr, process lifetime, per-turn cleanup, and ownership were not exercised.
- The VAD calibration and capture error handlers separately replaced the low-level exception with `pcm_stream_error:<exception class>`. The supplied old output therefore cannot identify the exact RuntimeError message, ALSA stderr, failing child status, or competing microphone owner. Phase 114 explicitly records this evidence gap and does not guess among those cases.
- One authoritative production active-audio factory is shared by the foreground launcher and diagnostic. The diagnostic follows the existing bounded per-turn model: reject a live runtime/gate conflict, reuse the production pipeline and adapter, create and initialize one fresh stream for each phrase, read only after open, close after that phrase's final read, release ownership on all paths, and never reuse a closed stream. One failed phrase cannot contaminate later fresh captures.
- Added a bounded structural preflight for the requested/resolved device, canonical PCM format, ownership, open/start/read state, exact 640-byte first frame, nonzero status, stream health, exact argv, process IDs/status, ALSA stderr, and cleanup. Silent but complete PCM is valid; the phrase prompt follows readiness so preflight does not consume an advertised owner command.
- Typed failures now distinguish microphone open, PCM read, invalid frame, no speech, VAD, WAV write, Whisper, empty transcript, and lifecycle parse stages. Explicit diagnostic output preserves exception class and complete message, adapter/method, device/format, lifecycle state, PID/child PID, stderr, cleanup, and full traceback. Normal production logs remain bounded and traceback-free.
- Deterministic acceptance proves factory use, CLI forwarding, open/read/close ordering, error preservation, stderr, phrase isolation, ownership cleanup, and production-composition stability. Physical acceptance remains pending until the Pi reports one complete PCM frame, owner speech yields nonzero PCM and a candidate longer than zero, and the retained WAV is audible. Lifecycle conclusions remain deferred until that audio gate passes.
- All 2514 collected tests pass with no skips or xfails. Focused diagnostic/PCM/VAD checks, a broader 1550-test audio/runtime/lifecycle/memory/event sweep, compileall, Phase 2 event/memory verification, parsing of all five nonempty configuration JSON files, and `git diff --check` are green. Four intentionally empty configuration placeholders are not JSON documents and remain excluded. This is deterministic evidence only, not a Raspberry Pi success claim.

Phase 115: Constrained ACTIVE Lifecycle Audio Recognition

- The owner-run repaired diagnostic passed the previously blocked hardware gate. `plughw:2,0` opened as 16 kHz mono `S16_LE`, returned one nonzero 640-byte frame, and created all four nonempty candidates. Whisper correctly heard `Calculate 2 plus 2.` and `Remember that I like video games!`, but spoken `goodbye Ares` became `*clears throat*` and spoken `shutdown Ares` became `Shut down, artist.`. The remaining defect is therefore short closed-set acoustic recognition, not PCM transport or candidate capture.
- `ActiveLifecycleAudioRecognizer` consumes the already-finalized canonical WAV before Whisper and uses the local `VoskLifecycleGrammarBackend`. At this historical checkpoint production Vosk used one reusable spawned worker; Phase 119 supersedes that unsafe post-result lifetime with one request-scoped child and mandatory reap. The component returns structured evidence and has no microphone, transition, CoreService, skill, event, or memory authority. Phase 116 replaces its initial 21-action/84-competitor table with a lifecycle-slot-generated 42 standby + 36 shutdown + 49 competitor + `[unk]` grammar.
- High-confidence exact evidence authorizes a typed lifecycle request without Whisper. Each constrained child-work deadline is 5 seconds, followed only on timeout by bounded cleanup grace. Standby uses `0.70` high/`0.50` medium; shutdown uses `0.78` high/`0.60` medium. Constructors enforce safety floors of standby `0.60`/`0.40`, shutdown `0.70`/`0.50`, and confirmation `0.75`. Medium evidence cannot execute and requires one session-bound next-turn confirmation within 10 seconds. Affirmative confirmation requires `0.80`; cancellation requires `0.50`; cancellation, mismatch, expiry, session replacement, or lifecycle-state change clears the request safely.
- Ordinary, low-confidence, unmatched, `[unk]`, missing-confidence, and backend-error results continue to the existing Whisper adapter on the same finalized WAV. Phase 116 makes the resulting exact ACTIVE Whisper lifecycle classification authoritative before ordinary routing; failures, empty text, negation, descriptive text, and unmatched phrases remain nonterminal and non-lifecycle. Calculator, owner memory, all other skills, Whisper timeout handling, playback gating, and microphone ownership remain unchanged.
- The four-phrase diagnostic uses the same production recognizer in compare-only mode. It reports constrained text/tokens/confidence/backend/classification/canonical/rejection and whether production would fall back, while still running Whisper on the same closed WAV for comparison. Comparison Whisper status and reason are always printed. It never recaptures or executes a lifecycle action. A comparison Whisper failure is nonfatal only for independently valid high-confidence lifecycle evidence that production would bypass; calculator and memory still require successful Whisper fallback. It returns nonzero unless both lifecycle phrases are high-confidence exact canonical actions without fallback and calculator/memory are ordinary with fallback; only that exact result prints `all four phrases passed the constrained lifecycle policy`. Deterministic verification must remain distinguished from the owner-run Raspberry Pi grammar and complete production-sequence acceptance.
- The Phase 115 checkpoint passed 2,662 collected tests and its focused 1,082-test sweep. Phase 116's current totals supersede this historical checkpoint; neither result is Raspberry Pi lifecycle acceptance.

Current State

Phase 115 closes the ACTIVE PCM/candidate checkpoint and adds a conservative, audio-first lifecycle classifier without weakening ordinary Whisper. The immediate physical checkpoint is the repaired four-phrase probe: the same real owner WAV must produce constrained `standby` for `goodbye Ares`, constrained `shutdown` for `shutdown Ares`, and ordinary/Whisper fallback for calculator and owner-memory speech. Only after that passes should the full wake, calculator, standby, reactivation, memory, recall, and shutdown foreground sequence be treated as accepted. No Windows result is a Raspberry Pi success claim.

Phase 116: Narrow ACTIVE Lifecycle `rs` Slot Canonicalization

- Real Raspberry Pi output proved a policy mismatch rather than another capture defect. Whisper returned `Goodbye, Aris.` and `Shut down Aris.` and the existing ACTIVE parser selected standby/shutdown, while constrained Vosk returned `goodbye rs` and `shutdown rs`. Exact constrained policy rejected the first as unmatched and the second as a bounded distractor.
- One authoritative constrained lifecycle assistant slot accepts exactly `ares`, `aris`, `aries`, `arris`, `rs`, or `r s` and canonicalizes it to `ares`. The slot exists only at the edge of the seven established standby and six established shutdown templates. It never rewrites name-only speech, substrings, ordinary sentences, missing-name commands, extra-word commands, or the 49 bounded competitors.
- Grammar generation and post-recognition canonicalization share those templates: 42 standby expansions + 36 shutdown expansions + 49 competitors + `[unk]` = 128 loaded alternatives. Diagnostics expose that grammar only in the explicit hardware probe and show raw text/tokens separately from alias/canonicalized values.
- Production decision order is accepted constrained lifecycle evidence, otherwise exact valid Whisper lifecycle parsing, otherwise ordinary CoreService routing. Constrained rejection no longer vetoes valid `Goodbye, Aris.` or `Shut down Aris.`. A failed, empty, negated, descriptive, or unmatched Whisper result cannot request standby or shutdown.
- Deterministic verification cannot prove the owner microphone now produces the expected post-fix constrained result. The four-phrase diagnostic and complete foreground sequence remain Raspberry Pi acceptance gates.
- The production composition revision is `constrained_active_lifecycle_alias_slot_v2`. The definitive post-final-code focused lifecycle/production/diagnostic sweep passes 539 tests and the complete suite passes all 2,688 tests; compileall, configuration validation, and `git diff --check` are green. This remains deterministic evidence only.

Phase 117: Interruptible Foreground Voice Process Lifecycle

- Real Raspberry Pi output reached the second ACTIVE lifecycle phrase, started Whisper, printed the configured 15-second timeout, and then blocked indefinitely. Code tracing found that timeout progress depended on one blocking child wait and that `KeyboardInterrupt` was converted into typed pipeline results before the foreground cleanup owner could run.
- Foreground child work now uses a parent-owned monotonic polling deadline. Linux `arecord`, `whisper-cli`, Piper, and `aplay` start in dedicated process groups; cancellation or timeout sends group `SIGTERM`, waits a bounded grace period, escalates to group `SIGKILL` only if needed, reaps the leader, closes standard streams, releases audio ownership, and finalizes temporary files.
- One `ForegroundSignalCoordinator` owns SIGINT/SIGTERM handling for the production launcher and the ACTIVE diagnostic. Cancellation is idempotent and propagates through pipeline/adapters to one top-level cleanup path; a first Ctrl+C therefore starts bounded cleanup instead of being reported as an ordinary voice failure.
- Medium or rejected constrained lifecycle evidence now requires same-turn Whisper fallback on the already-closed WAV. Only high-confidence accepted constrained evidence bypasses fallback. The invalid state `classification=uncertain`, `action=none`, `whisper_fallback=no` is no longer representable.
- Production still injects one authoritative `EventHistoryStore`. The two-phrase diagnostic injects a nonpersistent event sink and no longer competes for `data/event_history.json`. Event-history dead-lock recovery remains limited to expired local locks whose PID is proven dead, while live or malformed locks are preserved. `scripts/cleanup_ares_voice_processes.py` provides bounded, ARES-specific process cleanup and optional safe stale-lock recovery.
- The post-final-code focused subprocess/audio/runtime/event sweep passes 800 tests and the complete suite passes all 2,699 tests. This is deterministic Windows evidence only; interruptibility, process-group cleanup, and lifecycle decisions still require the owner-run Raspberry Pi diagnostic.

Phase 118: Independent STT Health and Controlled PCM Stop Semantics

- The next production-composed health check failed before inference with `AttributeError: 'WhisperSubprocessRunner' object has no attribute '_runner'`. `WhisperSubprocessRunner` overrides construction but omitted `SafeSubprocessRunner.__init__()`, even though inherited executable resolution depends on the parent's `_runner`. Superclass initialization restores the production resolver; this is separate from the already-bounded Whisper execution path.
- The nearby `arecord` exit-one/`pcm_read ... Interrupted system call` evidence came after valid PCM and an explicit stream stop. It is now classified as controlled cleanup only when the child was live at stop, valid PCM existed, no transport failure preceded the stop, stderr contains only that exact line, and process/pipes are completely reaped/closed. Cleanup-time SIGINT/SIGTERM may also be controlled; a reaped SIGKILL is accepted only after recorded bounded escalation and remains degraded-but-reusable. Other negative signals, mixed stderr, zero PCM, active failure, unexpected death/ownership loss, unrelated stderr, or incomplete cleanup remains unhealthy.
- Microphone and STT health print independently. STT validates executable resolution/permission, model readability, temporary-output writability, command construction, child/reap/handle state, and cancellation state without microphone access or inference. A lightweight `--stt-preflight-only` mode exercises that contract before the two owner-audio phrases.
- Diagnostic failures are stage-specific rather than a shared component error, and UTC progress markers distinguish preflight, valid PCM, controlled stop, readiness/capture, actual STT completion, microphone release, and file finalization. The two-phrase probe remains execution-disabled and deletes audio unless retention is explicit.
- Deterministic tests can prove superclass initialization, health classification, exact cleanup predicates, failure typing, and rendered sequencing. The post-final-code focused ALSA/Whisper/health/diagnostic/pipeline sweep passes 265 tests and the complete suite passes all 2,741 tests; compileall, Phase 2 event/memory verification, non-empty JSON configuration parsing, and `git diff --check` are green. Real Raspberry Pi proof remains required for local file permissions, ALSA child behavior, two complete candidates, Whisper output, and standby/shutdown selection.

Phase 119: Request-Scoped ACTIVE Lifecycle Worker

- Real production reached `Processing command` after successful capture and Whisper but did not complete spoken standby. Ctrl+C exposed `ares-active-lifecycle-vosk` blocked in its reusable worker loop's unconditional `connection.recv()`. This proves that the old reusable child survived and was waiting; it does not conclusively locate the foreground parent's block or prove that the child had already returned the current turn's evidence.
- ACTIVE lifecycle Vosk now uses one non-daemon child per bounded recognition. The versioned protocol has correlated ready/request/result/stop/stopped/error messages. No result reaches `BrainRuntime` until the child acknowledges stop or bounded terminate/kill cleanup proves it dead and reaps it. Every IPC receive and process join is bounded; EOF, crash, timeout, parent cancellation, malformed protocol, and cleanup failure are explicit outcomes.
- The child ignores SIGINT so the foreground coordinator remains the sole owner of Ctrl+C. A spoken lifecycle result remains distinct from `owner_cancellation`; no child traceback or idle worker can make cancellation appear to be spoken shutdown. Active transport release also delegates residual worker cleanup before standby microphone ownership resumes.
- The production-factory hardware verifier's `lifecycle` mode requests wake, goodbye, wake, and shutdown only. It reports request sent/received, result, stop, join, reap, transition, and microphone-release evidence and fails if any lifecycle child remains alive. Event-history append failure remains noncritical and cannot block standby/shutdown.
- Post-final-code focused lifecycle/runtime/audio/event verification passes 827 tests and the complete suite passes all 2,747 tests. Compileall, Phase 2 event/memory verification, all deterministic lifecycle verifiers, configuration JSON validation, and `git diff --check` pass. These are deterministic Windows results; Raspberry Pi spoken goodbye/shutdown and child cleanup remain owner-run acceptance.

Phase 120: Bounded Lifecycle Worker Socket Transport

- Phase 120 removes the remaining raw `multiprocessing.Connection`/pickle transport. One non-daemon worker and one private socket pair exist for one finalized candidate only. The wire format is size-capped, length-prefixed JSON with a 32 KiB limit and monotonic deadlines around every send/read/select.
- The `ares.active_lifecycle_worker v1` request/result/stop protocol remains explicit and request-ID correlated. Recognition evidence is held until stop acknowledgement and bounded join/terminate/kill/reap prove the worker is gone. EOF, crash, invalid length/JSON/protocol, timeout, cancellation, and cleanup failure propagate as typed failures into the existing safe same-WAV fallback/retry path.
- The production-composed hardware verifier requests exactly `Ares -> goodbye Ares -> Ares -> shutdown Ares`. Each prompted stage has an outer deadline, worker evidence is correlated, microphone ownership release is checked, any surviving child fails the run, and Ctrl+C remains `owner_cancellation` rather than lifecycle success.
- Phase 120 passes 747 focused lifecycle/runtime/audio/event tests and the complete 2,766-test suite. Compileall, Phase 2 event/memory verification, the deterministic lifecycle verifiers, configuration JSON validation, and `git diff --check` pass. Raspberry Pi spoken standby/reactivation/shutdown and real child timing remain owner-run acceptance.

ARES is currently at the completed Architecture Hardening foundation plus a central deterministic Brain session state machine and persistent foreground runtime, explicit ALSA microphone/speaker adapters, one continuously pumped and calibrated standby PCM stream, constrained Vosk wake recognition, request-scoped constrained Vosk ACTIVE lifecycle recognition over bounded JSON socket IPC, process-group-bounded offline Whisper ordinary-command STT, Piper TTS, adaptive end-of-speech capture, calculator routing, and CoreService-owned explicit owner memory. Owner hardware already proves production-composed ACTIVE preflight, nonzero complete PCM, candidate finalization, and ordinary Whisper transcription. Phase 120 removes reusable lifecycle workers and raw `Connection.recv()` from production, and requires a stop/reap handshake before runtime transition. The complete spoken standby/reactivation/shutdown sequence remains owner-run Raspberry Pi acceptance. Microphone gain, VAD thresholds, wake matching, and lifecycle confidence are not the next tuning target.
...
Phase 110 historically passed 2325 collected tests, Phase 111 historically passed 2358, and Phase 112 passes all 2447 collected tests. Focused and complete pytest, compileall, Phase 2 event/memory verification, configuration validation, and `git diff --check` are green. Deterministic evidence does not replace the bounded Raspberry Pi acceptance run.
...
38. Repair the continuously pumped persistent PCM path and compare it with native direct capture. Completed in deterministic tests and subsequently proved by the owner on Raspberry Pi through wake, acknowledgement, active capture, command finalization, and Whisper start.
39. Centralize exact raw-Whisper lifecycle normalization, ensure a completed/empty/failed/source-local input turn cannot stop the persistent runtime, bypass ordinary routing for standby and shutdown, and print an explicit terminal reason on every exit. Completed in deterministic verification; physical lifecycle behavior remains owner-run.
40. Replace leader-only active Whisper cleanup and unbounded noncritical append locking with the Phase 111 process-group and bounded-lock design. Completed in deterministic verification; Raspberry Pi hardware proof remains owner-run.
41. Run `scripts/manual_verify_standby_wake_hardware.py --verification-mode lifecycle --diagnostic-wake`. Require the production factory to complete wake, goodbye, reactivation, and shutdown with correlated worker request/result/stop evidence, confirmed reap, released microphone ownership, and no Ctrl+C.
42. Run the production calculator/standby/reactivation/shutdown sequence. Require goodbye to return to standby without process exit and shutdown to terminate once. Do not retune microphone, wake, VAD, Whisper, or Vosk thresholds during this checkpoint.
43. Only after that stable hardware proof consider a separately reviewed systemd/boot-startup checkpoint. Wider background listening and autonomous City activation remain out of scope.
