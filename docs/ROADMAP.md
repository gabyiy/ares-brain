...
Phase 114 repairs the active lifecycle-audio probe below transcription and routing. Phase 113's production prompt/pre-roll/state-authority changes remain intact, but the first hardware probe failed before its first useful candidate and therefore did not validate them acoustically. The immediate physical checkpoint is only the diagnostic preflight and nonzero PCM/candidate gate. Complete-phrase transcription and goodbye/shutdown classification may be evaluated only after the retained command WAV is audible. No Windows result is a Raspberry Pi success claim.

ARES is currently at the completed Architecture Hardening foundation plus a central deterministic Brain session state machine and persistent foreground runtime, explicit ALSA microphone/speaker adapters, one continuously pumped and calibrated standby PCM stream, constrained Vosk wake recognition, process-group-bounded offline Whisper active-command STT, Piper TTS, adaptive end-of-speech capture, calculator routing, and CoreService-owned explicit owner memory. Earlier owner runs proved the Raspberry Pi production path through wake, acknowledgement, active capture, and successful approximately two-second Whisper completion. The separate four-phrase diagnostic has not yet reproduced a valid ACTIVE candidate: its first run stopped in the PCM stream before useful audio. The next checkpoint is therefore the repaired diagnostic's structural preflight and audible nonzero candidate, not production lifecycle acceptance or threshold tuning.
