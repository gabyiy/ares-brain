...
Phase 113 is complete in deterministic code and test scope. The production prompt now marks a post-calibration PCM boundary, active candidates retain 0.5 seconds of pre-roll and end after 0.9 seconds of continuous quiet, and activation is legal only from `STANDBY` at the manager boundary. The next physical checkpoint is the four-phrase active lifecycle-audio probe, followed only on success by repeat-name, calculator, goodbye/reactivation, shutdown, `goodbye Aris`, and negated-shutdown production acceptance. No Windows result is a Raspberry Pi success claim.

ARES is currently at the completed Architecture Hardening foundation plus a central deterministic Brain session state machine and persistent foreground runtime, explicit ALSA microphone/speaker adapters, one continuously pumped and calibrated standby PCM stream, constrained Vosk wake recognition, process-group-bounded offline Whisper active-command STT, Piper TTS, adaptive end-of-speech capture, calculator routing, and CoreService-owned explicit owner memory. The owner has proved the Raspberry Pi path through wake, acknowledgement, active capture, and successful approximately two-second Whisper completion. Phase 112 isolates wake activation to `STANDBY`; active attention, lifecycle, and ordinary commands now use state-specific deterministic routing under one stable session. Empty/no-speech/failed turns remain nonterminal, telemetry contention remains bounded, and Brain/CoreService remain free of audio/subprocess implementation. The next physical checkpoint is the production acceptance sequence for repeat-name silence, calculator, goodbye/standby, reactivation, shutdown, `goodbye Aris`, and negated-shutdown retention; PCM, VAD, wake confidence, and Whisper timeout tuning are not part of it.
