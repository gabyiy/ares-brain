...
3. Verify real Raspberry Pi USB microphone plus offline Whisper transcription with the manual STT script.
...
- No real voice/audio implementation beyond the safe VoiceService placeholder, microphone adapter abstraction, Linux ALSA one-shot microphone adapter, speech-to-text adapter abstraction, offline Whisper adapter, VoiceCommandRouter, VoicePipeline, ModuleLifecycleManager, null input/output components, and one-shot VoiceLoop text bridge.
