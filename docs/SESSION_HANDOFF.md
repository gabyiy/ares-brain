...
Owner Raspberry Pi evidence now proves the production transport through calibration, `Ares` wake, one acknowledgement, active capture, speech finalization, and successful Whisper completion in approximately two seconds. The remaining defect was state/routing specific: active `Ares` still entered activation behavior, repeated the acknowledgement, and refreshed inactivity; lifecycle variants were constrained by a name-bearing complete-phrase parser. Phase 112 separates `STANDBY` wake activation from `ACTIVE` attention/lifecycle/ordinary routing, keeps active attention silent by default, and derives bounded bare or addressed command roots. All 2447 deterministic tests plus compileall, Phase 2 event/memory verification, configuration validation, and diff checks pass; the complete physical acceptance sequence remains owner-run.
...
- Active-command diagnostics print raw and cleaned text, removed assistant alias and prefix/suffix/standalone position, lifecycle-normalized command body, negation, classification/action, CoreService bypass, state/session before and after, pipeline status, and runtime terminal flag/reason. The launcher prints an explicit exit reason such as `explicit_shutdown_command`, `owner_cancellation`, or `unrecoverable_failure`.
...
Next Raspberry Pi acceptance command:
...
Pull Phase 112 and run the production foreground acceptance sequence. Persistent PCM, wake, active capture, and bounded Whisper completion are owner-proven, so microphone, VAD, Vosk, wake confidence, and timeout tuning are outside this checkpoint. Prove one acknowledgement on activation, silent active `Ares` with the same session, calculator result four, `goodbye Ares` returning to standby without exit, reactivation with a new session, and `Ares shut down` stopping once. In separate sessions prove `goodbye Aris` returns to standby and `do not shut down Ares` remains active. Runtime ownership remains in Capital/Core. Do not add boot startup, daemonization, barge-in, GPT, cloud services, autonomous City activation, or a second lifecycle/timer system.

Next technical choices:

- Pull latest `main` and run `python scripts/run_ares_standby_voice.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0`, then perform the documented production sequence plus the `goodbye Aris` and negated-shutdown checks.
