...
Current deterministic verification is green: all 2,699 collected tests pass with no skips or xfails, including the post-final-code 800-test subprocess/audio/runtime/event sweep. Compileall, Phase 2 event/memory verification, configuration parsing, and `git diff --check` are rerun before push. These are Windows deterministic results, not Raspberry Pi hardware acceptance.
...
Run the ACTIVE lifecycle-audio diagnostic first. It receives the same production active-audio factory and constrained recognizer as the foreground launcher, checks for a live runtime/gate owner, and prints a structural/resource preflight before its two bounded captures. Each phrase receives one fresh initialized stream and one independent cleanup transaction; no closed object is reused. Constrained Vosk and Whisper fallback consume the same finalized WAV. The probe constructs no Brain runtime and executes no lifecycle, skill, event-history file, or memory action. Audio is deleted unless `--retain-audio` is present:
...
The owner has already proved preflight/open/read, nonzero PCM, complete candidates, and ordinary Whisper on this composition. The next run must finish both phrases within the configured capture, Whisper, and total deadlines. A high constrained action may bypass fallback; medium or rejected constrained evidence must run same-turn Whisper fallback. Before phrase two, the previous child, microphone owner, gate reservation, cancellation state, and temporary transaction must all be clean. Require exit code zero and the summary `both lifecycle phrases passed the bounded decision policy`. Do not claim production lifecycle success until that real owner-audio gate passes. Then run:
...
Pull Phase 117 and run the two-phrase `manual_diagnose_active_lifecycle_audio.py`. The owner has already proved the shared-factory preflight, canonical PCM, nonzero candidates, and ordinary Whisper. Require bounded completion or visible TERM/KILL/reap cleanup, one-press Ctrl+C cancellation, same-turn fallback for medium/rejected constrained evidence, and clean readiness before phrase two. Only after that gate passes should the complete foreground sequence resume. Do not retune microphone gain, wake/VAD thresholds, or lifecycle confidence.

Next technical choices:

- Pull latest `main` and run the focused two-phrase lifecycle-audio diagnostic. Capture constrained text/tokens, confidence/tier/backend, classification, canonical phrase, rejection, selected action, fallback use/status, and process cleanup. High accepted evidence may bypass fallback; medium or rejected evidence must complete same-turn Whisper fallback. Never infer success from headers or Windows tests.
