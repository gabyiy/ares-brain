...
The production-composed ACTIVE diagnostic has passed its Raspberry Pi PCM/candidate gate. The latest real run then isolated a constrained-policy mismatch: Whisper returned `Goodbye, Aris.` and `Shut down Aris.` and the normal ACTIVE lifecycle parser classified them as standby and shutdown, while constrained Vosk returned `goodbye rs` and `shutdown rs`. Old exact policy rejected the first as unmatched and the second as a bounded distractor. Capture, ordinary Whisper, and the normal lifecycle parser are not the current defect.

Phase 116 keeps `ActiveLifecycleAudioRecognizer` at the already-closed WAV boundary and adds one lifecycle-only assistant slot shared by grammar generation and classification. Exactly `ares`, `aris`, `aries`, `arris`, `rs`, or `r s` may fill that slot and canonicalize to `ares`; raw Vosk text/tokens remain unchanged. Seven standby templates generate 42 phrases, six shutdown templates generate 36, and 49 bounded competitors plus `[unk]` make exactly 128 loaded alternatives. Production inference remains in one reusable spawned process with the existing timeout/cleanup policy. The recognizer still cannot open the microphone, recapture, route a skill, mutate memory, or execute a transition.

Each constrained child-work deadline is 5 seconds, followed only on timeout by bounded terminate/kill/reap grace; it is not an absolute preemption claim for synchronous parent process setup. Standby confidence remains `0.70` high and `0.50` medium; shutdown remains `0.78` and `0.60`. Medium evidence requires bounded confirmation. Production decision order is accepted constrained action, otherwise exact valid Whisper lifecycle action, otherwise ordinary routing. Failed, empty, negated, descriptive, and unmatched Whisper text cannot become lifecycle control. `BrainSessionManager` remains the only lifecycle authority.
...
The diagnostic uses the same production factory and recognizer in compare-only mode. It prints the exact loaded grammar only under the required diagnostic flag, then reports raw constrained transcript/tokens, detected alias and position, alias-canonicalized transcript, canonical phrase, confidence/classification/action/rejection/fallback, and comparison Whisper output. One finalized WAV is reused. It constructs no Brain runtime and executes no lifecycle, CoreService, skill, event, or owner-memory action; retained audio remains explicitly diagnostic-only.

Deterministic verification can prove slot boundaries, exact grammar, confidence handling, same-WAV reuse, constrained bypass, Whisper lifecycle fallback, false-positive rejection, and lifecycle authority. It cannot prove the owner's post-fix Vosk text/confidence. The next Raspberry Pi gate remains constrained `standby` for `goodbye Ares`, constrained `shutdown` for `shutdown Ares`, and ordinary fallback for calculator/memory.

Current deterministic verification is green: all 2,688 collected tests pass with no skips or xfails, including the definitive post-final-code 539-test lifecycle/production/diagnostic sweep. Compileall, Phase 2 event/memory verification, five nonempty configuration JSON parses, and `git diff --check` also pass. Four intentionally empty configuration placeholders are not JSON documents. Diagnostic composition reports revision `constrained_active_lifecycle_alias_slot_v2`.
...
Run the ACTIVE lifecycle-audio diagnostic first. It receives the same production active-audio factory and constrained recognizer as the foreground launcher, checks for a live runtime/gate owner, and prints a structural preflight before its four bounded per-phrase captures. Each phrase receives one fresh initialized stream and one independent cleanup transaction; no closed object is reused. Constrained Vosk and comparison Whisper consume the same finalized WAV. The probe constructs no Brain runtime and executes no lifecycle, skill, event, or memory action. Audio is deleted unless `--retain-audio` is present:
...
The owner has already proved preflight/open/read, nonzero PCM, complete candidates, and ordinary Whisper on this composition. The next run must report high-confidence constrained `standby` with canonical `goodbye ares` and no fallback, high-confidence constrained `shutdown` with canonical `shutdown ares` and no fallback, and ordinary/Whisper fallback for the calculator and memory phrases. Require exit code zero and the exact final summary `all four phrases passed the constrained lifecycle policy`. Do not claim production lifecycle success until that real owner-audio gate passes. Then run:
...
Pull Phase 116 and run `manual_diagnose_active_lifecycle_audio.py`. The owner has already proved the shared-factory preflight, canonical PCM, nonzero candidates, correct normal Whisper lifecycle text, and the exact constrained `rs` mismatch. Require the new slot canonicalizer to produce constrained `standby`/`goodbye ares` and constrained `shutdown`/`shutdown ares`, while calculator and memory remain ordinary. Only after that gate passes should the complete foreground sequence resume. Do not retune microphone gain, wake/VAD thresholds, or ordinary Whisper.
...
- Pull `main` on Raspberry Pi and run `manual_diagnose_active_lifecycle_audio.py`. Require constrained `standby`/`goodbye ares`, constrained `shutdown`/`shutdown ares`, and ordinary Whisper fallback for calculator and memory speech. Then run production: wake, calculator result 4, goodbye to standby without exit, new-session reactivation, memory create/recall, and exactly one shutdown. A medium result must confirm; a weak, expired, mismatched, or cancelled result must remain safe.
