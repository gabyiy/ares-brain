...
- `scripts/manual_diagnose_active_transcription.py` is now the next bounded hardware probe. It captures one command through the production ALSA adapter, releases the microphone gate, validates the finalized canonical WAV, runs the exact production Whisper helper, and prints command, PID/PGID, timing, transcript, exit, signal escalation, reap, handle cleanup, and audio cleanup. It never routes the transcript or changes lifecycle or persistent owner state. Deterministic tests prove the control and cleanup paths; the Raspberry Pi owner must still confirm that the WAV is audible and that the installed Whisper binary completes or is killed and reaped within the bound.
...
Production foreground runtime after that probe passes:
