...
The latest code trace found a production timing bug below lifecycle parsing: `ARES is waiting for your command...` was printed before active pipeline startup and before 0.75 seconds of ambient calibration. VAD correctly excludes calibration PCM, so owner speech begun at that prompt could lose a leading `goodbye`, `shutdown`, `calculate`, or `remember` and leave only later words such as `Ares`. The production prompt is now driven by a synchronous post-calibration ready callback. ACTIVE capture retains at least 0.5 seconds of rolling pre-roll, preserves the first onset/start-evidence frame without leading trimming, and retains the existing 0.9-second continuous terminal-quiet endpoint. Device, PCM format, gain, thresholds, persistent standby transport, Vosk, and Whisper confidence were not retuned.

Activation is now independently guarded at the lifecycle authority. `BrainSessionManager.activate_session()` accepts only `STANDBY`; every other state returns `activation_not_allowed` without changing state, session ID, transition history, or acknowledgement count. The runtime handler also rejects any forced non-standby activation. Normal ACTIVE `Ares`/`Aris`/`RS` input remains typed `attention_only`, silent by default, CoreService-bypassing, and session-preserving. Shutdown roots include exact bare or addressed `stop runtime`; negation and complete-phrase constraints are unchanged.

The supplied failing Raspberry Pi output did not contain the raw active Whisper transcript or state at routing, and those private values are intentionally not persisted. Current source cannot emit `Yes Gabi.` from an ACTIVE `Ares` result, so the exact historical transcript cannot be reconstructed honestly. Diagnostic mode now prints a production-composition revision plus capture, transcript, lifecycle, state/session, and activation-handler evidence to distinguish a stale checkout/process from an unexpected state transition on the next owner run.

The owner's first run of `manual_diagnose_active_lifecycle_audio.py` then failed before useful audio on all four phrases: empty transcript, zero pre-roll, zero candidate duration, and `pcm_stream_error:RuntimeError`. That output proves only a PCM capture-stage failure. The diagnostic had manually assembled its microphone and Whisper objects instead of using the foreground launcher's production active-audio composition, and its tests replaced `record_until_silence()` with a fake that wrote a complete WAV without exercising open/read/VAD/close. Independently, VAD discarded the underlying exception message and retained only its class. The exact old RuntimeError, ALSA stderr, child status, and competing-owner state are therefore not recoverable from the supplied terminal output and must not be guessed.

Phase 114 shares one authoritative production active-audio factory between the launcher and diagnostic. It preserves the working production architecture of one bounded stream per active command: reject a live runtime or playback-gate conflict, reuse the production pipeline and adapter, create and start a fresh PCM stream for a phrase, read only after initialization, close after its final read, release ownership in every exit, and never reuse a closed stream. A bounded preflight reports requested/resolved device, canonical format, ownership, open/read state, exact first-frame bytes, nonzero status, stream health, command, process IDs/status, stderr, and cleanup before the owner is invited to speak. One phrase's failed transaction cannot poison the next fresh capture.

Explicit diagnostic output retains a typed failure stage, exception class and complete message, failing adapter and method, microphone device/format, stream state, process context, ALSA stderr, cleanup result, and full traceback. Normal production logs remain bounded and traceback-free. A silent but complete 640-byte preflight frame is structurally valid. The next Raspberry Pi gate is nonzero owner speech reaching VAD and a candidate duration greater than zero; only then may retained-WAV audibility, transcription, and lifecycle classification be evaluated.

The Phase 114 deterministic checkpoint is green at 2514 tests with no skips or xfails. Focused diagnostic/PCM/VAD checks, a broader 1550-test audio/runtime/lifecycle/memory/event sweep, compileall, Phase 2 event/memory verification, parsing of all five nonempty configuration JSON files, and `git diff --check` passed; four intentionally empty configuration placeholders remain excluded from JSON parsing. Neither this checkpoint nor the historical Phase 113 result claims that the repaired diagnostic has yet captured Raspberry Pi audio.

ARES is at the completed Architecture Hardening foundation plus a central deterministic Capital/Core Brain session manager, persistent foreground runtime, and injected Raspberry Pi standby wake listener. Verified components include ALSA input/output, constrained Vosk standby recognition, process-group-bounded offline Whisper active-command STT, offline Piper TTS, configurable voice profiles, adaptive RMS end-of-speech capture, complete ordered utterance assembly, canonical 16 kHz mono PCM normalization, production calculator routing, and central explicit long-term owner memory.

Earlier owner Raspberry Pi evidence proves the production transport through calibration, `Ares` wake, acknowledgement, active capture, speech finalization, and approximately two-second Whisper completion. The separate four-phrase diagnostic has not yet produced one valid candidate on hardware. Physical lifecycle acceptance is deferred until its shared production composition passes preflight, captures nonzero PCM, creates an audible nonempty WAV, and only then returns meaningful transcripts.
...
- Lifecycle normalization is state-specific. `normalize_lifecycle_command()` remains the complete-phrase standby/wake-compatible boundary; `normalize_active_lifecycle_command()` is authoritative for successful active Whisper text before calculator normalization, `IntentParser`, `CoreService`, planning, or skills. The active function lowercases, removes ordinary punctuation, collapses spaces, canonicalizes `good bye`, `stand by`, and `shut down`, and removes at most one supported whole-token assistant address at a prefix or suffix. Canonical/pronunciation `ares`/`aris` and bounded acoustic `rs` addressing are supported without global or substring rewriting.
- Active name-only and bounded greeting forms (`Ares`, `Aris`, `RS`, `Hey Ares`, `Hello Ares`) return typed `attention_only`. They retain the same session and inactivity deadline, bypass CoreService, cannot activate, and are silent by default because the example and runtime default set `already_active_acknowledgement` to empty. Exact standby roots are `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; exact shutdown roots are `shutdown`, `turn off`, `power down`, and `stop runtime`. A root may be bare or addressed at one outer edge. Negation is evaluated first, so negated, explanatory, scheduled, and other longer sentences remain ordinary.
...
- Active-command diagnostics print the raw/cleaned transcript, post-calibration capture-ready reason, first/last speech frames, retained/expected pre-roll, clipping and trim status, candidate/raw durations, removed assistant alias and position, lifecycle-normalized command body, negation, classification/action, CoreService bypass, state/session before and after, activation-handler use, pipeline status, and runtime terminal flag/reason. The launcher prints a privacy-safe composition revision under diagnostic mode and an explicit exit reason such as `explicit_shutdown_command`, `owner_cancellation`, or `unrecoverable_failure`.
- `VoskWakeRecognizer` loads the configured local model once and constrains decoding to exact `ares`, `aris`, and `aries` alias slots under the empty, `hey`, `hello`, `okay`, and `wake up` prefixes plus `[unk]`. Standby/shutdown controls are excluded from the STANDBY request and can execute only after ACTIVE Whisper transcription. Complete-phrase token matching remains mandatory; there is no substring, edit-distance, fuzzy, or learned-alias fallback.
...
- The latest Raspberry Pi terminal stopped after `Transcribing command`. That observation proves capture returned but cannot distinguish a blocked inference wait, child cleanup, or subsequent telemetry append. Code tracing independently found that the former Whisper helper created no Linux process group, signalled only its leader, and could return with `reaped=False` while descendants retained output pipes. It also found that `EventHistoryStore` acquired its in-process `RLock` without a deadline. These are concrete defects; this handoff does not infer from the Pi line which one dominated that turn.
- Active command inference now has a dedicated 15-second Raspberry Pi timeout, one-second process-group termination grace, and three-second complete cleanup deadline; it remains independent of the broader 300-second launcher budget. `WhisperSubprocessRunner` starts a new Linux session, records PID/PGID, writes stdout/stderr to bounded temporary files, sends group `SIGTERM`, escalates to group `SIGKILL`, reaps the leader, closes output handles, and only then reports cleanup. Ctrl+C and pipeline shutdown use the same cancellable helper.
...
- Production now composes one authoritative `EventHistoryStore` across BrainRuntime, BrainSessionManager, CoreService/resource management, SkillManager, and SingleTurnVoicePipeline. A noncritical append waits at most 0.05 seconds for the in-process lock; contention or an expected persistence failure prints one bounded warning, increments the in-memory dropped-event counter, and cannot stop activation, acknowledgement, active commands, calculator or memory execution, standby, or shutdown. File-lock recovery remains limited to expired local locks whose PID is proven dead. Unexpected programming errors still surface.
...
Run the repaired ACTIVE lifecycle-audio diagnostic first. It receives the same production active-audio factory as the foreground launcher, checks for a live runtime/gate owner, and prints a structural preflight before its four bounded per-phrase captures. Each phrase receives one fresh initialized stream and one independent cleanup transaction; no closed object is reused. The probe constructs no Brain runtime and executes no lifecycle, skill, event, or memory action. Audio is deleted unless `--retain-audio` is present:
...
First require preflight open/read success with an exact 640-byte frame and then require owner speech to produce nonzero PCM, retained pre-roll, and a candidate duration above zero. Only after those WAVs are audible and retain complete leading command words should transcripts/classifications be considered or the production acceptance command be run:
...
Pull Phase 114 and first run `manual_diagnose_active_lifecycle_audio.py`. Confirm the shared-factory preflight reports the intended `plughw:2,0`, 16 kHz mono `S16_LE`, open/read success, a complete 640-byte frame, healthy ownership, and no unexplained child exit or stderr. Then require spoken PCM, a candidate duration greater than zero, and an audible retained WAV for each phrase. Do not draw lifecycle conclusions from an empty candidate. Only after the audio gate passes should the production foreground acceptance sequence resume. Runtime ownership remains in Capital/Core. Do not retune microphone gain, wake/VAD thresholds, Vosk, or Whisper confidence, and do not add boot startup, daemonization, barge-in, GPT, cloud services, autonomous City activation, or a second lifecycle/timer system.

Next technical choices:

- Pull latest `main`, run the focused four-phrase lifecycle-audio diagnostic with explicit retention, and stop at the first failed preflight or zero-duration candidate. Capture its complete exception, traceback, argv, process status, stderr, ownership, and cleanup output. Only after all retained WAVs are audible should production `goodbye`/`shutdown` acceptance resume.
...
- `core.AresIdentity` remains the bounded wake/general owner-name alias authority. `core.LifecycleControl.normalize_active_lifecycle_command()` is the active-state authority and runs on raw Whisper text before calculator normalization, `CoreService`, parser, planner, `SkillManager`, or skills. Active acoustic addressing is bounded to complete edge tokens and is never added to general substring normalization.
...
- Standby-wake and active-lifecycle tests cover versioned listener/recognizer contracts, shared whole-token `Ares`/`Aris`/exact-slot `Aries` identity normalization, grammar and alias normalization, shutdown/standby collisions, `[unk]`, exact confidence boundaries, bounded medium-confidence confirmation, persistent stream/calibration reuse, unchanged command VAD profiles, canonical-WAV-only recognition, one-session/one-acknowledgement activation, raw-Whisper lifecycle normalization before ordinary routing, punctuation/spacing/case variants, exact standby and shutdown phrase matrices, longer-sentence and negative-command rejection, CoreService bypass, empty/no-speech/Whisper-failure/source-local-EOF nontermination, standby session clearing, reactivation with a new session, exactly-once explicit shutdown, terminal-reason diagnostics, calculator/owner-memory ordinary routing, self-wake exclusion, output isolation, event and memory privacy, CLI defaults, and bounded hardware verification.
...
- Persistent PCM is owner-proven healthy through calibration, Vosk wake, acknowledgement, active capture, speech endpointing, and successful bounded Whisper completion. Do not restart microphone, VAD, threshold, or Whisper timeout tuning for the current routing acceptance.
- Pull `main` on Raspberry Pi and run the production foreground command. Require one acknowledgement, no reactivation or spoken response for active name-only input, stable session identity through calculator routing, goodbye-to-standby without exit, a new session after reactivation, one explicit shutdown, `goodbye Aris` standby behavior, and negated-shutdown retention.
