...
Phase 108 repairs the persistent raw-PCM transport before any further wake tuning. The production trace classified the code-level defect: the old `arecord` process remained open, but its stdout was drained only when calibration or VAD synchronously requested a frame. Prompt, recognition, and other consumer gaps left stale quiet PCM at the pipe head and could block the subprocess; stderr was also piped without a continuous drain. The same one-process/one-owner architecture now continuously reconstructs exact immutable 640-byte frames into a bounded latest-audio queue, drains stderr separately, and fails closed on malformed/ended/dead/pathologically repeated transport states. All 2203 local tests pass, proving deterministic framing, ownership, calibration-boundary continuity, and diagnostic failure gates but not Raspberry Pi amplitude. The repaired four-WAV direct-versus-persistent probe and explicit audibility confirmation remain mandatory before the 9/10 wake run or production hardware claim.
...
- `core.AresIdentity` provides one immutable validated owner-name alias policy to Vosk wake construction and active lifecycle parsing. Exact alias-slot expansion covers `ares`, `aris`, and `aries`; `paris`, `harris`, zodiac questions, and arbitrary surrounding sentences remain unchanged. `core.LifecycleControl` exact-matches the complete command before `CoreService`, `IntentParser`, planner, `SkillManager`, or skills.
...
- Active command inference now has a dedicated `--active-transcription-timeout` with a 30-second Raspberry Pi default; it is no longer coupled to the broader 300-second launcher budget. The timeout reaches `LinuxWhisperSpeechToTextAdapter` through the finalized audio contract and is enforced inside `WhisperSubprocessRunner` with bounded pipe draining, terminate/wait/kill/reap cleanup, and structured `transcription_timeout` failure. Ctrl+C reaps the child before cancellation continues.
...
- `scripts/manual_diagnose_persistent_pcm.py` is the mandatory Raspberry Pi prerequisite before another wake-reliability run. It acquires the foreground runtime lock; records separate three-second quiet and spoken native `hw:2,0` 44.1 kHz captures; records the same two stages through the exact production `plughw:2,0` 16 kHz persistent stream; and saves four WAVs plus `pcm_integrity_report.json`. The report includes actual headers, signed-sample distributions and amplitude bands, direct/persistent spoken-to-quiet ratios, cross-path comparability, exact commands/PID/exit state, and complete framing/queue accounting. Playback occurs only with explicit `--playback`, after ALSA capture closes, and success requires the owner to confirm both spoken WAVs are clear and audible. Local tests do not prove hardware amplitude.
...
Keep ARES stopped while this command acquires the foreground runtime lock. Follow the four stage prompts: three seconds quiet and three seconds speaking through native direct capture, then three seconds quiet and three seconds speaking through the exact production persistent stream. The probe must retain valid `direct_quiet.wav`, `direct_spoken.wav`, `persistent_quiet.wav`, `persistent_spoken.wav`, and `pcm_integrity_report.json`; report complete transport/process accounting and comparable signal contrast; play both spoken WAVs only after ALSA closes; and record explicit owner audibility confirmation. The code-level pull-only/backpressure defect is classified and repaired, but its physical correction is not proven until this Raspberry Pi probe passes. Neither the ten-attempt wake verifier nor production standby voice is the next hardware action before that proof.
...
Pull Phase 108 and run the repaired direct-versus-persistent PCM probe before any more wake reliability or threshold tuning. Require four structurally valid WAVs, complete framing/process accounting, a persistent spoken stage above its quiet stage, reasonably comparable direct/persistent contrast, and explicit confirmation that both spoken WAVs are clear and audible. Local passing tests cannot establish Raspberry Pi microphone amplitude. Only after the probe passes may the owner resume the 9/10 wake verifier and complete constrained Vosk wake, output-only acknowledgement, active Whisper calculator, exact standby, reactivation, and shutdown evidence. Runtime ownership remains in Capital/Core. Do not add boot startup, daemonization, barge-in, GPT, cloud services, autonomous City activation, or a second lifecycle/timer system.

Next technical choices:

- Pull latest `main`, keep the foreground runtime stopped, and run `python scripts/manual_diagnose_persistent_pcm.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --record-seconds 3 --playback`. Do not run the wake verifier until the probe passes and both spoken WAVs are explicitly confirmed audible.
...
- Standby-wake tests cover versioned listener/recognizer contracts, shared whole-token `Ares`/`Aris`/exact-slot `Aries` identity normalization, grammar and alias normalization, shutdown/standby collisions, `[unk]`, exact confidence boundaries, bounded medium-confidence confirmation, validated exact missing-confidence activation, malformed missing-confidence rejection, unrelated-word and partial-word rejection, queued/Linux listener lifecycle, persistent stream/calibration reuse, wake-only and unchanged command VAD profiles, actual-header duration guards, canonical-WAV-only recognition, constrained grammar construction, in-process timeout/cancellation, terminal-only separated wake/active diagnostics, bounded latest-candidate retention, missing dependency/model failures before capture, capability metadata, one-session/one-acknowledgement runtime activation, active Whisper calculator and owner-memory routing, strict lifecycle interception before CoreService/skills, standby/inactivity/reactivation/shutdown, self-wake exclusion, output isolation, event and memory privacy, CLI defaults, one-attempt diagnosis, and bounded hardware verification.
...
- Pull `main` on Raspberry Pi, stop any foreground ARES process, and first run `python scripts/manual_diagnose_persistent_pcm.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --record-seconds 3 --playback`. Require four structurally valid WAVs, complete transport accounting, realistic and comparable persistent speech contrast, and explicit audibility confirmation for both spoken files. The code-level pull-only/backpressure root cause is fixed; this probe is the remaining physical proof.
