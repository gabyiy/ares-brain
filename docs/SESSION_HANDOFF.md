...
- Lifecycle authorization is state-specific. STANDBY wake recognition can only activate. Real ACTIVE audio must first pass `ActiveLifecycleAudioRecognizer` at the finalized-WAV boundary; only accepted high-confidence evidence or a confirmed medium result can request standby/shutdown. Transcript normalization remains available for typed/simulated input, attention-only handling, diagnostics, and ordinary command shaping, but fallback Whisper text cannot bypass the audio authorization policy.
- Active name-only and bounded greeting forms (`Ares`, `Aris`, `RS`, `Hey Ares`, `Hello Ares`) return typed `attention_only`. They retain the same session and inactivity deadline, bypass CoreService, cannot activate, and are silent by default. They are also explicit non-action competitors in the constrained recognizer. Real audio standby/shutdown authorization uses only the exact 9/12 phrase grammar above; broader typed/simulated normalization cannot authorize fallback Whisper audio.
...
- `VoskWakeRecognizer` loads the configured local model once and constrains STANDBY decoding to exact `ares`, `aris`, and `aries` alias slots under the empty, `hey`, `hello`, `okay`, and `wake up` prefixes plus `[unk]`. Standby/shutdown controls remain excluded from that request. ACTIVE uses the separate `VoskLifecycleGrammarBackend` on an existing finalized WAV, with `ares`/`aris` only and its own exact bounded grammar. Complete-phrase token matching remains mandatory; there is no substring, edit-distance, fuzzy, semantic, or learned-alias fallback.
...
Pull Phase 115 and run `manual_diagnose_active_lifecycle_audio.py`. The owner has already proved the shared-factory preflight, canonical 640-byte PCM, nonzero candidates, and ordinary Whisper. Now require constrained `standby`/`goodbye ares` for spoken goodbye, constrained `shutdown`/`shutdown ares` for spoken shutdown, and ordinary fallback for calculator and memory. Only after that audio-classification gate passes should the complete foreground sequence resume. Runtime ownership remains in Capital/Core. Do not retune microphone gain, wake/VAD thresholds, or ordinary Whisper, and do not add boot startup, daemonization, barge-in, GPT, cloud services, autonomous City activation, or a second lifecycle/timer system.
...
- `core.AresIdentity` remains the bounded wake/general owner-name alias authority. `core.ActiveLifecycleAudioRecognizer` is the real ACTIVE audio authorization boundary and runs on the finalized WAV before Whisper. `core.LifecycleControl.normalize_active_lifecycle_command()` remains a deterministic typed/simulated/diagnostic normalization helper, but fallback Whisper cannot bypass constrained confidence or confirmation. Acoustic aliases are never added to general substring normalization.
