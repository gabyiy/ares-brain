...
Deterministic verification is green: 2487 tests passed with no skips or xfails; focused active-capture/lifecycle/composition and calculator/owner-memory/event/Whisper regressions passed; compileall, Phase 2 event/memory verification, all configuration JSON parsing, and `git diff --check` passed. These results do not claim Raspberry Pi audio success.
...
Owner Raspberry Pi evidence proves the transport through calibration, `Ares` wake, acknowledgement, active capture, speech finalization, and approximately two-second Whisper completion. It does not yet prove that complete ACTIVE phrase beginnings survive capture or that the latest state-isolated route is running on the Pi. Phase 113 repairs and deterministically tests those boundaries; physical lifecycle acceptance remains owner-run and must begin with the focused four-phrase audio probe.
...
Run the frame-safe ACTIVE lifecycle-audio diagnostic first. It uses the exact production argument mapping, recorder profile, ALSA adapter, bounded Whisper helper, and central lifecycle normalizer, but constructs no Brain runtime and executes no lifecycle, skill, event, or memory action. It asks for four fixed phrases and deletes audio unless `--retain-audio` is present:
...
Only after those WAVs audibly retain complete leading command words and their transcripts/classifications are correct, run the production acceptance command:
...
Pull Phase 113 and first run `manual_diagnose_active_lifecycle_audio.py`. Confirm the retained WAVs audibly contain the complete beginnings of all four phrases and that raw transcripts/classifications agree. Only then run the production foreground acceptance sequence: one acknowledgement on activation, silent active `Ares` with the same session, calculator result four, `goodbye Ares` returning to standby without exit, reactivation with a new session, and `Ares shut down` stopping once. In separate sessions prove `goodbye Aris` returns to standby and `do not shut down Ares` remains active. Runtime ownership remains in Capital/Core. Do not retune microphone gain, wake/VAD thresholds, Vosk, or Whisper confidence, and do not add boot startup, daemonization, barge-in, GPT, cloud services, autonomous City activation, or a second lifecycle/timer system.

Next technical choices:

- Pull latest `main`, run the focused four-phrase lifecycle-audio diagnostic with explicit retention, listen to each WAV, then run `python scripts/run_ares_standby_voice.py --microphone-device plughw:2,0 --speaker-device plughw:CARD=Device,DEV=0 --diagnostic-routing` for the documented production sequence plus `goodbye Aris` and negated-shutdown checks.
