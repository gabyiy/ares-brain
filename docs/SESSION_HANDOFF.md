...
ARES is at the completed Architecture Hardening foundation plus a central deterministic Capital/Core Brain session manager, persistent foreground runtime, and injected Raspberry Pi standby wake listener. Verified components include ALSA input/output, constrained Vosk standby recognition, offline Whisper active-command STT, offline Piper TTS, configurable voice profiles, controlled single-turn and bounded multi-turn pipelines, adaptive RMS end-of-speech capture, complete ordered utterance assembly, duration-checked canonical 16 kHz mono PCM normalization, production natural-language calculator routing, and central explicit long-term owner memory with confirmation-gated CRUD. Real Raspberry Pi evidence proves Vosk recognized `ares` at about 0.823 confidence, activated one session, spoke `Yes Gabi.`, and the active Whisper/calculator/Piper route returned `Result: 4`; `goodbye Ares` also succeeded in one sequence. Wake remained intermittent because standby capture reopened ALSA and recalibrated on every poll. Whisper also emitted `Shut down Aries.`, but the prior active alias policy knew only `ares`/`aris`. This checkpoint fixes those paths in deterministic Windows verification. A quiet-room 9/10 wake result and the complete standby/reactivation/shutdown sequence remain owner-run Raspberry Pi requirements.
...
- The real intermittent-wake cause was architectural: `listen_once()` invoked one-shot ALSA capture on every poll, so `arecord` reopened, ambient calibration reran, pre-roll reset, and a short word could fall between fresh capture windows. Standby now holds one explicitly owned ALSA stream, calibrates once per standby epoch, retains bounded rolling pre-roll, and reuses both across unlimited timeout/rejection candidates. Timed/manual recalibration happens on the same stream; device failure closes and reopens one replacement stream.
...
- Word output is mandatory. Exact wake confidence at or above 0.8 activates immediately. Exact 0.72-0.8 results require two identical recognitions within eight seconds. Missing/invalid confidence, lower confidence, `[unk]`, extra words, `okay`, `bye`, `alrighty`, `areas`, `air`, and sentences that merely mention Ares all reject. `Aries` resolves to canonical `ares` only in an exact configured name slot.
