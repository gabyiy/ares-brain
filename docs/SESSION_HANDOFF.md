...
The production Raspberry Pi path now proves wake, active capture, and ordinary Whisper completion. The latest run reached `Processing command` for spoken `goodbye Ares` but did not complete standby. Ctrl+C exposed an old reusable `ares-active-lifecycle-vosk` child blocked in `_vosk_lifecycle_worker_main` at an unconditional `connection.recv()`. That traceback proves a leaked waiting child, but it does not conclusively identify the foreground parent's blocking location. Ctrl+C was owner cancellation, not spoken lifecycle success.

Phase 119 addressed the proven leaked-worker boundary without claiming it was the foreground block: it replaced the reusable daemon with one non-daemon child per candidate, correlated a versioned request/result with a unique ID, added explicit stop acknowledgement, and withheld the recognition result until bounded join/terminate/kill cleanup confirmed process reap. The child ignores SIGINT so Ctrl+C remains the foreground parent's distinct `owner_cancellation` path.

Phase 120 supersedes the raw `multiprocessing.Connection` transport used by that first request-scoped repair. Production now gives each candidate one private socket pair and exchanges only size-capped, length-prefixed JSON. There is no production `Connection.send()`/`recv()` or pickle-framed lifecycle message. The explicit `ares.active_lifecycle_worker v1` `ready -> recognize_request -> recognize_result -> stop_request -> stopped` protocol remains correlated by request ID, and every send/read/select/join is bounded by a monotonic deadline. A result remains withheld until bounded cleanup proves the worker dead and reaped. Cleanup failures propagate as typed failures into the safe same-WAV fallback/retry path instead of releasing a result alongside a live child.
...
Phase 118 isolates the new pre-inference failure precisely. `WhisperSubprocessRunner.__init__()` did not call `SafeSubprocessRunner.__init__()`, so the parent's `_runner` executable-resolution backend never existed and inherited `which()` raised `AttributeError: 'WhisperSubprocessRunner' object has no attribute '_runner'` during `LinuxWhisperSpeechToTextAdapter.health_check()`. Calling the superclass constructor fixes that production health path. The STT check now independently verifies command resolution and execute permission, model readability, writable temporary output, command construction, child/reap/handle state, and clear cancellation state without opening the microphone or starting inference.

The adjacent microphone preflight evidence is a different process boundary. An `arecord` exit code 1 with only the exact `pcm_read ... read error: Interrupted system call` line after a valid frame and explicit stop is expected controlled cleanup only when the child was alive at stop, no active transport failure preceded it, and process/pipes were completely reaped/closed. That exact case has no health effect. Cleanup-time SIGINT/SIGTERM may be controlled; a reaped SIGKILL is degraded-but-reusable only after recorded bounded escalation. Other negative signals, mixed stderr, zero valid PCM, active failure, unexpected death or ownership loss, unrelated stderr, or incomplete cleanup remains unhealthy. The exact stderr signature is not a broad exit-one exemption.

The ACTIVE constrained decision policy is now complete: high-confidence accepted evidence may bypass Whisper; medium/uncertain or rejected evidence must run same-turn Whisper fallback on the already-closed WAV. Valid fallback selects standby/shutdown, while a failed or unrelated fallback remains ordinary. The invalid state `classification=uncertain`, `action=none`, `whisper_fallback=no` is no longer representable. `BrainSessionManager` remains the only lifecycle authority.

The accepted slot occurs only after `goodbye`, `good bye`, `bye`, `go standby`, `go to standby`, `standby`, `sleep`, `shutdown`, `shut down`, `turn off`, or `power off`, or before the already-supported `shutdown`/`shut down` roots. Matching remains complete-command and token-boundary based. `artist`, `paris`, `harris`, name-only input, missing-name commands, unrelated sentences, and extra words remain non-actions. There is no substring, fuzzy, edit-distance, semantic, learned-alias, or general phonetic matching.
...
Phase 119 historically passed all 2,747 tests and its post-final-code focused lifecycle/runtime/audio/event sweep passed 827 tests. Phase 120 passes 747 focused lifecycle/runtime/audio/event tests and the complete 2,766-test suite. Compileall, Phase 2 event/memory verification, the deterministic lifecycle verifiers, configuration JSON validation, and `git diff --check` pass. These deterministic results can prove request/stop/timeout/cancellation/reap behavior; they cannot prove spoken goodbye or shutdown on the Raspberry Pi.
...
The bounded verifier must complete exactly `Ares -> goodbye Ares -> Ares -> shutdown Ares`, apply an outer deadline to every prompted stage, correlate request/result/stop/reap evidence, report microphone ownership released, prove no lifecycle child remains, and exit without Ctrl+C. Production must then complete the same standby/reactivation/shutdown transitions. Windows automation is not hardware proof.

Checkpoint root causes and fixes:

- Phase 113 separates active setup/calibration from owner waiting. After playback and settling, the one-shot stream opens and calibrates before a synchronous ready callback prints the prompt. The `active_command_v1` wrapper preserves existing thresholds/PCM and applies 0.5-second pre-roll plus 0.9-second continuous terminal quiet; deterministic PCM tests prove onset, first-frame, read-boundary, pause, and endpoint preservation.
- Phase 114 fixes the probe used to validate Phase 113 on real hardware. The prior probe was not factory-identical to production and erased low-level failure detail. The shared factory, per-phrase bounded ownership, structural preflight, exact exception/stderr reporting, diagnostic-only traceback, phrase isolation, and unconditional cleanup are now the required checkpoint. Goodbye and shutdown recognition remain unverified until capture produces a nonzero candidate.
- Phase 115 records that the Phase 114 hardware gate passed and isolates the remaining failure to short lifecycle acoustics. A typed finalized-audio hook now runs constrained Vosk on the existing canonical WAV before Whisper; high evidence bypasses Whisper, medium evidence requires confirmation, and ordinary/weak/error evidence reaches unchanged Whisper without a second capture. The diagnostic reports both recognizers without executing the proposed action.
...
- Phase 120 replaces raw `multiprocessing.Connection`/pickle framing with a private, size-capped, length-prefixed JSON socket transport; withholds results until cleanup/reap; propagates cleanup failures; and hardens the exact four-stage hardware verifier with outer deadlines and no-orphan evidence. Raspberry Pi spoken lifecycle acceptance remains pending.
...
- Lifecycle authorization is state-specific. STANDBY wake recognition can only activate. ACTIVE evaluates constrained lifecycle evidence first, then the existing exact Whisper lifecycle parser, then ordinary routing. Both lifecycle paths remain complete-phrase deterministic requests to `BrainSessionManager`.
- Active name-only and bounded greeting forms (`Ares`, `Aris`, `RS`, `Hey Ares`, `Hello Ares`) return typed `attention_only`. They retain the same session and inactivity deadline, bypass CoreService, cannot activate, and are silent by default. Slot aliases never turn name-only or unrelated input into lifecycle action.
...
- Active-command diagnostics print capture readiness/timing, canonical WAV evidence, constrained text/tokens/confidence/tier/backend/classification/canonical/rejection, confirmation status, selected action, whether Whisper fallback ran, optional raw/cleaned Whisper text, CoreService bypass, state/session before and after, pipeline status, and runtime terminal flag/reason. The launcher prints a privacy-safe composition revision under diagnostic mode and an explicit exit reason such as `explicit_shutdown_command`, `owner_cancellation`, or `unrecoverable_failure`.
- The lifecycle-audio probe separately reports `microphone_open_error`, `microphone_health_error`, `microphone_cleanup_error`, `pcm_read_error`, `invalid_frame_error`, `no_speech_timeout`, `VAD_error`, `wav_write_error`, `whisper_configuration_error`, `speech_to_text_health_error`, `whisper_error`, `empty_transcript`, `lifecycle_parse_error`, and `lifecycle_recognition_mismatch`. UTC markers identify preflight start, valid PCM, controlled stop, preflight pass, actual capture start, genuine finalized STT completion, microphone release, and temporary-file finalization. It never prints completion merely because a failed pipeline invocation returned.
- `VoskWakeRecognizer` loads the configured local model once and constrains STANDBY decoding to its unchanged exact wake aliases and prefixes plus `[unk]`. Standby/shutdown controls remain excluded from that request. ACTIVE uses the separate `VoskLifecycleGrammarBackend` on an existing finalized WAV with its six lifecycle-only slot forms. Complete-phrase token matching remains mandatory; there is no substring, edit-distance, fuzzy, semantic, or learned-alias fallback.
...
- The persistent stream contract is 16,000 Hz, mono, signed little-endian `S16_LE`; 20 ms is exactly 320 samples and 640 bytes. Low-level chunks, pump frames, and rolling/pre-roll boundaries copy into immutable `bytes`. Arbitrary short reads accumulate losslessly, extra complete frames are preserved, and incomplete/error paths never synthesize zero-filled bytes. A single `ContinuousPcmFrameSource` owns stdout for the full process lifetime and keeps at most 50 recent frames, dropping and counting the oldest backlog instead of allowing the pipe to block. Reset epochs discard only queued or in-flight pre-boundary frames.
- Integrity diagnostics expose exact `arecord` argv, requested/resolved device and format, PID/poll status, stdout/stderr transport modes, read-size counts, `total_low_level_reads`, `valid_full_pcm_frames`, `partial_reads`, `accumulated_partial_bytes`, `empty_reads`, EOF/dead-process classes, `read_errors`, `discarded_bytes`, `zero_filled_bytes`, queue/reset drops, `repeated_frame_hashes`, non-silent duplicate runs, tiny-RMS runs, `mutable_buffer_reuse_detected`, and exact bytes delivered to VAD. Stderr has a separate bounded drain and can never enter PCM. Because headerless raw PCM cannot independently report negotiated format, diagnostics label 16 kHz/mono/`S16_LE` as the requested production contract and use actual WAV headers for the native direct and serialized persistent stages.
...
- Wake and active recognition have separate authority. Vosk wake classification can create a session only in `STANDBY`; it is never invoked as activation from active/processing/responding/returning/shutdown states. Active attention/lifecycle parsing uses only bounded whole tokens and cannot corrupt `paris`, `harris`, zodiac questions, or embedded ordinary text.
- Recognized standby and shutdown commands bypass ordinary routing. Standby transitions `ACTIVE -> RETURNING_TO_STANDBY -> STANDBY`, clears the session once, and cannot produce the unknown fallback. Shutdown transitions through `SHUTTING_DOWN -> STOPPED` and closes adapters. Usable ordinary owner speech resets the 30-second activity deadline; attention-only input, empty/no-usable speech, ARES playback, and diagnostics do not.
- `VoiceRuntimeGate` makes microphone ownership testable: standby owns one stream only in `STANDBY`, releases it before acknowledgement and active capture, and blocks capture throughout every playback. A fresh active capture opens only after playback plus 0.35 seconds of settling. The wait prompt is emitted after that gate succeeds and capture ownership is secured, so a 0.25-second poll that expires during settling prints no duplicate waiting message.
...
- The latest Raspberry Pi terminal reached phrase two, printed `Whisper process started` and the 15-second timeout, then blocked indefinitely. Source tracing found that the banner only declared configuration: the parent then entered one blocking child wait instead of owning deadline progress. The pipeline and runtime adapters also caught `KeyboardInterrupt` and returned typed failures, so Ctrl+C never reached the foreground cleanup owner.
- Active command inference retains its dedicated 15-second Raspberry Pi timeout, one-second group termination grace, and three-second cleanup deadline. `WhisperSubprocessRunner` now enforces wall time through short monotonic `poll()` checks, starts a new Linux session, records PID/PGID, writes stdout/stderr to bounded private files, sends group `SIGTERM`, escalates to group `SIGKILL`, verifies exit, reaps the leader, and closes handles. The same bounded primitive protects one-shot `arecord`, Piper, and `aplay`; persistent `arecord` has a bounded group close path.
- `ForegroundSignalCoordinator` owns SIGINT/SIGTERM at the production/diagnostic boundary. Voice pipeline and runtime adapters clean local resources and re-raise `KeyboardInterrupt`; all cancellation routes therefore converge on one idempotent top-level cleanup transaction. Progress timestamps are diagnostic-only.
- The ALSA adapter and shared `VoiceRuntimeGate` both release active microphone ownership before Whisper starts. The WAV writer is closed, then the pipeline reopens and validates a nonempty 16 kHz mono signed 16-bit PCM file. Current-turn command audio uses `delete_always` in normal foreground operation, so success, timeout, and expected transcription failure do not leave WAVs; explicit diagnostic retention remains opt-in.
- `--diagnostic-wake` now prints active finalization timestamps, optional local WAV path, bytes and format, transcription backend/start/completion/timeout/duration, parse status, routing timing/result, lifecycle before/after, and cleanup status. Raw active transcripts and paths remain terminal-only and do not enter events, memory, or persistent logs.
- Production composes one authoritative `EventHistoryStore` across BrainRuntime, BrainSessionManager, CoreService/resource management, SkillManager, and SingleTurnVoicePipeline. Expected event-history persistence/lock failures print one bounded warning, increment the in-memory dropped-event counter, and cannot stop live behavior; unexpected programming errors still surface. The ACTIVE diagnostic injects a nonpersistent sink and never opens production event history. File-lock recovery remains limited to expired local locks whose PID is proven dead.
...
- Event-history appends automatically recover only expired local lock metadata whose PID is proven dead. Live, malformed, remote, and unprovable legacy locks remain untouched and produce a bounded telemetry warning. Owner-memory and other critical durable stores remain strict.
- `scripts/run_ares_standby_voice.py` holds a separate process lock at `data/runtime/ares_standby_voice.runtime.lock`; a second process exits with `ARES is already running (PID <owner>)`, and normal shutdown, handled runtime errors, or Ctrl+C release ownership. Hardware verification uses an isolated temporary runtime lock.
...
- The contradictory `ares`/`1.000` plus zero-duration `invalid_audio` display came from reading `last_result` and `last_diagnostics` independently. `WakeAttemptResult` is now frozen and generation-bound; its constructor rejects recognition on invalid/empty/noncanonical audio, and runtime/verifier lookup requires the exact attempt ID.
- Stream recovery now follows `CLOSED -> OPENING -> CALIBRATING -> HEALTHY` or `FAILED`. Calibration failure attempts a bounded close and returns an infrastructure failure. Confirmed closure clears rolling/candidate/Vosk/calibration state and releases capture ownership; a failed close retains the exact handle and `VoiceRuntimeGate` owner for retry, preventing another capture from entering while ALSA may still be locked. A later successful reopen creates a new generation; older results cannot activate or appear in its diagnostics.
...
- `scripts/manual_diagnose_persistent_pcm.py` remains the bounded native-direct versus production-persistent integrity probe. It acquires the foreground runtime lock; records separate three-second quiet and spoken native `hw:2,0` 44.1 kHz captures; records the same stages through the exact production `plughw:2,0` 16 kHz persistent stream; and saves four WAVs plus `pcm_integrity_report.json`. The owner has now proved the repaired production route reaches live wake and active Whisper capture with practical microphone audio, so rerunning this probe is not the next lifecycle action.
- `scripts/manual_diagnose_active_transcription.py` remains available for bounded process diagnosis. The owner has now proved production Whisper starts in its own group, completes in approximately two seconds, and exits zero, so it is not the next routing checkpoint.
...
Run the production-composed STT configuration preflight first. It validates the command/model/output/process state without opening the microphone or starting inference:
...
Require `Microphone opened: no`, `Speech inference requested: no`, executable/model/output/command checks `yes`, no current child, prior child reaped, cancellation clear, STT `final decision: healthy`, and `STT configuration preflight result: passed`.

Then run the ACTIVE lifecycle-audio diagnostic. It receives the same production active-audio factory and constrained recognizer as the foreground launcher, checks for a live runtime/gate owner, and prints structural microphone and independent STT health before its two bounded captures. Each phrase receives one fresh initialized stream and one independent cleanup transaction; no closed object is reused. Constrained Vosk and Whisper fallback consume the same finalized WAV. The probe constructs no Brain runtime and executes no lifecycle, skill, event-history file, or memory action. Audio is deleted unless `--retain-audio` is present:
...
The owner has already proved preflight/open/read, nonzero PCM, complete candidates, and ordinary Whisper on this composition. The next run must show timestamped preflight start, valid PCM, controlled stop, preflight pass, capture start, actual capture/STT completion, microphone release, and file finalization. Controlled cleanup requires a reaped child, complete cleanup, no unexpected failure, and no health effect; exit one plus the exact interrupted-`pcm_read` message is acceptable only with all of that control-plane evidence. Each phrase must report independent microphone/STT health and finish within the configured capture, Whisper, and total deadlines. A high constrained action may bypass fallback; medium or rejected constrained evidence must run same-turn Whisper fallback. Before phrase two, the previous child, microphone owner, gate reservation, cancellation state, and temporary transaction must all be clean. Require exit code zero and the summary `both lifecycle phrases passed the bounded decision policy`. Do not claim production lifecycle success until that real owner-audio gate passes. Then run:
...
- Owner-run `scripts/manual_diagnose_persistent_pcm.py` native-direct versus production-persistent comparison with four WAVs, one JSON report, signed-sample distributions, process/framing/queue counters, and explicit-only playback/audibility confirmation
...
Owner-proven Raspberry Pi persistent-PCM checkpoint:
...
Say `Ares` and hear `Yes Gabi.` once; say `Ares` again and require silence with the same session; say `calculate two plus two` and hear `Result: 4.`; say `goodbye Ares` and require standby without process exit; reactivate and require a new session; then say `Ares shut down` and require one clean exit. Run separate sessions for `goodbye Aris` returning to standby and `do not shut down Ares` remaining active. These owner runs are physical transcription, playback-settling, and lifecycle proof; deterministic tests cannot replace them.
...
Pull Phase 120 and run the production-composed lifecycle verifier with `--verification-mode lifecycle`, explicit `plughw:2,0` microphone and `plughw:CARD=Device,DEV=0` speaker, the 45-second stage deadline, and diagnostic routing/wake output. Require exactly `Ares -> goodbye Ares -> Ares -> shutdown Ares`, correlated worker request/result/stop/reap timestamps, confirmed child reap, released microphone ownership, and no Ctrl+C. Then run the foreground production sequence. Do not retune microphone gain, wake/VAD thresholds, Whisper, or lifecycle confidence for this worker-lifetime checkpoint.
...
- `core.AresIdentity` remains the bounded wake/general owner-name alias authority. `core.ActiveLifecycleAudioRecognizer` owns only the six lifecycle-slot forms on the finalized WAV. `core.LifecycleControl.normalize_active_lifecycle_command()` remains the exact post-Whisper lifecycle fallback. Neither component performs general substring normalization.
...
- Standby-wake and active-lifecycle tests cover separate wake/lifecycle Vosk grammars, canonical-WAV-only recognition, exact grammar and `[unk]`, minimum aligned confidence boundaries, action-specific high/medium tiers, confirmation/cancellation/expiry, same-WAV reuse, high-confidence Whisper bypass, ordinary/error Whisper fallback, fallback lifecycle non-authorization, one-session/one-acknowledgement activation, CoreService bypass, empty/no-speech/Whisper-failure/source-local-EOF nontermination, standby session clearing, reactivation, exactly-once explicit shutdown, terminal-reason diagnostics, calculator/owner-memory routing, self-wake exclusion, playback gating, output isolation, event/memory privacy, production composition, and bounded hardware verification.
...
- Pull `main` on Raspberry Pi and run the Phase 120 lifecycle verifier with the explicit microphone/speaker devices, 45-second stage deadline, and diagnostic routing/wake flags documented above. Require the exact `Ares -> goodbye Ares -> Ares -> shutdown Ares` sequence, every lifecycle child stopped and reaped before transition completion, goodbye preserving the process in standby, reactivation creating a new session, shutdown stopping exactly once, and no Ctrl+C. Then run production: wake, calculator result 4, goodbye to standby without exit, new-session reactivation, and exactly one shutdown.
...
- Do not add systemd/boot startup or City activation until the 9/10 reliability target and complete lifecycle sequence pass on real Raspberry Pi hardware. Daemonization, cloud speech, barge-in, hidden worker loops, automatic transcript memory writes, semantic/vector memory, autonomous fact extraction, embeddings, and cloud synchronization remain out of scope.
