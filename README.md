...
Wake VAD uses 20 ms frames and separates owner waiting from the bounded utterance. `WAITING_FOR_SPEECH` keeps the persistent microphone live for up to 5 seconds and retains 0.3 seconds of pre-roll; that wait does not consume the post-start safety budget. Two qualifying frames enter `RECORDING_SPEECH`. A frame below the continuation threshold enters `POSSIBLE_END_OF_SPEECH`, and 0.9 seconds of continuous calibrated quiet completes the candidate; one continuation-level frame resumes recording and records a terminal-silence reset. The listener retains 0.15 seconds of post-speech grace, accepts a 0.08-second minimum, and applies a 4-second maximum only from validated speech start. The maximum is therefore a failsafe rather than the normal endpoint. Excess leading/trailing silence is trimmed to bounded 0.24/0.20-second padding before Vosk, while PCM frame indices ensure pre-roll, speech, and post-roll are assembled once. Candidate history and buffered pipe audio are reset after each recognition so stale tails cannot leak into the next attempt. Ordinary rejected candidates do not close or recalibrate ALSA. The reliability helper resets only pre-prompt candidate history, prints `Ready for attempt N. Say 'Ares' now.`, leaves the existing PCM stream live for a 0.6-second readiness interval, and waits 0.5 seconds between completed attempts, including invalid-audio and other bounded infrastructure retries. Every completed attempt prints capture metrics before any infrastructure retry is excluded from the recognition denominator. Its normal cumulative result remains one stream open and one successful calibration. Wake diagnostics report literal qualifying-speech duration separately from the complete post-start speech window and preserve the first qualifying speech-frame index rather than substituting the later state-transition frame. A real activation deliberately closes standby capture before acknowledgement/active capture; returning after playback and the 0.35-second settling delay opens and calibrates one new standby epoch. Every open, close, calibration, and handoff carries an owner-visible reason and identifiers in diagnostics. The shared ownership gate prevents simultaneous standby and active capture. Full-command VAD and Whisper defaults remain unchanged. Candidate audio is removed by default; explicit retention keeps only the latest candidate and never implies playback. The process is foreground-only: systemd, boot startup, daemonization, barge-in, GPT, cloud fallback, and autonomous City activation remain unimplemented.
