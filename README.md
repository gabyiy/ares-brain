...
Active command Whisper now has its own validated hard timeout, independent of the launcher's broader 300-second process budget. The Raspberry Pi foreground default is 30 seconds (`--active-transcription-timeout 30`). `WhisperSubprocessRunner` drains stdout/stderr with `communicate(timeout=...)`; on timeout it terminates, waits for a bounded grace period, kills only if needed, reaps the process, and returns `transcription_timeout`. Ctrl+C also kills and reaps an active Whisper child before cancellation continues. Immediately after successful transcription, the raw text reaches `normalize_lifecycle_command()` before generic transcript cleanup. A standby or shutdown match bypasses ordinary routing; an ordinary result continues through transcript normalization and the established skill route. Empty text, no speech, timeout, transcription failure, active-command EOF, and a completed single-turn transport result remain nonterminal and cannot be converted into shutdown.
