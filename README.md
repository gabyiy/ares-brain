...
The latest production run passed wake, capture, and ordinary Whisper, printed `Processing command`, then required Ctrl+C instead of completing spoken standby. The traceback showed `ares-active-lifecycle-vosk` blocked in an unconditional child-side `connection.recv()`. The constrained lifecycle backend had returned its result but kept a daemon worker alive waiting for an unspecified future request, while active-session release cleared only controller policy state. The worker now serves exactly one versioned request, returns one correlated result, receives an explicit stop request, acknowledges it, and exits before its result is released to `BrainRuntime`. Parent receive and join operations are bounded; cleanup closes pipe endpoints, joins, terminates, kills if necessary, reaps the child, and fails closed if death cannot be confirmed. The child ignores foreground SIGINT so Ctrl+C remains a distinct parent-owned `owner_cancellation` path without a child traceback.
...
Each constrained request applies a 5-second deadline to child readiness, model loading, recognizer construction, PCM decoding, native Vosk work, and result waiting. IPC uses `ares.active_lifecycle_worker v1` messages with a unique request ID and explicit `ready`, `recognize_request`, `recognize_result`, `stop_request`, `stopped`, and `error` types. Every receive follows a bounded `poll`; every join is bounded and has terminate/kill fallback. Once a result is received, it is held until worker cleanup is confirmed. Standby uses `0.70` high and `0.50` medium confidence; shutdown uses `0.78` and `0.60`. A high exact result may execute directly. Medium evidence cannot execute and no longer creates the contradictory `action=none, whisper_fallback=no` state: it must complete the bounded Whisper fallback in the same turn. Rejected constrained evidence follows the same fallback. Failed, empty, negated, descriptive, or unmatched Whisper text remains ordinary and cannot reach `SHUTTING_DOWN` or `RETURNING_TO_STANDBY`.
...
The `lifecycle` mode is the bounded production-factory regression for this checkpoint. It requests `Ares`, `goodbye Ares`, `Ares`, and `Ares, shut down`; prints capture/Whisper evidence plus worker request, result, stop, join, and microphone-release timestamps; and fails if a lifecycle worker remains alive. It does not count Ctrl+C as spoken standby or shutdown success.
