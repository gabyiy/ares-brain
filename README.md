...
ARES v2.15 - Production-Composed Active PCM Diagnostic
...
The first Raspberry Pi run of `scripts/manual_diagnose_active_lifecycle_audio.py` failed before producing a candidate for every requested phrase. Repository tracing found two deterministic diagnostic defects. The script manually assembled its microphone and Whisper adapters instead of consuming the production active-audio factory, while its tests substituted a fake microphone whose `record_until_silence()` wrote a finished WAV directly and therefore never crossed real open/read/VAD/close boundaries. Separately, `RmsVoiceActivityCapture` reduced low-level PCM exceptions to `pcm_stream_error:<class>`, discarding the exact exception message needed to distinguish an ALSA child exit, stdout closure, stream-state error, or integrity guard. Consequently, the old `pcm_stream_error:RuntimeError` output cannot reveal the exact RuntimeError, ALSA stderr, or whether another process owned the microphone.

The repaired diagnostic shares one authoritative production active-audio factory with `run_ares_standby_voice.py` and follows production's bounded per-phrase architecture: acquire diagnostic microphone ownership, use the shared production adapter to create and start one fresh PCM stream for that phrase, read only after successful initialization, close the stream after the final read, and never reuse that closed stream. A bounded preflight reports the requested/resolved device, canonical PCM contract, ownership/gate state, open status, first complete 640-byte frame, nonzero status, process IDs, exact command, and stream health without advertising the first owner phrase until capture is ready. Diagnostic failures retain their typed stage, exception class and complete message, adapter/method and stream lifecycle context, ALSA stderr, cleanup result, and a full traceback only in explicit diagnostic output. Normal production logs do not print tracebacks. A phrase-level failure is isolated from later fresh stream captures.
...
`core.BrainRuntime` is the persistent foreground Capital/Core process controller built on that manager. It boots once to `STANDBY`, creates one active session after a verified bounded activation, serially routes multiple commands through the existing production `SkillManager -> IntentParser -> Planner -> ExecutionPipeline -> Skill` path, returns to standby on exact owner stop phrases or the manager's 30-second inactivity deadline, and terminates only for an explicit ACTIVE shutdown command, owner cancellation, or an unrecoverable failure. A completed single voice turn, active-command EOF, empty transcript, no-speech result, transcription failure, and unmatched lifecycle text are nonterminal. `BrainSessionManager` remains the sole lifecycle-state authority. In standby, the injected Linux listener opens one canonical ALSA PCM stream, calibrates once, keeps a bounded rolling pre-roll, and finalizes wake candidates without reopening the device after ordinary rejection. Local `VoskWakeRecognizer` receives activation vocabulary only and accepts complete configured `ares`/`aris`/`aries` wake phrases in exact name slots; standby/shutdown commands are not advertised or executed there. An exact phrase at confidence `0.55` or above activates immediately; confidence from `0.40` through less than `0.55` requires the same exact phrase twice within eight seconds; lower, wrong, `[unk]`, or extra-word results reject. Missing confidence is accepted only for one exact permitted activation phrase after the listener has validated a real VAD candidate and canonical WAV; unrestricted Whisper text never receives this exception. For active commands, the authoritative `normalize_active_lifecycle_command()` boundary receives the preserved raw Whisper transcript before `CoreService`, parser, planner, or skill routing. It returns `attention_only` for name-only input, removes at most one complete supported assistant address from an outer edge, and exact-matches bounded standby/shutdown command bodies. Whisper's acoustic `rs`, `r s`, and `are s` forms are bounded to those active address slots; ordinary internal text is not rewritten. Exact negation is checked before execution, so `do not shut down Ares`, `don't shutdown Ares`, `never`, and `should not` forms remain ordinary input. Active command capture remains on the existing base-English Whisper `SingleTurnVoicePipeline`; Piper and ALSA output are unchanged. No daemon, systemd unit, boot startup, City activation, GPT, cloud service, network listener, or lifecycle worker was added. The ALSA adapter owns one bounded transport pump thread solely to keep the already-owned raw stdout pipe drained; it cannot activate sessions, route commands, or change lifecycle state.
...
Before any further Raspberry Pi wake-reliability run, execute the persistent-PCM integrity probe. It records three quiet seconds and three spoken seconds through the known-good native `hw:2,0` path at 44.1 kHz, then repeats those phases through the exact production `plughw:2,0` 16 kHz persistent stream. It saves `direct_quiet.wav`, `direct_spoken.wav`, `persistent_quiet.wav`, `persistent_spoken.wav`, and one JSON report under `data/runtime/pcm_integrity/`. The report includes real headers, signed-sample distributions, RMS ratios, partial-read and queue accounting, exact commands, PID/exit state, and cross-path comparability. Playback is explicit, occurs only after capture releases ALSA, and requires the owner to confirm both spoken WAVs are audible:
...
Do not continue to wake reliability unless the probe reports structural PCM integrity, realistic persistent spoken amplitude, comparable spoken/quiet contrast, no EOF/partial-byte/duplicate-frame failure, and explicit audibility for both spoken WAVs. Deterministic tests prove the repaired framing, pump, ownership, and failure gates; only this owner-run Raspberry Pi probe can prove physical amplitude and audibility.
...
Only after the persistent-PCM probe passes and both comparison WAVs are audible, the bounded Raspberry Pi hardware helper may check silence and unrelated speech, score ten separately prompted wake classifications against a 9/10 target while keeping the runtime in `STANDBY`, and reuse one listener, ALSA stream, and calibration. This isolates recognizer reliability from intentional microphone handoffs. It can also run the bounded lifecycle-only sequence: activate, return to standby while the process stays alive, reactivate with a new session, then explicitly shut down. Each requested phrase gets at most three attempts, and active-command output includes the actual Whisper transcript and lifecycle classification:
...
Deterministic tests prove raw lifecycle interception, lifecycle-only acoustic aliasing, negation precedence, safe phrase normalization/rejection, ordinary-command fallthrough, CoreService bypass for controls, session clearing/reactivation, explicit shutdown, source-local EOF handling, and persistent-loop continuation. They do not prove what the Raspberry Pi Whisper binary will hear; the standby and shutdown verifier modes remain the bounded owner-run hardware proof.
...
Do not interpret a structurally valid silent preflight frame as speech, and do not evaluate goodbye/shutdown classification while a phrase still reports a zero-duration candidate. The first Raspberry Pi acceptance gate is nonzero PCM reaching VAD and a nonempty candidate WAV. Only after that gate passes and the retained WAV is audible should transcript and lifecycle recognition be evaluated.
...
`ACTIVE` uses a separate deterministic normalizer and never invokes wake activation. Name-only or bounded greeting input such as `Ares`, `Aris`, `RS`, `Hey Ares`, or `Hello Ares` returns typed `attention_only`, preserves the current session and inactivity deadline, bypasses `CoreService`, and is silent by default because `already_active_acknowledgement` is empty. For other active input, at most one whole-token assistant alias at the beginning or end is optional addressing; no substring replacement is used. After punctuation/space cleanup, `good bye`, `stand by`, and `shut down` are joined. The exact standby command roots are `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; exact shutdown roots are `shutdown`, `turn off`, `power down`, and `stop runtime`. Each root may be bare or addressed at one outer edge, including `goodbye Aris`, `Ares goodbye`, `shutdown RS`, and `Ares shut down`. Negation is checked before lifecycle execution, so `do not shut down Ares`, `don't go to sleep`, and longer explanatory or scheduled sentences remain ordinary input. Genuine ordinary owner commands reset the 30-second inactivity deadline; attention-only input, empty/no-usable speech, playback, and diagnostics do not.

The shared playback gate remains closed during acknowledgement and every response. Active capture obtains ownership only after playback finishes and the 0.35-second settling interval; because runtime polling is shorter than that interval, `ARES is waiting for your command...` is printed only after capture ownership is actually secured, once per genuine waiting phase. After acknowledgement, diagnostics report waiting, capture start, speech detection, command capture, transcription, and processing. `--diagnostic-routing` additionally prints raw and cleaned text, removed alias and its prefix/suffix/standalone position, lifecycle-normalized text, negation, classification/action, CoreService bypass, state/session before and after, pipeline status, and terminal reason. Wake and active-command transcripts remain separate owner-terminal diagnostics and are absent from events, memory, and persistent logs.
...
Exact configured wake phrases remain the primary classifier. A narrowly guarded second path may collapse exactly two identical normalized alias tokens from one bounded candidate: `ares ares`, `aris aris`, or `aries aries`. Both tokens must be the same surface alias, both must map to canonical `ares`, no `[unk]`, prefix, or unrelated token may exist, the candidate must fit the configured 4-second wake bound, and the existing confidence policy must pass using one Vosk confidence entry per token and the minimum token confidence. A malformed Vosk word entry that normalizes to multiple tokens fails closed. Diagnostics expose original and canonical tokens, minimum and mean confidence, whether collapse was used, and the true recognized surface alias (`aris` remains `aris` even though the accepted identity is canonical `ares`). Mixed aliases such as `ares aris`, three repetitions, `[unk] aris`, unrelated words, and over-duration candidates remain rejected. This is wake-only classification; global transcript normalization and active Whisper commands are unchanged.
