...
ARES v2.01 - Persistent Standby Wake Reliability
...
Wake VAD uses 20 ms frames, two-frame speech-start evidence, three-frame resume hysteresis, 0.4 seconds of pre-roll, 0.55 seconds of terminal silence, 0.12 seconds of retained post-roll, a 0.08-second minimum speech duration, and a 1.6-second active-utterance cap. In wake mode, every frame below the continuation threshold advances terminal silence; noise in the former silence/continue threshold gap can no longer erase endpoint progress without satisfying resume hysteresis. Excess leading/trailing silence is trimmed to bounded 0.24/0.20-second padding before Vosk, while PCM frame indices ensure pre-roll, speech, and post-roll are assembled once. Candidate history and buffered pipe audio are reset after each recognition so stale tails cannot leak into the next attempt. Ordinary rejected candidates do not close or recalibrate ALSA. The reliability phase deliberately classifies all ten prompts without activation, so its normal cumulative result is one stream open and one successful calibration. A real activation deliberately closes standby capture before acknowledgement/active capture; returning after playback and the 0.35-second settling delay opens and calibrates one new standby epoch. Every open, close, calibration, and handoff carries an owner-visible reason and identifiers in diagnostics. The shared ownership gate prevents simultaneous standby and active capture. Full-command VAD and Whisper defaults remain unchanged. Candidate audio is removed by default; explicit retention keeps only the latest candidate and never implies playback. The process is foreground-only: systemd, boot startup, daemonization, barge-in, GPT, cloud fallback, and autonomous City activation remain unimplemented.

Exact configured wake phrases remain the primary classifier. A narrowly guarded second path may collapse exactly two complete alias tokens, such as `ares ares` or `aries ares`, only when both tokens map to the same configured ARES identity, no `[unk]` or other token exists, the trimmed candidate is at most 1.4 seconds, and the existing confidence policy passes using the minimum token confidence. Diagnostics also expose minimum and mean confidence and state when this collapse was used. `[unk] aris`, longer repetitions, prefix duplication, unrelated words, and over-duration candidates remain rejected. This is wake-only classification; global transcript normalization and active Whisper commands are unchanged.
