...
Each constrained request applies a 5-second deadline to child readiness, first-use model loading, recognizer construction, PCM decoding, native Vosk work, and result waiting. Once a bounded child wait expires, cleanup terminates, kills if necessary, and requires the worker to be reaped; otherwise the recognizer fails closed. Standby uses `0.70` high and `0.50` medium confidence; shutdown uses `0.78` and `0.60`. A high exact result may execute directly. Medium evidence cannot execute and no longer creates the contradictory `action=none, whisper_fallback=no` state: it must complete the bounded Whisper fallback in the same turn. Rejected constrained evidence follows the same fallback. Failed, empty, negated, descriptive, or unmatched Whisper text remains ordinary and cannot reach `SHUTTING_DOWN` or `RETURNING_TO_STANDBY`.
