...
ARES v2.22 - Bounded Active Lifecycle Worker Transport
...
The latest production run passed wake, capture, and ordinary Whisper, printed `Processing command`, then required Ctrl+C instead of completing spoken standby. The Ctrl+C traceback proves that an old reusable `ares-active-lifecycle-vosk` child had survived and was blocked in its unconditional child-side `connection.recv()`. It does **not** by itself prove where the foreground parent was blocked, so the hardware hang is not attributed to that line without further Raspberry Pi evidence. The unsafe fact is narrower and sufficient to fix: a lifecycle turn could leave a reusable worker alive after its result.

Phase 120 removes that transport and lifetime entirely. Each constrained candidate gets one non-daemon, request-scoped child and one private socket pair. IPC is bounded, size-capped, length-prefixed JSON rather than `multiprocessing.Connection.send()`/`recv()` and pickle framing. The `ares.active_lifecycle_worker v1` protocol carries a correlated `ready`, `recognize_request`, `recognize_result`, `stop_request`, `stopped`, or `error` message. A result is withheld from `BrainRuntime` until the parent has completed the bounded stop/join/terminate/kill/reap sequence. Cleanup failures remain typed failures, propagate to the existing safe fallback/retry path, and cannot release a lifecycle result while a child remains live. The child ignores foreground SIGINT so Ctrl+C stays the parent's distinct `owner_cancellation` path and cannot masquerade as spoken standby or shutdown.
...
The ACTIVE path gives the already-finalized WAV to one side-effect-free `ActiveLifecycleAudioRecognizer` before ordinary Whisper. Its local `vosk_active_lifecycle_constrained_grammar` backend uses one isolated spawned worker for one request; the worker loads the configured Vosk model, processes one finalized candidate, completes the stop handshake, and is reaped before the foreground runtime can act on the result. One authoritative lifecycle-slot canonicalizer accepts only `ares`, `aris`, `aries`, `arris`, `rs`, or tokenized `r s`, and only where one complete supported lifecycle template has its assistant-name slot. It rewrites that slot to `ares` before exact classification while preserving the raw Vosk transcript and tokens for diagnostics and confidence alignment. The loaded grammar contains 42 standby expansions, 36 shutdown expansions, 49 bounded non-action competitors, and `[unk]`: 128 alternatives total. It does not perform substring, fuzzy, edit-distance, semantic, or general phonetic replacement. High-confidence exact lifecycle audio bypasses Whisper. Medium, rejected, unmatched, competitor, `[unk]`, missing-confidence, timeout, and backend-error results use ordinary Whisper on the same closed WAV; only a complete valid Whisper lifecycle phrase may then execute an action.

Each constrained request applies a 5-second deadline to child readiness, model loading, recognizer construction, PCM decoding, native Vosk work, result waiting, and shutdown handshaking. Every socket read/write and process join is deadline-bound; messages larger than 32 KiB or with invalid length/JSON/protocol/request identity fail closed. Join uses terminate and kill fallback, and death/reap must be confirmed before success is returned. Standby uses `0.70` high and `0.50` medium confidence; shutdown uses `0.78` and `0.60`. A high exact result may execute directly. Medium evidence cannot execute and no longer creates the contradictory `action=none, whisper_fallback=no` state: it must complete the bounded Whisper fallback in the same turn. Rejected constrained evidence follows the same fallback. Failed, empty, negated, descriptive, or unmatched Whisper text remains ordinary and cannot reach `SHUTTING_DOWN` or `RETURNING_TO_STANDBY`.
...
ACTIVE uses a separate `ActiveLifecycleAudioRecognizer` on the finalized command WAV before Whisper. The recognizer only reports evidence; Capital/Core owns confirmation and every transition. Production decision order is explicit: an accepted constrained result executes first; otherwise the same WAV reaches Whisper and an exact valid result from the existing ACTIVE lifecycle normalizer executes; only then does remaining text enter ordinary CoreService routing. Constrained rejection never vetoes a valid exact Whisper lifecycle result. Empty, failed, timed-out, negated, descriptive, or unmatched Whisper text still cannot request a transition. Piper, ALSA, ordinary calculator/memory/skill routing, and the bounded Whisper helper are unchanged. No daemon, systemd unit, boot startup, City activation, GPT, cloud service, network listener, or second lifecycle worker was added.
...
python scripts/manual_verify_standby_wake_hardware.py \
  ...
  --microphone-device plughw:2,0 \
  --speaker-device plughw:CARD=Device,DEV=0 \
  --hardware-stage-timeout-seconds 45 \
  --diagnostic-routing \
  --diagnostic-wake
```

The `lifecycle` mode is the bounded production-factory regression for this checkpoint. It requests exactly `Ares`, `goodbye Ares`, `Ares`, and `shutdown Ares`; applies an outer wall-clock deadline to every prompted stage; prints capture/Whisper evidence plus correlated worker request, result, stop, join/reap, transition, and microphone-release timestamps; and fails if a lifecycle worker remains alive. Ctrl+C is reported only as `owner_cancellation` and never counts as spoken standby or shutdown success.
...
`EventHistoryStore` is operational telemetry, not owner state. The production runtime injects one authoritative instance into `BrainRuntime`, `BrainSessionManager`, `CoreService`, its resource manager, `SkillManager`, and `SingleTurnVoicePipeline`. The lifecycle diagnostic now injects a nonpersistent sink instead of opening `data/event_history.json`. Append waits at most 0.05 seconds for its noncritical in-process lock, emits one bounded `WARNING: event history append skipped: ...`, increments the in-memory dropped-event count, and continues wake activation, acknowledgement, commands, standby, or shutdown. The file lock records PID, hostname, timestamp, owner kind, and token; automatic recovery requires an expired local lock whose PID is proven dead. Live, malformed, remote, and unprovable locks are preserved. Unexpected programming errors still surface, and owner-memory and other critical durable stores retain strict lock behavior.
...
The latest Raspberry Pi line `Whisper timeout: 15 seconds` was only a startup declaration, not proof that the deadline callback had run. The parent then delegated progress to one blocking wait call, and inner voice layers converted `KeyboardInterrupt` into typed local results, preventing the top-level cleanup owner from unwinding promptly. The repaired runner enforces its own monotonic deadline with short `poll()` intervals. SIGINT, SIGTERM, timeout, adapter cancellation, and failure all converge on the same process-group cleanup path. The previous event-history warning was independently explained by a durable diagnostic/production lock after abnormal termination; production does not construct duplicate history stores in one process.

Active command Whisper has a validated wall-clock boundary independent of the launcher's broader 300-second process budget. Raspberry Pi defaults are 15 seconds for inference, 1 second for graceful termination, and 3 seconds for complete cleanup. `WhisperSubprocessRunner` starts Whisper with `shell=False` in a new Linux session, records PID and PGID, writes stdout/stderr to private bounded temporary files, and polls against a monotonic parent deadline. Timeout sends group `SIGTERM`, waits a bounded grace, escalates to group `SIGKILL`, verifies exit, reaps the leader, and closes handles. The same shared bounded runner now protects one-shot `arecord`, Piper, and `aplay`; persistent `arecord` also owns a Linux process group. A timeout returns typed `transcription_timeout`, bypasses CoreService, deletes current-turn audio unless explicit retention is enabled, releases microphone ownership, and keeps the session ACTIVE for a retry.

The foreground launcher and the two-phrase lifecycle diagnostic install one idempotent SIGINT/SIGTERM coordinator. A first Ctrl+C requests adapter cancellation and unwinds to final cleanup; cleanup may safely run again. `KeyboardInterrupt` is not swallowed by `SingleTurnVoicePipeline` or the runtime voice adapters. Before phrase two, the diagnostic proves there is no live prior Whisper PID, no live prior arecord stream, an idle microphone/playback gate, and a unique temporary path. It has per-capture, per-Whisper, and total bounds and reports timestamped start, timeout, TERM/KILL, reap, ownership release, and file cleanup in diagnostic mode.

After an interrupted run, this bounded command identifies only ARES foreground Python processes and their allowlisted `arecord`, `whisper-cli`, Piper, and `aplay` descendants. It sends TERM, escalates after a bound, reports survivors, and recovers only validated expired locks whose local owner is dead:

```bash
python scripts/cleanup_ares_voice_processes.py --recover-stale-locks
```
...
That compare-only probe remains available for isolated capture/STT diagnosis. The current production-lifecycle gate is the bounded hardware verifier's `lifecycle` mode. It must complete wake, goodbye, reactivation, and shutdown without Ctrl+C, and each active turn must show a correlated lifecycle worker request/result/stop transaction with the child reaped before transition completion. Deterministic tests can prove protocol and cleanup branches, but not Raspberry Pi process behavior, transcript, or owner-accent recognition.
...
The existing exact ACTIVE Whisper normalizer remains a second deterministic lifecycle path. After constrained rejection, a valid complete Whisper lifecycle result—such as `Goodbye, Aris.` or `Shut down Aris.`—may request the corresponding transition before ordinary routing. That fallback does not reinterpret the constrained transcript and does not accept a Whisper failure, empty result, substring, negation, descriptive phrase, or unrelated longer sentence. `good bye`, `stand by`, and `shut down` retain their bounded text canonicalization.
...
`ACTIVE` never invokes wake activation. Name-only or bounded greeting input such as `Ares`, `Aris`, `RS`, `Hey Ares`, or `Hello Ares` returns typed `attention_only`, preserves the current session and inactivity deadline, bypasses `CoreService`, and is silent by default because `already_active_acknowledgement` is empty. The decision order is constrained lifecycle action, exact Whisper lifecycle fallback, then ordinary routing. Genuine ordinary owner commands reset the 30-second inactivity deadline; attention-only input, pending lifecycle evidence, empty/no-usable speech, playback, and diagnostics do not.
...
- The Phase 115 checkpoint passed 2,662 collected tests and its focused 1,082-test sweep. Phase 116's current totals supersede that historical checkpoint. Neither deterministic result proves the owner's constrained Vosk result on Raspberry Pi.
...
- A Raspberry Pi Ctrl+C traceback exposed an old reusable lifecycle worker waiting in `multiprocessing.Connection.recv()` after the foreground had reached `Processing command`. That evidence proves a leaked child lifetime, but it does not conclusively locate the foreground parent's hang.
- The first request-scoped repair added a correlated request/result/stop handshake and bounded process reap. Phase 120 supersedes its raw `Connection` transport.

Phase 120

- Every ACTIVE constrained candidate now owns one non-daemon worker and one private bounded socket transport. Messages are size-capped, length-prefixed JSON; raw `multiprocessing.Connection.send()`/`recv()` and pickle framing are absent from the production lifecycle protocol.
- The v1 request/result/stop protocol is correlated by request ID. The parent withholds recognition evidence until stop acknowledgement and bounded join/terminate/kill/reap complete. EOF, crash, malformed/oversized messages, timeout, cancellation, and cleanup failure propagate as typed failures; the ordinary same-WAV Whisper fallback/retry remains safe.
- The production-factory verifier runs exactly `Ares -> goodbye Ares -> Ares -> shutdown Ares`, gives each owner-prompt stage an outer deadline, correlates worker request/result/stop/reap evidence, verifies microphone release and zero surviving lifecycle workers, and keeps Ctrl+C exclusively classified as `owner_cancellation`.
- Phase 120 passes 747 focused lifecycle/runtime/audio/event tests and the complete 2,766-test suite. Compileall, Phase 2 event/memory verification, the deterministic lifecycle verifiers, configuration JSON validation, and `git diff --check` also pass. These are deterministic Windows results; spoken goodbye/shutdown and timing on Raspberry Pi remain owner-run hardware acceptance and are not claimed here.
