...
Current pytest collection: `2097 tests`.
...
Wake VAD uses 20 ms frames and separates owner waiting from the bounded utterance. `WAITING_FOR_SPEECH` keeps the persistent microphone live for up to 5 seconds and retains 0.3 seconds of pre-roll; that wait does not consume the post-start safety budget. Two qualifying frames enter `RECORDING_SPEECH`. A frame below the continuation threshold enters `POSSIBLE_END_OF_SPEECH`, and 0.9 seconds of continuous calibrated quiet completes the candidate; one continuation-level frame resumes recording and records a terminal-silence reset. The listener retains 0.15 seconds of post-speech grace, accepts a 0.08-second minimum, and applies a 4-second maximum only from validated speech start. The maximum is therefore a failsafe rather than the normal endpoint. Excess leading/trailing silence is trimmed to bounded 0.24/0.20-second padding before Vosk, while PCM frame indices ensure pre-roll, speech, and post-roll are assembled once. Candidate history and buffered pipe audio are reset after each recognition so stale tails cannot leak into the next attempt. Ordinary rejected candidates do not close or recalibrate ALSA. The reliability helper resets only pre-prompt candidate history, prints `Ready for attempt N. Say 'Ares' now.`, leaves the existing PCM stream live for a 0.6-second readiness interval, and waits 0.5 seconds between completed attempts. Its normal cumulative result remains one stream open and one successful calibration. A real activation deliberately closes standby capture before acknowledgement/active capture; returning after playback and the 0.35-second settling delay opens and calibrates one new standby epoch. Every open, close, calibration, and handoff carries an owner-visible reason and identifiers in diagnostics. The shared ownership gate prevents simultaneous standby and active capture. Full-command VAD and Whisper defaults remain unchanged. Candidate audio is removed by default; explicit retention keeps only the latest candidate and never implies playback. The process is foreground-only: systemd, boot startup, daemonization, barge-in, GPT, cloud fallback, and autonomous City activation remain unimplemented.

Exact configured wake phrases remain the primary classifier. A narrowly guarded second path may collapse exactly two identical normalized alias tokens from one bounded candidate: `ares ares`, `aris aris`, or `aries aries`. Both tokens must be the same surface alias, both must map to canonical `ares`, no `[unk]`, prefix, or unrelated token may exist, the candidate must fit the configured 4-second wake bound, and the existing confidence policy must pass using the minimum token confidence. Diagnostics expose original and canonical tokens, minimum and mean confidence, and whether collapse was used. Mixed aliases such as `ares aris`, three repetitions, `[unk] aris`, unrelated words, and over-duration candidates remain rejected. This is wake-only classification; global transcript normalization and active Whisper commands are unchanged.
...
- Current deterministic collection is 2097 tests. The quiet-room 9/10 target and complete production lifecycle still require owner-run Raspberry Pi microphone verification.
