...
ARES v2.18 - Lifecycle-Slot Constrained Alias Canonicalization
...
The repaired `scripts/manual_diagnose_active_lifecycle_audio.py` has passed its Raspberry Pi PCM gate. Production-composed preflight opened `plughw:2,0` as canonical 16 kHz mono `S16_LE`, returned one nonzero 640-byte frame, reported healthy one-shot ownership, and all four requested phrases produced nonempty candidates. The latest real run also proved that ordinary Whisper correctly returned `Goodbye, Aris.` and `Shut down Aris.`, and the normal ACTIVE lifecycle parser correctly resolved those complete transcripts to standby and shutdown. The constrained Vosk recognizer independently emitted `goodbye rs` and `shutdown rs`; the former missed the old action table and the latter occupied an explicit rejection slot, so exact constrained matching rejected both. The remaining defect was therefore a mismatch between two local recognition policies, not PCM transport, capture timing, ordinary Whisper, or the normal lifecycle parser.

The ACTIVE path gives the already-finalized WAV to one side-effect-free `ActiveLifecycleAudioRecognizer` before ordinary Whisper. Its local `vosk_active_lifecycle_constrained_grammar` backend keeps one spawned worker process and loads the configured Vosk model once per worker. One authoritative lifecycle-slot canonicalizer accepts only `ares`, `aris`, `aries`, `arris`, `rs`, or tokenized `r s`, and only where one complete supported lifecycle template has its assistant-name slot. It rewrites that slot to `ares` before exact classification while preserving the raw Vosk transcript and tokens for diagnostics and confidence alignment. The loaded grammar contains 42 standby expansions, 36 shutdown expansions, 49 bounded non-action competitors, and `[unk]`: 128 alternatives total. It does not perform substring, fuzzy, edit-distance, semantic, or general phonetic replacement. High-confidence exact lifecycle audio bypasses Whisper; medium-confidence exact audio creates a session-bound confirmation request; low-confidence, unmatched, competitor, `[unk]`, missing-confidence, timeout, or backend-error results run the unchanged Whisper path on the same `AudioChunk` and closed WAV.
...
- Pytest automated coverage for 2688 tests across current core modules
...
Current pytest collection: `2688 tests`; all 2688 pass. The definitive post-final-code focused lifecycle/production/diagnostic sweep passes all 539 tests.
...
The owner has passed the structural/audio gate on Raspberry Pi and has supplied the exact mismatch that this revision repairs: Whisper returned valid lifecycle phrases while Vosk returned the bounded `rs` form. The next hardware gate must show raw constrained `goodbye rs` or another permitted slot alias becoming `goodbye ares`, and raw constrained `shutdown rs` or another permitted slot alias becoming `shutdown ares`, without fallback. Calculator and memory must remain ordinary and use Whisper. Compare-only Whisper output is still printed, and diagnostic mode never executes the shown action. A passing run ends with `all four phrases passed the constrained lifecycle policy`. Deterministic tests and WAV headers cannot establish the owner's post-fix Vosk output or authorize a production lifecycle-success claim.
