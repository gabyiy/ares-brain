...
`core.BrainRuntime` is the persistent foreground Capital/Core process controller built on that manager. It boots once to `STANDBY`, creates one active session after a verified bounded activation, serially routes multiple commands through the existing production `SkillManager -> IntentParser -> Planner -> ExecutionPipeline -> Skill` path, returns to standby on exact owner stop phrases or the manager's 30-second inactivity deadline, and terminates only for an explicit shutdown command, owner cancellation, or an unrecoverable failure. A completed single voice turn, active-command EOF, empty transcript, no-speech result, transcription failure, and unmatched lifecycle text are nonterminal. `BrainSessionManager` remains the sole lifecycle-state authority. In standby, the injected Linux listener opens one canonical ALSA PCM stream, calibrates once, keeps a bounded rolling pre-roll, and finalizes wake candidates without reopening the device after ordinary rejection. Local `VoskWakeRecognizer` accepts only complete configured `ares`/`aris`/`aries` phrases in exact name slots. An exact phrase at confidence `0.55` or above activates immediately; confidence from `0.40` through less than `0.55` requires the same exact phrase twice within eight seconds; lower, wrong, `[unk]`, or extra-word results reject. Missing confidence is accepted only for one exact permitted activation phrase after the listener has validated a real VAD candidate and canonical WAV; unrestricted Whisper text never receives this exception. For active commands, the authoritative `normalize_lifecycle_command()` boundary receives the preserved raw Whisper transcript before generic `TranscriptNormalization`, `CoreService`, parser, planner, or skill routing. Strict whole-phrase parsing accepts `ares`, `aris`, and `aries` in exact lifecycle name slots and additionally treats Whisper's bounded acoustic forms `rs`, `r s`, and `are s` as ARES only while matching a complete standby or shutdown control. It supports name-last and name-first forms such as `shut down RS`, `Ares shut down`, `goodbye RS`, and `RS goodbye`. Those acoustic forms are not wake aliases and ordinary text containing `rs` is not rewritten. Exact negation is checked first, so `do not shut down Ares`, `don't shutdown Ares`, `never`, and `should not` forms remain ordinary input. Active command capture remains on the existing base-English Whisper `SingleTurnVoicePipeline`; Piper and ALSA output are unchanged. No daemon, systemd unit, boot startup, City activation, GPT, cloud service, network listener, or lifecycle worker was added. The ALSA adapter owns one bounded transport pump thread solely to keep the already-owned raw stdout pipe drained; it cannot activate sessions, route commands, or change lifecycle state.
...
`core.LifecycleControl.normalize_lifecycle_command()` is the one deterministic lifecycle normalizer used by the runtime and its tests. It preserves the raw transcript, trims ordinary terminal punctuation, folds case and repeated whitespace, canonicalizes `good bye` to `goodbye` and `shut down` to `shutdown`, and accepts `ares`, `aris`, or `aries` only in the exact final name slot of a complete supported phrase. There is no substring, fuzzy, edit-distance, or semantic matching. The configured standby templates are `goodbye ares`, `go to standby ares`, `go to sleep ares`, `standby ares`, and `sleep ares`; each accepts the three exact name aliases, and `good bye` is the safe surface variant of `goodbye`. Full shutdown accepts only `shutdown {ares|aris|aries}` or its `shut down` surface form. `stop ares completely`, a missing name, negated controls, and lifecycle words embedded in longer sentences do not terminate the production runtime.
...
Before another lifecycle acceptance run, use the bounded active-transcription probe once:
...
It uses `plughw:2,0`, `plughw:CARD=Device,DEV=0`, `vosk-model-small-en-us-0.15` for bounded standby wake recognition, `ggml-base.en.bin` for full active commands, and `en_US-hfc_male-medium` by default. Say `Ares` once for one `Yes Gabi.` acknowledgement. The constrained grammar is generated from exact aliases `ares`, `aris`, and `aries` with the empty, `hey`, `hello`, `okay`, and `wake up` prefixes, plus exact lifecycle controls and `[unk]`. `Aries` is a name only in the exact final alias slot of one configured complete phrase. Confidence at or above `0.55` accepts an exact wake immediately; `0.40` through less than `0.55` requires two identical exact recognitions within eight seconds. The hardware verifier now prints `Low-confidence wake detected. Say Ares once more.` and consumes that immediate second candidate before scoring the prompted attempt. Below `0.40`, wrong, or unknown-token results reject. Missing confidence is accepted only for a single exact permitted activation phrase whose VAD and canonical audio checks succeeded; missing-confidence controls and malformed candidates reject. `okay`, `bye`, `alrighty`, `areas`, `air`, partial words, zodiac questions, and sentences that merely mention Ares remain rejected. Issue multiple commands under one session. Exact standby controls include `goodbye <name>`, `<name> goodbye`, `standby <name>`, `<name> standby`, `go to standby <name>`, `sleep <name>`, and `go to sleep <name>`. Exact shutdown controls include `shutdown <name>`, `shut down <name>`, `<name> shutdown`, and `<name> shut down`. In lifecycle name slots, `<name>` accepts `Ares`, `Aris`, `Aries`, or the active-Whisper acoustic form `RS`; split `R S` and `Are S` are normalized only there. After acknowledgement, the terminal reports waiting, capture start, speech detection, command capture, transcription, and processing stages. A bounded no-speech result prints `No command heard; still active`; manager-owned inactivity still returns the session to standby at 30 seconds. `--diagnostic-routing` prints raw, cleaned, normalized, and canonicalized Whisper text; matched alias and alias type; canonical name; negation; matched complete phrase; classification and action; CoreService bypass; before/after state and session; pipeline status; runtime-terminal status; and terminal reason. Before it exits, the launcher prints the explicit outer terminal reason, such as `explicit_shutdown_command`, `owner_cancellation`, or `unrecoverable_failure`. Wake and active-command transcripts remain separate owner-terminal diagnostics and are absent from events, memory, and persistent logs.
