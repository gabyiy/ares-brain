...
The repaired `scripts/manual_diagnose_active_lifecycle_audio.py` has now passed its Raspberry Pi PCM gate. Production-composed preflight opened `plughw:2,0` as canonical 16 kHz mono `S16_LE`, returned one nonzero 640-byte frame, reported healthy one-shot ownership, and all four requested phrases produced nonempty candidates. Whisper correctly returned `Calculate 2 plus 2.` from a 2.640-second candidate and `Remember that I like video games!` from a 3.200-second candidate. The same owner and capture path produced `*clears throat*` for spoken `goodbye Ares` and `Shut down, artist.` for spoken `shutdown Ares`. This is direct hardware evidence that the remaining failure is short closed-set acoustic recognition, not PCM transport, capture timing, lifecycle parsing of a correct transcript, or ordinary open-ended Whisper quality.

The ACTIVE path now gives the already-finalized WAV to one side-effect-free `ActiveLifecycleAudioRecognizer` before ordinary Whisper. Its local `vosk_active_lifecycle_constrained_grammar` backend keeps one spawned worker process and loads the configured Vosk model once per worker. The worker advertises 21 bounded owner-validated action phrases, 84 bounded non-action acoustic/negative/descriptive competitors, and `[unk]`. Competitors include the observed `artist`, `aries`, `RS`, and `clears throat` forms, 13 bounded no-name negative/descriptive phrases, and a finite `do not`/`don't`/`never` × lifecycle-root × `ares`/`aris` negation matrix; they can only produce an ordinary rejection and are not aliases. The result includes classification, canonical phrase, recognized text/tokens, minimum aligned word confidence, confidence tier, backend, confirmation requirement, and rejection reason. The component never opens a microphone, captures a second candidate, executes a transition, invokes CoreService, or writes memory. High-confidence exact lifecycle audio bypasses Whisper and is transported to Capital/Core; medium-confidence exact audio creates a session-bound confirmation request; low-confidence, unmatched, competitor, `[unk]`, missing-confidence, timeout, or backend-error results run the unchanged Whisper path on the same `AudioChunk` and closed WAV.

Each constrained request applies a 5-second deadline to child readiness, first-use model loading, recognizer construction, PCM decoding, native Vosk work, and result waiting. Python's parent-side `Process.start()` and pipe `send()` calls are synchronous setup and are not independently preempted by that deadline. Once a bounded child wait expires, cleanup terminates, kills if necessary, and requires the worker to be reaped; otherwise the recognizer fails closed and will not start a competing worker. A later ACTIVE turn may start one fresh worker after confirmed cleanup, and ordinary Whisper remains available. Standby uses `0.70` high and `0.50` medium confidence. Shutdown is deliberately stricter at `0.78` high and `0.60` medium. Configuration is rejected below safety floors of standby `0.60`/`0.40`, shutdown `0.70`/`0.50`, and positive confirmation `0.75`. A medium result cannot execute: ARES asks `Did you say goodbye Ares?` or `Did you say shutdown Ares?`, then accepts only a bounded confirmation at confidence `0.80` or a cancellation at confidence `0.50`. Pending confirmation is tied to the active session, expires after one next owner turn or 10 seconds, and is cleared by cancellation, lifecycle/session change, or expiry. A weak shutdown guess never reaches `SHUTTING_DOWN`.
...
`core.BrainSessionManager` is the central Capital/Core lifecycle controller. It owns the deterministic Brain states `STOPPED`, `BOOTING`, `INITIALIZING`, `STANDBY`, `ACTIVE`, `PROCESSING`, `RESPONDING`, `RETURNING_TO_STANDBY`, `SHUTTING_DOWN`, and `ERROR`; validates every transition; creates one session ID on activation; clears it on standby; tracks activity, inactivity deadlines, and consecutive failures; and emits bounded lifecycle events. `CoreService.get_brain_session_snapshot()` inspects it without activating a City. This is separate from `ModuleLifecycleManager`, which still controls removable modules. The manager itself contains no microphone, Vosk, Whisper, Piper, memory-store, skill, City activation, GPT, or cloud behavior.

`core.BrainRuntime` is the persistent foreground Capital/Core process controller built on that manager. It boots once to `STANDBY`, creates one active session after a verified bounded activation, serially routes multiple commands through the existing production `SkillManager -> IntentParser -> Planner -> ExecutionPipeline -> Skill` path, returns to standby on exact owner stop phrases or the manager's 30-second inactivity deadline, and terminates only for an explicit ACTIVE shutdown command, owner cancellation, or an unrecoverable failure. A completed single voice turn, active-command EOF, empty transcript, no-speech result, transcription failure, and unmatched lifecycle audio are nonterminal. `BrainSessionManager` remains the sole lifecycle-state authority. In standby, the injected Linux listener opens one canonical ALSA PCM stream, calibrates once, keeps a bounded rolling pre-roll, and finalizes wake candidates without reopening the device after ordinary rejection. Local `VoskWakeRecognizer` receives activation vocabulary only and accepts complete configured `ares`/`aris`/`aries` wake phrases in exact name slots; standby/shutdown commands are not advertised or executed there. An exact phrase at confidence `0.55` or above activates immediately; confidence from `0.40` through less than `0.55` requires the same exact phrase twice within eight seconds; lower, wrong, `[unk]`, or extra-word results reject. Missing confidence is accepted only for one exact permitted activation phrase after the listener has validated a real VAD candidate and canonical WAV.

ACTIVE uses a separate `ActiveLifecycleAudioRecognizer` on the finalized command WAV before Whisper. Only the explicit audio grammar documented below can authorize standby or shutdown, and the recognizer only reports evidence; Capital/Core owns confirmation and every transition. High-confidence exact audio bypasses Whisper, medium confidence requires bounded confirmation, and every ordinary/low/unmatched result falls through to the unchanged base-English Whisper path. The deterministic transcript normalizer remains useful for typed tests, attention-only handling, diagnostics, and conservative post-transcription command shaping, but a Whisper acoustic alias cannot bypass the audio recognizer's shutdown confidence policy. Piper, ALSA, ordinary calculator/memory/skill routing, and the bounded Whisper helper are unchanged. No daemon, systemd unit, boot startup, City activation, GPT, cloud service, network listener, or lifecycle worker was added. The ALSA adapter owns one bounded transport pump thread solely to keep the already-owned raw stdout pipe drained; it cannot activate sessions, route commands, or change lifecycle state.
...
Central owner memory is a Brain/CoreService capability rather than a voice-owned store. `CoreService` owns `memory.OwnerMemoryService`; `OwnerMemorySkill` sends versioned requests through `CoreService.execute_owner_memory()`, and only the service knows the concrete `OwnerProfileStore`. Voice, typed text, and future interfaces therefore share the same canonical `data/memory/owner_profile.json`. The `ares.owner_profile` v3 schema preserves keyed facts and adds bounded structured general memories for explicit owner requests, deterministic lexical retrieval, duplicate control, confirmation-gated CRUD, migration of every existing v1/v2 fact, one retained last-known-good backup, transaction/write locks, and atomic validated replacement. Voice launchers do not import or instantiate either JSON store, and no transcript, recording, complete conversation, or inferred fact becomes owner memory automatically.
...
The current real audio surface includes explicit Linux ALSA capture, calibrated foreground standby VAD, constrained local Vosk wake recognition, offline Whisper command transcription, offline Piper WAV generation, explicit ALSA playback, one controlled single turn, one bounded owner-triggered multi-turn session, and the owner-started foreground standby runtime. It is not a daemon or boot service; GPT, cloud services, unbounded background capture, and autonomous City activation remain absent.
...
Permanent owner-memory rule: owner facts belong to the central Brain/CoreService memory boundary. Input/output adapters may submit text and render responses, but they may not own, duplicate, cache as authority, or directly mutate persistent owner memory. Facts are persisted only after explicit bounded owner requests.
...
- Pytest automated coverage for 2662 tests across current core modules
...
Current pytest collection: `2662 tests`; all 2662 pass.
...
Type `Ares` to activate, `goodbye Ares` to return to standby, and `shutdown Ares` to stop the process. This remains the stable hardware-free text mode.

`core.LifecycleControl.normalize_active_lifecycle_command()` is the authoritative ACTIVE normalizer used by production and tests; standby wake classification remains a separate exact boundary. It preserves raw routed command content, folds case/punctuation only for classification, canonicalizes `good bye`, `stand by`, and `shut down`, and removes at most one complete supported assistant address from the command prefix or suffix. Exact standby roots are `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; exact shutdown roots are `shutdown`, `turn off`, `power down`, and `stop runtime`. Each may be bare or addressed. There is no substring, fuzzy, edit-distance, or semantic matching. Negated controls and lifecycle words embedded in longer sentences remain ordinary input.

Manual Standby Wake Runtime Verification

The deterministic wake verifier injects candidate audio, constrained Vosk wake results, active-command Whisper results, Piper output, and speaker behavior while using the real `BrainRuntime`, `BrainSessionManager`, `SingleTurnVoicePipeline`, CoreService, calculator, and owner-memory routes:

```powershell
py scripts\manual_verify_standby_wake_runtime.py
```

Install the Python dependency and the recommended small English Vosk model. ARES never downloads a speech model at runtime:

The model is the Raspberry Pi-suitable small English entry from the [official Vosk model catalog](https://alphacephei.com/vosk/models); package installation is described by the [official Vosk installation guide](https://alphacephei.com/vosk/install).

```bash
python -m pip install -r requirements.txt
mkdir -p models/vosk
curl -fL https://alphacephei.com/vosk/models/vosk-model-small-en-us-0.15.zip \
...
Run the focused automated wake checks:

```bash
python -m pytest tests/test_wake_vad_profile.py \
  tests/test_standby_wake_vad_sensitivity.py \
  tests/test_vosk_wake_recognizer.py \
  tests/test_standby_wake_listener.py \
  tests/test_brain_runtime_wake.py \
  tests/test_run_ares_standby_voice.py \
  tests/test_linux_alsa_voice_activity_capture.py \
  tests/test_store_lock_safety.py \
  tests/test_event_history_store.py
```

The persistent-PCM integrity probe remains available for transport regression diagnosis. It records three quiet seconds and three spoken seconds through the known-good native `hw:2,0` path at 44.1 kHz, then repeats those phases through the exact production `plughw:2,0` 16 kHz persistent stream. It saves `direct_quiet.wav`, `direct_spoken.wav`, `persistent_quiet.wav`, `persistent_spoken.wav`, and one JSON report under `data/runtime/pcm_integrity/`. The owner has already passed this transport/candidate checkpoint for the current lifecycle issue, so it is not the next command unless PCM regresses.
...
On any future transport regression, require structural PCM integrity, realistic persistent spoken amplitude, comparable spoken/quiet contrast, no EOF/partial-byte/duplicate-frame failure, and explicit audibility for both spoken WAVs before changing VAD or recognition.
...
The bounded wake helper remains available to check silence/unrelated speech and score ten separately prompted wake classifications while keeping the runtime in `STANDBY`. It is not the current ACTIVE lifecycle checkpoint. The current owner gate is the production-composed four-phrase constrained-audio diagnostic documented below.
...
Deterministic tests prove finalized-WAV interception, exact constrained grammar, confidence tiers, false-positive rejection, confirmation safety, same-WAV Whisper fallback, CoreService bypass, session clearing/reactivation, explicit shutdown, source-local EOF handling, and persistent-loop continuation. They do not prove what constrained Vosk will hear from the owner's Raspberry Pi audio.
...
`EventHistoryStore` is operational telemetry, not owner state. The production runtime injects one authoritative instance into `BrainRuntime`, `BrainSessionManager`, `CoreService`, its resource manager, `SkillManager`, and `SingleTurnVoicePipeline`. Its former fail-open disk-lock policy still entered an unbounded in-process `RLock`, so another thread holding that lock could stop voice progress before the disk-lock policy ran. Append now waits at most 0.05 seconds for that noncritical lock, emits one bounded `WARNING: event history append skipped: ...`, increments the in-memory dropped-event count, and continues wake activation, acknowledgement, commands, standby, or shutdown. Valid locks owned by live processes are never removed. Unexpected programming errors still surface, and owner-memory and other critical durable stores retain strict lock behavior.
...
The Raspberry Pi line `Transcribing command` proved that capture completed but, by itself, could not distinguish a blocked Whisper wait, child cleanup, or event-history append. Code tracing found two concrete defects: the old Whisper launcher did not create a Linux process group and terminated only the leader, and its final bounded `communicate()` fallback could return with `reaped=False` while inherited stdout/stderr pipes or descendants remained live. Separately, the event-history in-process lock described above had no acquisition deadline. These are evidence-backed defects; the old Pi log alone does not prove which one dominated that hardware turn.

Active command Whisper now has its own validated wall-clock boundary, independent of the launcher's broader 300-second process budget. The Raspberry Pi foreground defaults are 15 seconds for inference, 1 second for graceful termination, and 3 seconds for the complete cleanup deadline. `WhisperSubprocessRunner` starts Whisper in a new Linux session, records PID and process-group ID, sends `SIGTERM` to the whole group on timeout, escalates to group `SIGKILL`, reaps the leader, and closes all handles before reporting cleanup complete. stdout and stderr use separate bounded temporary files, so neither pipe can fill and deadlock the parent. Ctrl+C and pipeline shutdown reach the same cancellable helper. A timeout returns typed `transcription_timeout` metadata, bypasses `CoreService`, deletes current-turn audio unless diagnostic retention is enabled, releases microphone ownership, and leaves `ACTIVE -> ACTIVE` so the owner can retry. It can never become standby or shutdown.

Deterministic tracing found several defects around the owner-observed lifecycle failure. `good bye` had no compound-word normalization, lifecycle classification was split and downstream of generic transcript cleanup, and the launcher suppressed the loop's stop reason behind the generic `ARES standby voice runtime stopped cleanly.` message. Source-local EOF handling and a negative-list loop-success rule also left false-terminal and false-clean paths. The trace disproved normal `SingleTurnVoicePipeline` completion itself as an outer-runtime terminal collision. Because the old Raspberry Pi output omitted the transcript, classification, and stop reason, this checkpoint does not claim which one of those terminal paths occurred on that specific hardware turn; the new diagnostics make the next result explicit.
...
The bounded active-transcription probe remains available for process diagnosis, but the owner has now proved production Whisper completes in approximately two seconds with exit code zero, so it is no longer a prerequisite for the routing acceptance run:

```bash
python scripts/manual_diagnose_active_transcription.py \
  --microphone-device plughw:2,0 \
  --speaker-device plughw:CARD=Device,DEV=0 \
...
The focused ACTIVE lifecycle-audio probe receives the same production active-audio composition and constrained lifecycle recognizer as the foreground launcher instead of manually rebuilding nearly identical adapters. It first rejects a live runtime/microphone-owner conflict without stealing a valid lock, then performs a bounded structural PCM preflight. Production ACTIVE capture is bounded per turn, so the diagnostic likewise creates one fresh initialized stream per phrase and closes it after that phrase; closed adapters and streams are never reused. It prompts only after readiness for `goodbye Ares`, `shutdown Ares`, `calculate two plus two`, and `remember that I like video games`.

The preflight prints the requested and resolved device, `16000 Hz / mono / S16_LE` format, ownership, open/read state, expected and actual first-frame bytes, nonzero status, process identity, exact `arecord` command, stderr, cleanup, and stream health. Phrase output separates microphone-open, PCM-read, invalid-frame, no-speech, VAD, WAV-write, constrained-recognition, Whisper, empty-transcript, and lifecycle-parse failures. For each nonempty candidate it prints the comparison Whisper transcript, status and reason; constrained text/token list, confidence, backend, classification, canonical phrase, selected action, rejection reason; and whether production would run Whisper fallback. The comparison status and reason remain visible even when an accepted high-confidence lifecycle result makes comparison Whisper nonfatal. The diagnostic's compare-only finalized-audio decision allows Whisper to inspect the same closed WAV even when production would bypass it; it never recaptures. It still constructs no `BrainRuntime`, invokes no CoreService skill or owner memory, and executes no lifecycle action. Audio is deleted unless `--retain-audio` is explicitly supplied:
...
The owner has passed the structural/audio gate on Raspberry Pi: preflight returned a healthy nonzero 640-byte frame, all four captures were nonempty, and ordinary commands survived through Whisper. The next hardware gate is specifically the constrained results for the retained `goodbye Ares` and `shutdown Ares` WAVs. The diagnostic exits nonzero unless goodbye is a high-confidence `standby` result canonicalized to `goodbye ares` with no production fallback, shutdown is a high-confidence `shutdown` result canonicalized to `shutdown ares` with no production fallback, and calculator plus memory are `ordinary` results that request and successfully complete Whisper fallback. Compare-only Whisper output is still printed for lifecycle phrases, but a comparison timeout, blank result, or error cannot invalidate lifecycle evidence that production would already have accepted without Whisper. A passing run ends with `all four phrases passed the constrained lifecycle policy`. Deterministic tests and WAV headers cannot establish the owner's acoustic confidence or authorize a production lifecycle-success claim.

The constrained standby grammar is exactly: `goodbye ares`, `goodbye aris`, `good bye ares`, `good bye aris`, `bye ares`, `bye aris`, `go standby ares`, `standby ares`, and `sleep ares`. The constrained shutdown grammar is exactly: `shutdown ares`, `shutdown aris`, `shut down ares`, `shut down aris`, `ares shutdown`, `aris shutdown`, `ares shut down`, `aris shut down`, `turn off ares`, `turn off aris`, `power off ares`, and `power off aris`. `artist`, `paris`, `harris`, `aries`, assistant-name-only input, `shutdown computer`, `goodbye everyone`, `go to sleep`, and `turn it off` are not audio lifecycle aliases. There is no fuzzy, substring, edit-distance, semantic, or learned-alias fallback.

For typed/simulated compatibility only, the broader exact ACTIVE standby roots remain `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; shutdown roots are `shutdown`, `turn off`, `power down`, and `stop runtime`. That text normalizer cannot authorize real ACTIVE audio. Real audio uses only the narrower named constrained grammar in the preceding paragraph. `good bye`, `stand by`, and `shut down` are canonicalized for typed/simulated input; negated, descriptive, scheduled, embedded, or otherwise longer phrases do not execute lifecycle actions.
...
Owner-local wake and routing diagnostics are separately opt-in:

```bash
python scripts/run_ares_standby_voice.py --diagnostic-wake
python scripts/run_ares_standby_voice.py --diagnostic-routing
python scripts/run_ares_standby_voice.py --diagnostic-wake --retain-diagnostic-audio
```

The final form retains at most the configured number of wake candidates (one by default) and prints a manual `aplay` command. None of these forms automatically plays captured microphone audio.

It uses `plughw:2,0`, `plughw:CARD=Device,DEV=0`, `vosk-model-small-en-us-0.15` for bounded standby wake recognition, `ggml-base.en.bin` for full active commands, and `en_US-hfc_male-medium` by default. Wake classification runs only in `STANDBY`: one exact supported wake creates one session and one `Yes Gabi.` acknowledgement. The constrained wake grammar uses exact `ares`, `aris`, and `aries` name slots with bounded prefixes. Confidence at or above `0.55` accepts immediately; `0.40` through less than `0.55` requires two identical exact recognitions within eight seconds. Lower-confidence, wrong, unknown-token, malformed, partial, zodiac, and surrounding-sentence candidates reject.

`ACTIVE` never invokes wake activation. Name-only or bounded greeting input such as `Ares`, `Aris`, `RS`, `Hey Ares`, or `Hello Ares` returns typed `attention_only`, preserves the current session and inactivity deadline, bypasses `CoreService`, and is silent by default because `already_active_acknowledgement` is empty. Real audio standby/shutdown authorization uses only the exact constrained grammar and confidence policy above. The older deterministic text normalizer remains for typed/simulated compatibility and diagnostics; its broader punctuation/segmentation/address handling cannot authorize a fallback Whisper lifecycle action. Genuine ordinary owner commands reset the 30-second inactivity deadline; attention-only input, pending lifecycle evidence, empty/no-usable speech, playback, and diagnostics do not.

The shared playback gate remains closed during acknowledgement and every response. Active capture obtains ownership only after playback finishes and the 0.35-second settling interval; because runtime polling is shorter than that interval, `ARES is waiting for your command...` is printed only after capture ownership is actually secured, once per genuine waiting phase. After acknowledgement, diagnostics report waiting, capture start, speech detection, command capture, constrained recognition, optional Whisper transcription, and processing. Diagnostic routing additionally prints constrained text/tokens/confidence/tier/backend/classification/canonical/rejection, confirmation status, selected action, whether Whisper fallback ran, optional raw/cleaned Whisper text, CoreService bypass, state/session before and after, pipeline status, and terminal reason. Wake and active-command transcripts remain separate owner-terminal diagnostics and are absent from events, memory, and persistent logs.

Standby does not run Whisper. It opens one `arecord` stream for the whole standby epoch and performs one bounded three-second RMS calibration. Calibration no longer demands an uninterrupted sequence of frames below the pre-calibration RMS 200 gate: real USB hiss repeatedly violated that rule and prevented startup. The guarded bootstrap now collects the complete bounded sample, uses the median of the quietest 25 percent as its noise estimate, excludes high-energy outliers, and quality-checks valid-frame count, quiet-sample coverage, speech dominance, clipping, all-zero PCM, and a maximum usable noise floor. One likely speech-contaminated sample gets one bounded retry after candidate/history reset; zero, clipped, malformed, missing, or persistently unusable input fails closed. Successful thresholds still use bounded multiplier-plus-absolute-margin hysteresis and are reused for up to 300 seconds unless manual recalibration or device recovery is requested. Fresh calibration remains authoritative; no cached calibration profile is used in this checkpoint.

The next real Raspberry Pi run proved that the persistent stream and calibration were healthy (`stream opens=1`, `calibrations=1`) but every wake poll ended with zero promoted speech frames. The calibrated ambient level was roughly 389-430 RMS while the generic speech-start gate reached about 658.5 RMS. Post-calibration RMS samples were still arriving, so this was not another stream-owner or exhausted-reader failure; the wake-only start gate was too high for the short utterance. Standby wake calibration now selects one validated sensitivity policy: `conservative`, `normal` (default), or `sensitive`. Each derives start, continuation, and silence gates from the greater of the robust floor and measured median using bounded multipliers plus absolute margins. Every policy keeps `start > continue > silence > ambient`, and two consecutive start-level frames are still required. `--wake-vad-sensitive` selects the bounded sensitive policy for hardware diagnosis. It does not change Vosk grammar, confidence, aliases, active-command VAD, or Whisper.

The later near-zero Raspberry Pi result exposed a different fault below those thresholds. Direct native `arecord` continuously wrote a healthy 44.1 kHz WAV, but the supposedly persistent raw subprocess was pull-driven: stdout was consumed only while calibration or VAD called `read_frame`. During prompts, recognition, and other gaps the pipe accumulated quiet PCM and could block `arecord`; the next poll could then exhaust its wait budget on stale RMS 3-7 frames while the owner was speaking live. Stderr was also piped without a continuous drain. This explains valid 640-byte frames, intermittent success, and zero-length candidates without invoking a sample-decoding or confidence-policy change.

Wake VAD uses 20 ms frames and separates owner waiting from the bounded utterance. `WAITING_FOR_SPEECH` keeps the persistent microphone live for up to 5 seconds and retains 0.3 seconds of pre-roll; that wait does not consume the post-start safety budget. Two qualifying frames enter `RECORDING_SPEECH`. A frame below the continuation threshold enters `POSSIBLE_END_OF_SPEECH`, and 0.9 seconds of continuous calibrated quiet completes the candidate; one continuation-level frame resumes recording and records a terminal-silence reset. The listener retains 0.15 seconds of post-speech grace, accepts a 0.08-second minimum, and applies a 4-second maximum only from validated speech start. The maximum is therefore a failsafe rather than the normal endpoint. Excess leading/trailing silence is trimmed to bounded 0.24/0.20-second padding before Vosk, while PCM frame indices ensure pre-roll, speech, and post-roll are assembled once. Candidate history and the bounded pump queue use an epoch reset after each recognition, so only pre-boundary queued or in-flight frames are discarded and post-ready speech is retained. Ordinary rejected candidates do not close or recalibrate ALSA. The reliability helper resets only pre-prompt candidate history, prints `Ready for attempt N. Say 'Ares' now.`, leaves the existing PCM stream live for a 0.6-second readiness interval, and waits 0.5 seconds between completed attempts, including invalid-audio and other bounded infrastructure retries. Every completed attempt prints capture metrics before any infrastructure retry is excluded from the recognition denominator. Its normal cumulative result remains one stream open and one successful calibration. Wake diagnostics report literal qualifying-speech duration separately from the complete post-start speech window and preserve the first qualifying speech-frame index rather than substituting the later state-transition frame. A real activation deliberately closes standby capture before acknowledgement/active capture; returning after playback and the 0.35-second settling delay opens and calibrates one new standby epoch. Every open, close, calibration, and handoff carries an owner-visible reason and identifiers in diagnostics. A failed close retains the same handle and capture gate for a safe retry instead of falsely reporting ALSA released. The shared ownership gate prevents simultaneous standby and active capture. Full-command VAD and Whisper defaults remain unchanged. Candidate audio is removed by default; explicit retention keeps only the latest candidate and never implies playback. The process is foreground-only: systemd, boot startup, daemonization, barge-in, GPT, cloud fallback, and autonomous City activation remain unimplemented.

Exact configured wake phrases remain the primary STANDBY classifier. A narrowly guarded second path may collapse exactly two identical normalized alias tokens from one bounded candidate: `ares ares`, `aris aris`, or `aries aries`. Both tokens must be the same surface alias, both must map to canonical `ares`, no `[unk]`, prefix, or unrelated token may exist, the candidate must fit the configured 4-second wake bound, and the existing confidence policy must pass using one Vosk confidence entry per token and the minimum token confidence. A malformed Vosk word entry that normalizes to multiple tokens fails closed. Diagnostics expose original and canonical tokens, minimum and mean confidence, whether collapse was used, and the true recognized surface alias (`aris` remains `aris` even though the accepted identity is canonical `ares`). Mixed aliases such as `ares aris`, three repetitions, `[unk] aris`, unrelated words, and over-duration candidates remain rejected. This wake-only policy is separate from constrained ACTIVE lifecycle recognition and ordinary active Whisper.
...
The persistent standby stream has one explicit raw-PCM contract: 16,000 Hz, mono, signed 16-bit little-endian `S16_LE`; each 20 ms VAD frame contains exactly 320 samples and 640 bytes. `ContinuousPcmFrameSource` continuously drains the one existing stdout pipe, reconstructs exact frames from arbitrary short or multi-frame reads, copies them to immutable `bytes`, and retains at most 50 recent frames. If a consumer pauses, the oldest queued frame is dropped and counted instead of allowing the process pipe to back up and later masquerade as current audio. Partial bytes remain owned until a complete frame exists; odd trailing PCM, an alive-process stdout close, clean or nonzero process exit, and incomplete EOF are distinct terminal failures. Missing data is never zero-filled. A separate bounded stderr drain prevents subprocess backpressure and can never enter PCM stdout. Exact repeated non-silent payloads fail closed after a bounded run, while naturally repeated quiet frames remain diagnostic data. Owner-terminal diagnostics include read-size counts, accumulated/pending partial bytes, PID/poll state, EOF reason, queue/reset drops, non-silent duplicate and tiny-RMS runs, mutable reuse, and exact bytes delivered to VAD.

Raw `arecord -t raw` output has no header from which ARES can independently read a negotiated rate or channel layout. Diagnostics therefore label 16 kHz/mono/`S16_LE` as the requested production contract, not a measured hardware-native rate. The comparison probe uses `hw:2,0` at its known-good native 44.1 kHz for direct quiet/spoken WAVs and independently validates the 16 kHz persistent quiet/spoken WAVs. It does not relabel 44.1 kHz bytes as 16 kHz. A no-speech attempt may retain zero of the expected pre-roll without implying clipping: `beginning_clipped=false` and `beginning_clipped_status=not_applicable`. With detected speech, complete pre-roll reports `no`; only detected speech with incomplete pre-roll reports `yes`.
...
```text
input adapter -> CoreService / Brain routing -> OwnerMemoryService
              -> OwnerProfileStore -> structured response -> optional output adapter
```
...
Owner-memory management supports bounded `remember`, `recall`, `list`, `inspect`, `count`, `forget_specific`, `forget_topic`, `forget_all_general`, `forget_keyed_fact`, `confirm_delete`, and `cancel_delete` actions through the same production skill route. Specific deletion requires exactly one high-confidence active match. Multiple matches return clarification without creating a pending action. Topic deletion snapshots the matching general-memory ids and explicitly reports related keyed facts that will remain. Deleting all general memories snapshots only active general ids and always preserves keyed facts. Keyed-fact deletion targets only the normalized key and binds confirmation to a revision digest so an updated value cannot be removed under a stale prompt.

Every destructive owner-memory operation is two-step. The first request writes no owner-profile mutation; it creates one bounded `ares.pending_owner_memory_action` v1 record at repository-root `data/runtime/pending_owner_memory_action.json`. The record contains the operation, exact target ids or keyed-fact key/revision, summary, normalized request, creation time, and expiry, but no raw transcript or audio. It is atomically written, owner scoped, valid for 60 seconds by default, and intentionally survives the one-turn launcher process. `Yes, delete it`, the bounded operation-specific confirmation forms, or `Never mind` are checked before ordinary skill routing only while valid pending state exists. Unrelated commands preserve it until expiry; cancellation, successful confirmation, expiry, corruption, or invalid state clears or refuses it safely. A new destructive request replaces the prior pending request. Vague commands such as `forget it`, `delete my memory`, and `remove everything` remain clarification failures.

The profile enforces 100 keyed facts, 100 active general memories, 20 inactive revisions, 320-character owner text, 360-character canonical text, 8 topics of at most 48 characters, 5 retrieval/spoken results, and 65,536 serialized data bytes. Protected credentials, executable/system-instruction content, path-like payloads, control characters, nested objects, and ambiguous broad deletion are rejected.

`ares.owner_profile` v3 is reached sequentially. V1 imports/migrates to v2, and v2 migrates to v3 by preserving every keyed fact and adding empty `memories` plus the legacy bounded broad-confirmation field required by the v3 schema. Current CRUD confirmations use the separate transient pending-action store rather than that durable field. Loading a valid v3 profile performs no migration. Missing files mean an empty profile; malformed JSON, unknown future versions, invalid fields, and failed writes fail closed. Mutations use a per-profile transaction lock, the shared migration write lock, deterministic UTF-8 JSON, one retained last-known-good backup under `.migration_backups`, a flushed temporary file, atomic replacement, and final reload validation. Recovery is explicit; ARES never silently resets or auto-restores corrupt owner memory.
...
Run the complete confirmation and deletion lifecycle against isolated files:
...
This verifier uses fresh Brain/service instances for request and confirmation. It proves no mutation occurs before confirmation; exact, topic, all-general, and keyed-fact targets remain separated; cancellation and expiry preserve data; corrupt transient state cannot execute; existing keyed facts survive general deletion; and the transient file clears after a terminal decision.
...
`--pending` reads and validates the transient confirmation file without changing it. General-memory listing/counting never exposes raw audio or transcript history and never activates voice hardware.
...
Voice CRUD remains one foreground turn per process. For a specific deletion, run `python scripts/run_ares_voice.py`, say `Forget that I like going to the gym.`, then start a fresh process within 60 seconds and say `Yes, delete it.` To cancel instead, say `Never mind` in the second process. `Forget everything about gaming` similarly requires confirmation and removes only the exact general-memory snapshot; a keyed `favorite_game` remains. `Forget all my general long-term memories` requires `Confirm delete all general memories` and preserves every keyed fact. `Forget my favorite color` creates a separate keyed-fact confirmation and cannot delete similarly worded general memories.
...
- Bounded ambient calibration with robust mean/median/p90/peak diagnostics
- Separate speech-start, speech-continue, and silence thresholds with conservative post-speech adaptation
- Consecutive start/resume/end evidence and bounded `POSSIBLE_SILENCE` hangover
...
- Centralized general owner memory behind CoreService-owned `OwnerMemoryService`
...
- Added complete central owner-memory management actions for list, inspect, count, specific deletion, topic deletion, all-general deletion, keyed-fact deletion, confirmation, and cancellation
...
- Added an injected Capital/Core standby wake listener without moving lifecycle authority out of `BrainRuntime` and `BrainSessionManager`
...
- Real Raspberry Pi diagnosis captured `Aris, Aris, hello, Aris.` from one wake attempt; exact alias matching correctly rejected it but could not distinguish tiny-Whisper repetition from an unrelated sentence
- Kept exact matching first, then added a wake-only fallback that accepts only configured alias/prefix tokens within bounded token and repetition limits; substring and fuzzy matching remain forbidden
- Added privacy-safe classification-path/count diagnostics, owner-terminal collapsed wake representation, and explicit rejection guidance without putting the local transcript into contracts, events, or memory
- Shortened only the wake capture profile to 0.25-second pre-roll, 0.7-second terminal silence, and a two-second active cap; full-command VAD remains unchanged
- Historical Phase 95 pytest collection was 1828 tests

Phase 96

- Real Raspberry Pi tests showed tiny Whisper forcing the isolated wake word into unrelated outputs such as `Alrighty`, `Okay`, and `Bye`; accepting those as aliases would create unsafe false activation.
...
- Added one persistent ALSA stream and one calibration per standby epoch, bounded rolling pre-roll, wake-only VAD, exact `ares`/`aris`/`aries` name slots, and explicit microphone ownership handoffs.
- Ordinary timeout and rejected wake candidates reuse the same stream. Real activation still closes standby capture before acknowledgement and active-command capture, then opens a new standby epoch after return.
- Added strict `Aries` lifecycle interception, local-only wake/active diagnostics, and the ten-attempt owner-run 9/10 reliability requirement.
- Historical Phase 98 pytest collection was 1938 tests.

Phase 99

- Real Raspberry Pi reliability was 6/10. Exact constrained-grammar `ares` results at confidence `0.574` and `0.616` exposed an overly strict threshold; the apparent seven opens/calibrations came from the verifier activating and returning to standby after each accepted prompt.
...
- Historical Phase 100 collection was 1967 tests. Raspberry Pi 9/10 wake reliability and the complete production lifecycle remain owner-run requirements.

Phase 101

- Added a bounded 30-second active Whisper subprocess timeout, canonical closed-WAV validation, child terminate/kill/reap cleanup, and active-command timing diagnostics after a real Raspberry Pi run stopped at `Transcribing command`.
- Historical Phase 101 collection was 1989 tests.

Phase 102

- Real Raspberry Pi wake verification exposed a wake-only endpoint bug: frames below the continuation threshold but above the stricter silence floor repeatedly reset terminal-silence progress, producing 2.4-second maximum-duration candidates and duplicated Vosk aliases.
...
- Real Raspberry Pi startup failed with `calibration_non_speech_window_not_found`. The old guarded bootstrap required 30 consecutive 20 ms frames below a fixed RMS 200 threshold within 1.8 seconds and cleared all progress after every louder frame, so constant USB hiss could prevent a usable microphone from ever reaching `HEALTHY`.
- Standby calibration now samples a fixed three-second window, estimates ambient energy from the median of the quietest quarter, excludes robust high-energy speech bursts, and fails closed for all-zero, clipped, malformed, missing, speech-dominated, or above-limit input. One likely speech-contaminated attempt receives one bounded retry without invoking Vosk.
- Startup and the hardware verifier now distinguish model, adapter, ALSA-open attempt, current handle state, valid PCM, calibration quality, cleanup closure, and listener health. Local diagnostics expose bounded RMS distribution summaries and read-only capture/boost/AGC controls without changing hardware settings.
- Current deterministic collection is 2072 tests. Real Raspberry Pi calibration, 9/10 wake reliability, and the complete lifecycle sequence remain owner-run hardware requirements.

Phase 105

- The next real Raspberry Pi run held one healthy ALSA stream and one calibration, but no wake frame crossed the generic 658.5 RMS start gate despite post-calibration PCM activity. The remaining fault was wake-only VAD sensitivity, not microphone ownership or Vosk.
- Added immutable `conservative`, `normal`, and `sensitive` wake-only threshold policies. The default `normal` and opt-in `--wake-vad-sensitive` policies preserve ambient separation, hysteresis, two-frame speech evidence, bounded pre-roll, and silence rejection without changing active command capture.
- Added source-sequence/byte invariants and bounded per-frame diagnostic traces so the verifier distinguishes absent PCM, below-threshold PCM, candidate assembly failure, and recognizer rejection.
- Current deterministic collection is 2087 tests. The quiet-room 9/10 target and complete production lifecycle still require owner-run Raspberry Pi microphone verification.

Phase 106

- The following Raspberry Pi reliability run proved the stream and calibration stable but accepted only 6/10 prompts. Wake capture frequently reached its old 1.6-second post-start maximum, while immediate verifier pacing made it easy to speak outside the intended prompt boundary. Valid one-utterance Vosk outputs such as `ares ares` and `aris aris` were also rejected by the old 1.4-second duplicate guard.
- Wake capture now separates a 5-second owner speech wait from a 4-second post-start failsafe, preserves 0.3 seconds of pre-roll, requires two start frames, and normally completes after 0.9 seconds of continuous calibrated terminal quiet plus bounded 0.15-second grace. Diagnostics identify the speech-start time, wait duration, terminal confirmation/resets, first/last speech frames, and exact completion reason.
...
- Owner hardware evidence proved the native `hw:2,0` 44.1 kHz direct path loud and clear while the persistent path intermittently returned valid-size RMS 3-7 frames. The code trace identified the difference: direct `arecord` continuously wrote its file, while persistent stdout was drained only during foreground calibration/VAD calls. Pauses allowed stale quiet PCM and pipe backpressure; separately undrained stderr could also block the process. No wake threshold or confidence rule was changed.
- The existing one-process/one-owner architecture now wraps raw stdout in `ContinuousPcmFrameSource`. One producer reconstructs exact immutable 640-byte frames into a bounded 50-frame latest-audio queue, drops/counts oldest backlog, isolates bounded stderr, exposes PID/liveness/EOF/read-size/queue diagnostics, and fails closed on odd/incomplete terminal PCM or pathological identical non-silent runs. Epoch resets discard only pre-boundary queued/in-flight frames. Calibration and wake remain consumers of the same stream.
- A failed persistent close now retains the handle, calibration state, and capture gate until retry succeeds. Known-dead/ended transports cannot report healthy or ALSA-open. Deterministic tests prove calibration-to-live-speech continuity, post-ready speech preservation, arbitrary short/multi-frame reads, mutable ownership, dead/empty/odd EOF classification, and terminal-silence behavior.
- `manual_diagnose_persistent_pcm.py` now records separate three-second quiet/spoken native 44.1 kHz stages and separate three-second quiet/spoken production 16 kHz stages. Its single JSON report requires real headers, exact commands and process state, complete byte accounting, signal distributions, direct/persistent RMS ratios, cross-path comparability, successful playback, and explicit owner audibility confirmation. Passing deterministic tests do not claim Raspberry Pi amplitude; wake reliability remains blocked until this repaired probe passes on the owner’s hardware.
- All 2203 collected tests pass locally.

Phase 112

- Separated standby wake recognition from active-command normalization. Wake phrases can create a session only in `STANDBY`; active name-only and bounded greeting phrases return `attention_only`, preserve the existing session and inactivity deadline, bypass ordinary routing, and produce no second acknowledgement by default.
- Added whole-token outer-edge addressing plus exact bounded active lifecycle roots. Standby accepts bare or addressed `goodbye`, `bye`, `standby`, `go to standby`, `go to sleep`, and `sleep`; shutdown accepts bare or addressed `shutdown`, `turn off`, and `power down`. Common `good bye`, `stand by`, and `shut down` segmentation is canonicalized without fuzzy or substring matching. Negated and longer sentences remain ordinary.
...
- The first owner run of the new four-phrase diagnostic returned `pcm_stream_error:RuntimeError`, zero retained pre-roll, and a zero-duration candidate before Whisper. That output proves only that PCM capture failed; it does not identify the underlying RuntimeError or support any lifecycle-recognition conclusion.
- Auditing found that the diagnostic manually constructed microphone and Whisper adapters instead of using one launcher-owned production active-audio factory. Its unit tests replaced `record_until_silence()` with a fake that directly wrote a valid WAV, so they proved formatting and classification but never exercised stream initialization, a first PCM read, VAD, subprocess stderr, or close/retry ownership.
- The VAD error boundary independently discarded `str(error)` and exposed only the exception class. The exact historical exception message, ALSA stderr, and competing-owner state cannot be reconstructed from the supplied terminal output and are not guessed here.
- The repaired contract uses the same active-audio factory in the diagnostic and production launcher, preserves production's fresh bounded stream per phrase, adds a non-consuming structural preflight, retains exact typed failure evidence and diagnostic-only tracebacks, and isolates each phrase and cleanup transaction. Real Raspberry Pi acceptance is still pending a successful first complete frame, nonzero speech PCM, a candidate duration greater than zero, and an audible retained WAV. Goodbye/shutdown recognition has not yet been revalidated.
- All 2514 collected tests pass with no skips or xfails. The focused diagnostic/PCM/VAD checks, broader 1550-test audio/runtime/lifecycle/memory/event sweep, compileall, Phase 2 event/memory verification, five nonempty configuration JSON parses, and `git diff --check` are green. Four intentionally empty configuration placeholders remain excluded from JSON parsing. These deterministic results do not prove Raspberry Pi audio.

Phase 115

- The repaired production-composed probe passed its Raspberry Pi audio gate: `plughw:2,0` opened as 16 kHz mono `S16_LE`, returned a nonzero complete 640-byte frame, and captured all four phrases. Whisper correctly transcribed the calculator and owner-memory phrases, while spoken `goodbye Ares` became `*clears throat*` and spoken `shutdown Ares` became `Shut down, artist.`. That isolates a short closed-set acoustic-recognition problem and closes the PCM/capture checkpoint.
- Added `ActiveLifecycleAudioRecognizer` at the closed canonical-WAV boundary before Whisper. It uses a local constrained Vosk grammar, minimum aligned word confidence, action-specific high/medium thresholds, `[unk]`, and exact complete phrases. It reports evidence only and has no microphone, lifecycle, CoreService, or memory authority.
- High-confidence exact standby/shutdown audio bypasses Whisper. Medium-confidence audio creates a session-bound next-turn confirmation; cancellation, mismatch, expiry, or state/session change clears it without action. Low, unmatched, `[unk]`, missing-confidence, timeout, and backend-error results use ordinary Whisper on the same finalized WAV. Shutdown therefore cannot be authorized by a weak Whisper guess.
- The diagnostic runs the same constrained recognizer in compare-only mode and runs Whisper on the same finalized WAV for side-by-side evidence without a second capture or lifecycle execution. Real Raspberry Pi confirmation of constrained `goodbye Ares` and `shutdown Ares`, followed by the complete calculator/memory/standby/shutdown production sequence, remains owner-run acceptance.
- All 2,662 collected tests pass with no skips or xfails. The focused lifecycle-audio, Vosk, Whisper, pipeline, runtime, session, calculator, owner-memory, event, launcher, and diagnostic sweep passes 1,082 tests. Compileall, Phase 2 event/memory verification, parsing of all five nonempty configuration JSON files, and `git diff --check` are green. These deterministic results do not prove the owner's constrained Vosk result on Raspberry Pi.
