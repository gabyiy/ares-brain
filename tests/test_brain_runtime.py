...
def test_goodbye_after_ordinary_command_processing_stays_serialized_and_completes():
    entered = ThreadEvent()
    release = ThreadEvent()
    calls = []

    def delayed_handler(text):
        calls.append(text)
        entered.set()
        assert release.wait(2.0)
        return "ordinary response"

    runtime, _ = _runtime(handler=delayed_handler)
    _start_active(runtime)
    results = {}
    ordinary = Thread(
...
def test_shutdown_after_failed_transcription_remains_authoritative_and_bypasses_core():
    inputs = QueuedRuntimeInputAdapter()
    routed = []
    runtime, _ = _runtime(
        inputs=inputs,
    ...
    inputs.push("shutdown Ares")

    stopped = runtime.poll_once()

    assert timed_out.status == "input_timeout"
    assert stopped.status == "stopped"
    assert stopped.command_category == "shutdown"
    assert stopped.data["core_service_bypassed"] is True
    assert runtime.session_manager.state == BRAIN_STOPPED
    assert routed == []
